...
#### Core Infrastructure
![FatP CI](https://github.com/schroedermatthew/FatP/actions/workflows/fatp-test-core.yml/badge.svg)
![FatPTest CI](https://github.com/schroedermatthew/FatP/actions/workflows/fatp-test.yml/badge.svg)
![FatPBenchmarkRunner CI](https://github.com/schroedermatthew/FatP/actions/workflows/fatp-benchmark-runner.yml/badge.svg)
![FatPConcepts CI](https://github.com/schroedermatthew/FatP/actions/workflows/fatp-concepts.yml/badge.svg)
...
![CborLite CI](https://github.com/schroedermatthew/FatP/actions/workflows/cbor-lite.yml/badge.svg)
![CborStreamLite CI](https://github.com/schroedermatthew/FatP/actions/workflows/cbor-stream-lite.yml/badge.svg)
![CheckedArithmetic CI](https://github.com/schroedermatthew/FatP/actions/workflows/checked-arithmetic.yml/badge.svg)
![CircularBuffer CI](https://github.com/schroedermatthew/FatP/actions/workflows/circular-buffer.yml/badge.svg)
![Concepts CI](https://github.com/schroedermatthew/FatP/actions/workflows/concepts.yml/badge.svg)
![ConcurrencyPolicies CI](https://github.com/schroedermatthew/FatP/actions/workflows/concurrency-policies.yml/badge.svg)
...
![DiagnosticLogger_ScopeGuard CI](https://github.com/schroedermatthew/FatP/actions/workflows/diagnostic-logger-scopeguard.yml/badge.svg)
![Enforce CI](https://github.com/schroedermatthew/FatP/actions/workflows/enforce.yml/badge.svg)
![EnforcedInit CI](https://github.com/schroedermatthew/FatP/actions/workflows/enforced-init.yml/badge.svg)
![EnhancedBoundsChecking CI](https://github.com/schroedermatthew/FatP/actions/workflows/enhanced-bounds-checking.yml/badge.svg)
![EnumPlus CI](https://github.com/schroedermatthew/FatP/actions/workflows/enum-plus.yml/badge.svg)
![EqualityAny CI](https://github.com/schroedermatthew/FatP/actions/workflows/equality-any.yml/badge.svg)
![EqualityComparisons CI](https://github.com/schroedermatthew/FatP/actions/workflows/equality-comparisons.yml/badge.svg)
...
## Desktop sync workflow (commit, push, sync)

Fat-P development often happens in **agent worktrees** or secondary clones (Cursor, Grok, CI sandboxes). The **canonical local tree** Matthew uses day to day is the Desktop clone:

```
C:\Users\mtthw\Desktop\AI Projects\FatP
```

A worktree can commit and push to `origin/main` while Desktop still points at an older commit. The scripts in `tools/` close that gap: after every push, **fast-forward Desktop to `origin/main`** so the folder on disk matches what GitHub has.

This is the same pattern as SandBox's `ulib/scripts/sync-desktop-sandbox.ps1`, adapted for Fat-P.

### Scripts

| Script | What it does |
|--------|----------------|
| [`tools/commit-push-sync-desktop.ps1`](tools/commit-push-sync-desktop.ps1) | **Primary workflow:** `git add` → `git commit` → `git push` → sync Desktop |
| [`tools/push-and-sync-desktop.ps1`](tools/push-and-sync-desktop.ps1) | `git push` then sync Desktop (commit already done) |
| [`tools/sync-desktop-fatp.ps1`](tools/sync-desktop-fatp.ps1) | Sync only: `git fetch origin main` + fast-forward Desktop |
| [`tools/install-git-hooks.ps1`](tools/install-git-hooks.ps1) | Install `post-push` hook so **every** `git push` auto-syncs Desktop |

All scripts are PowerShell. Run them from the **Fat-P repo root** (any clone or worktree).

### `commit-push-sync-desktop.ps1` (usual entry point)

Stages files, commits with your message, pushes to `origin main` (by default), then runs the sync script.
...
1. `git fetch origin main` on the Desktop path.
2. If Desktop `HEAD` already equals `origin/main`, print the SHA and **exit successfully** — even if Desktop has other uncommitted edits (common when you commit from Desktop itself and unrelated workflow files are dirty).
3. If Desktop is **behind** `origin/main` and the working tree is **clean**, `git merge --ff-only origin/main`.
4. If Desktop is behind and the working tree is **dirty**, sync **aborts** with local/remote SHAs and instructions. Fix by committing or stashing on Desktop, or force-reset:

```powershell
.\tools\sync-desktop-fatp.ps1 -Force   # discard Desktop local edits; reset --hard to origin/main
```

**Desktop path override** (non-default install location):
...
If the Desktop path does not exist or is not a git repo, sync prints a warning and exits 0 (no-op).

### `install-git-hooks.ps1` (auto-sync after push)
...
Copies [`tools/git-hooks/post-push`](tools/git-hooks/post-push) into `.git/hooks/post-push`. After every successful `git push`, the hook invokes `sync-desktop-fatp.ps1` via PowerShell so Desktop stays current even when you forget the manual sync step.
