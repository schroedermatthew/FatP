...
|--------|--------------|---------|----------|---------|
| `StandardJsonPolicy` | Compact | ❌ Reject | ❌ No | APIs, data exchange |
| `PrettyJsonPolicy` | Indented | ❌ Reject | ❌ No | Human-readable output |
| `CompatJsonPolicy` | Compact | ✅ Allow | ❌ No | Scientific computing |
| `ConfigJsonPolicy` | Compact | ❌ Reject | ✅ Parse | Configuration files |
...
**Maps must have string keys** because JSON object keys are always strings:

```cpp
std::map<std::string, int> ok;  // ✅ Works
std::map<int, std::string> no;  // ❌ Won't compile - JSON can't have integer keys
```
...
| Computed fields (area = width × height) | Custom `to_json()` |
