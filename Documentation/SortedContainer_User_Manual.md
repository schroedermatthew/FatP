...
**Dependencies:** Expected.h, ConcurrencyPolicies.h, EqualityComparisons.h, enforce.h, CheckedArithmetic.h, EnforcedInit.h, TypeTraits.h, ScopeGuard.h, DiagnosticLogger_Core.h
...
**Warning:** In multi-threaded code, the returned iterator becomes invalid when the lock is released. Use `findCopy()` or `findApply()` instead.
...
Binary search returning a **copy** of the found element (or `nullopt`).

**Why:** Safe for multi-threaded use—the copied value outlives the lock, avoiding use-after-free issues with iterators.

**When:** In read-heavy multi-threaded environments where you need to use the value outside the critical section.

```cpp
auto opt = container.findCopy(42);
if (opt) {
    process(*opt);  // Value is copied, safe after lock release
}
```

**Complexity:** O(log N) + copy of T
...
Find an element and execute a callback while holding the lock.

**Why:** Allows complex operations on found elements without copying, while maintaining thread safety.

**Returns:** `true` if element was found and callback executed, `false` otherwise.

```cpp
container.findApply(42, [](const int& val) {
    std::cout << "Found: " << val << "\n";
    // Lock held during this entire lambda
});
```

**Complexity:** O(log N) + callback execution time
...
Manually check the container's sorted invariant. In debug builds, this is called automatically after each mutation. In release builds, it's a no-op.

**When to use:** Explicitly verify invariant in unit tests or after unusual operations.

```cpp
#ifndef NDEBUG
container.validateInvariant();  // Throws if unsorted (debug only)
#endif
```

**Note:** This method is automatically called by `insert()`, `insertRange()`, `erase()`, and `clear()` in debug builds. You rarely need to call it manually.
...
```mermaid
flowchart TD
    A[insert value] --> B[Binary search for position]
    B --> C{Which Policy?}
    
    C --> D[AllowDuplicatesPolicy]
    D --> E[Use upper_bound]
    E --> F[Insert at position]
    F --> G[Return true]
    
    C --> H[OnlyUniquePolicy]
    H --> I{Exact duplicate exists?}
    I -- Yes --> J[Return false]
    I -- No --> K[Insert at position]
    K --> G
    
    C --> L[FuzzyUniquePolicy]
    L --> M{Within epsilon of neighbor?}
    M -- Yes --> J
    M -- No --> K
```
...
**Understanding epsilon parameters:** `HybridComparisonPolicy` requires two epsilon values:
- **Absolute tolerance** (first eps): Used for comparisons near zero where relative differences explode
- **Relative tolerance** (second eps): Used for large magnitudes where absolute differences are misleading

For most applications, passing the same value for both works well. Use different values only when domain-specific tuning is needed (e.g., mixing very small and very large measurements). See `EqualityComparisons.h` for details on each comparison policy.

**Available equality policies:**
- `ExactComparisonPolicy` — Bitwise equality (no epsilon)
- `AbsoluteComparisonPolicy` — |a - b| < ε (single epsilon)
- `RelativeComparisonPolicy` — |a - b| / max(|a|, |b|) < ε (single epsilon)
- `HybridComparisonPolicy` — Combines absolute and relative (two epsilons, recommended)
- `ULPComparisonPolicy` — Units in Last Place comparison (single ULP count)
...
sequenceDiagram
    ...
    Note over ThreadA,ThreadB: UNSAFE PATTERN
    ThreadA->>Container: begin() [acquires lock]
    Container-->>ThreadA: iterator [releases lock]
    ThreadB->>Container: insert() [acquires lock]
    Container-->>ThreadB: success [releases lock]
    Note over Container: Vector reallocated!
    ThreadA->>Container: *iterator
    Note over ThreadA: Use-after-free!
```
...
sequenceDiagram
    ...
    Note over ThreadA,ThreadB: SAFE PATTERN
    ThreadA->>Container: withInternalContainer(lambda)
    activate Container
    Note over Container: Lock held
    ThreadA->>Container: iterate safely
    ThreadB->>Container: insert() [blocks]
    Note over ThreadB: Waiting for lock...
    ThreadA->>Container: lambda returns
    deactivate Container
    Note over Container: Lock released
    ThreadB->>Container: insert() proceeds
```
...
All benchmarks were run in Release build (`/O2 /DNDEBUG`) using FatPTest.h's `measure_perf()` for accurate timing. Each operation was measured over 10 iterations after 3 warmup runs, reporting the average time. `DoNotOptimize()` was used to prevent compiler elision of results. Variability was <5% across runs.

```cpp
// Example benchmark pattern (from test_SortedContainer.cpp)
...
*boost::flat_set is template-based and usable header-only, though some Boost.Container features may require linking.
...
**Key difference:** `insert()` returns `Expected<bool>`, not `std::pair<iterator, bool>`.
