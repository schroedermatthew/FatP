...
### The Mechanism: SFINAE Priority Chain

```cpp
template<typename T>
std::string stringify(const T& value) {
    // Priority 1: .toString() method
    if constexpr (has_toString_method_v<T>) {
        return value.toString();
    }
    // Priority 2: .to_string() method
    else if constexpr (has_to_string_method_v<T>) {
        return value.to_string();
    }
    // Priority 3: ADL to_string()
    else if constexpr (has_adl_to_string_v<T>) {
        using std::to_string;
        return to_string(value);
    }
    // Priority 4: operator<< for std::ostream
    else if constexpr (is_ostream_insertable_v<T>) {
        std::ostringstream oss;
        oss << value;
        return oss.str();
    }
    // Priority 5: Container iteration
    else if constexpr (is_iterable_v<T>) {
        return stringify_container(value);
    }
    else {
        static_assert(always_false<T>, "No stringify method found");
    }
}
```
...
stringify(CustomType{});  // Uses .toString() → "custom"
stringify(LegacyType{});  // Uses operator<< → "legacy"
stringify(42);            // Uses std::to_string → "42"
```

**Mechanism:** `if constexpr` chains check traits at compile time. Only the matching branch is instantiated.
...
stringify(s);  // "{1.1, 2.2, 3.3}"
```

**Mechanism:** `is_iterable_v<T>` detects `begin()`/`end()`. Map-like containers (with `key_type`/`mapped_type`) use `key: value` format.

### 3. Nested Container Support

```cpp
std::vector<std::vector<int>> nested = {{1, 2}, {3, 4}};
stringify(nested);  // "[[1, 2], [3, 4]]"

std::map<std::string, std::vector<int>> complex = {
    {"evens", {2, 4, 6}},
    {"odds", {1, 3, 5}}
};
stringify(complex);  // "{evens: [2, 4, 6], odds: [1, 3, 5]}"
```

**Mechanism:** Recursive `stringify()` calls handle nesting. Recursion depth is limited (100 debug, 200 release) to prevent stack overflow on cyclic structures.

### 4. Tuple/Pair/Optional Support

```cpp
std::pair<int, std::string> p = {42, "hello"};
stringify(p);  // "(42, hello)"

std::tuple<int, double, std::string> t = {1, 2.5, "three"};
stringify(t);  // "(1, 2.5, three)"

std::optional<int> opt = 42;
stringify(opt);  // "42"

std::optional<int> empty;
stringify(empty);  // "nullopt"
```
...
opts.quoted_strings = true;

stringify(3.14159, opts);    // "3.14e+00"
stringify("hello", opts);    // "\"hello\""
```
...
// User provides specialization
template<>
struct EnumStringifier<Color> {
    static std::string stringify(Color c) {
        switch (c) {
...
stringify(Color::Green);  // "Green" (not "1")
```

**Mechanism:** `EnumStringifier<T>` trait is checked before falling back to integer conversion.
...
Stringify.h
    ↓ uses
TypeTraits.h   (detection idiom helpers)
    ↓ used by
...
### 1. Permanence
The standard will never provide universal stringification—too many design choices with no consensus. Stringify makes the decision once with sensible defaults and trait-based escape hatches.

### 2. Specialization
Compile-time method detection via SFINAE ensures zero runtime dispatch. The priority chain (toString → to_string → ADL → operator<< → container) reflects common C++ patterns. Recursion depth limits prevent stack overflow.
...
*Stringify.h (906 lines) — Fat-P Library*
