...
3. [When to Use FatPTest](#when-to-use-fatptest)
...
22. [Limitations and Design Trade-offs](#limitations-and-design-trade-offs)

---

## Introduction

**FatPTest** is a zero-dependency, header-only testing framework for C++17 and later. It provides comprehensive testing and benchmarking facilities—all without requiring external dependencies beyond the C++ standard library.
...
- **Advanced benchmarking** with percentiles, outliers, and baseline comparison
...
- **Zero dependencies** over framework sophistication
- **Deterministic execution** over parallel performance
- **Ease of debugging** over advanced capabilities
- **Integrated benchmarking** as a first-class feature
...
### FatPTest vs. GoogleTest

| Feature | FatPTest | GoogleTest |
|---------|----------|------------|
| **Dependencies** | Zero (header-only) | Requires installation/linking |
| **Basic Assertions** | ✅ 18+ macros | ✅ Extensive |
| **Floating-Point** | ✅ Primitive (sufficient for testing) | ✅ Production-grade |
| **Exception Testing** | ✅ `ASSERT_THROWS`, `ASSERT_NO_THROW` | ✅ `EXPECT_THROW`, etc. |
| **Fixtures** | ✅ Setup/TearDown | ✅ SetUp/TearDown |
| **Parameterized Tests** | ✅ `run_parameterized_test` | ✅ `INSTANTIATE_TEST_SUITE_P` |
| **Death Tests** | ❌ Not supported | ✅ `EXPECT_DEATH` |
| **Mocking** | ❌ Not supported | ✅ GoogleMock integration |
| **Benchmarking** | ✅ Built-in with statistics | ❌ Requires Google Benchmark |
| **Timeout Support** | ✅ `run_test_with_timeout` | ✅ `--gtest_timeout` |
| **Flakiness Detection** | ✅ `run_test_repeat`, `run_until_failure` | ✅ `--gtest_repeat` |
| **Test Filtering** | ✅ Wildcard patterns | ✅ `--gtest_filter` |
| **JUnit XML** | ✅ Built-in | ✅ `--gtest_output=xml` |
| **Colored Output** | ✅ ANSI codes | ✅ Platform-specific |
| **Parallel Execution** | ❌ Single-threaded | ✅ Thread-safe |
| **Auto-Discovery** | ❌ Manual registration | ✅ Automatic |
| **Installation** | Copy header file | CMake/package manager |

### FatPTest vs. Catch2

| Feature | FatPTest | Catch2 |
|---------|----------|---------|
| **Dependencies** | Zero | Single header (v2) or modular (v3) |
| **Style** | Explicit assertions | BDD-style (`SECTION`, `SCENARIO`) |
| **Assertions** | 18+ explicit macros | Natural expressions with `REQUIRE` |
| **Benchmarking** | ✅ Statistical analysis | ✅ Catch2 Benchmark (v2.13+) |
| **Generators** | ❌ Not supported | ✅ `GENERATE` macro |
| **Matchers** | ❌ Not supported | ✅ Extensive matcher library |
| **Compilation Time** | Fast | Moderate (improving in v3) |
| **Header Size** | ~1400 lines | ~8000+ lines (v2) |

### FatPTest vs. Boost.Test

| Feature | FatPTest | Boost.Test |
|---------|----------|------------|
| **Dependencies** | Zero | Boost library |
| **Installation** | Header-only | Requires Boost installation |
| **Learning Curve** | Gentle | Steeper (comprehensive framework) |
| **Assertions** | 18+ macros | Extensive suite |
| **Fixtures** | ✅ Simple | ✅ Comprehensive |
| **Benchmarking** | ✅ Built-in | ❌ Requires external tools |
| **Subtests** | ✅ `SUBTEST` macro | ✅ Test cases within suites |
| **Compilation Time** | Fast (minimal) | Slow (Boost headers) |
...
FatPTest is header-only. Simply copy `FatPTest.h` to your project:

```bash
# Copy to your test directory
cp FatPTest.h /path/to/your/project/tests/
```
...
bool test_basic_arithmetic()
{
    ...
    ASSERT_NE(5, 3, "Five is not three");
    ASSERT_LT(1, 10, "1 < 10");
    return true;
}
...
int main()
{
    ...
    runner.run_test("basic_arithmetic", test_basic_arithmetic);
...
**Output:**
```
Running: basic_arithmetic ... PASSED (0.023 ms)
...
**Purpose**: Measure and print performance in a formatted way.
...
```
std::sort:
  Average time per operation: 2.345 µs
  Total for 100000 iterations: 234.5 ms

std::stable_sort:
  Average time per operation: 3.123 µs
  Total for 100000 iterations: 312.3 ms
```
...
**Purpose**: Detailed benchmark with full statistics and baseline comparison.
...
**Purpose**: Compare two functions and show speedup/slowdown.
...
Comparing: std::vector reserve vs std::vector no reserve
  std::vector reserve: 8.234 µs
  std::vector no reserve: 12.456 µs
...
**Limitation**: Wildcard pattern matching is limited to recursion depth of 100.

**Impact**: Patterns with >100 consecutive wildcards may fail.

**Workaround**: Simplify patterns or use multiple simpler patterns.

---

### 7. No Automatic Test Discovery
...
FatPTest provides a comprehensive, zero-dependency testing framework for C++17 and later. It offers:

- **Simplicity**: Easy integration, no external dependencies
- **Completeness**: Assertions, fixtures, parameterized tests, benchmarking
- **Practicality**: CI/CD integration, timeout support, flakiness detection
- **Independence**: No circular dependencies, self-contained infrastructure
- **Integrated Performance Testing**: Built-in benchmarking with statistical analysis

### Project Suitability Assessment

**FatPTest is well-suited for:**
- Projects requiring zero external dependencies
- Embedded or constrained environments
- Projects valuing integrated benchmarking
- Small to medium test suites
- Rapid prototyping and development
- Teams preferring simplicity and transparency

**Consider alternatives for:**
- Very large test suites requiring fast parallel execution
- Projects requiring advanced features (death tests, mocking)
- Teams with established GoogleTest workflows
- Enterprise projects with complex testing requirements

### Making the Choice

The decision to use FatPTest should be based on:
1. **Dependency constraints** (can you/should you use external frameworks?)
2. **Test suite scale** (execution time acceptable with single-threading?)
3. **Feature requirements** (need death tests, mocking, etc.?)
4. **Team preferences** (value simplicity or need advanced features?)
5. **Performance testing needs** (integrated benchmarking valuable?)

### Getting Help

For issues, questions, or contributions:
1. Review this manual thoroughly
2. Check examples in the test suite
