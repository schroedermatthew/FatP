...
|--------------|---------|
| Generation counter | Detects stale handles (ABA safety) |
...
// Stale handle safely returns nullptr
...
// Fast: for pre-validated handles in hot loops
...
| ABA safety | ❌ None | ❌ None | ❌ None | ✅ Generation counters |
...
3. **Design philosophy:** The standard library avors general containers over specialized ones
...
- **Simple use cases:** For non-deletable collections, `std::vector` is simpler
...
### 1. Permanence
The standard library will never provide this container—it's too domain-specific. SlotMap makes a game-engine-grade data structure available as a single header, permanently solving the "safe handles to deletable objects" problem.
...
### 3. Control
Checked vs. unchecked access lets developers choose their safety/performance tradeoff. Hot paths use `get_unchecked()`; general code uses `get()`. No runtime configuration—the choice is explicit at each call site.

**Architectural Verdict:** SlotMap transforms the "safe handle to deletable object" problem from scattered `shared_ptr` usage or risky raw pointers into a **single-container solution** with O(1) operations, ABA safety, and cache-friendly iteration. It's the data structure game engines use—now available without the engine.
