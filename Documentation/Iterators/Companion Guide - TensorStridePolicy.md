...
**Historical context:** Inspired by NumPy's strided array model; adapted for C++ zero-overhead principle
...
| Tier | Policy | Overhead | Dimensions |
|------|--------|----------|------------|
| 1 | TensorStridePolicy | 1.7-3.1x | Any N |
| 2 | Stride2DPolicy | ~1.1x | 2D row-major |
| 3 | Stride1DPolicy | ~1.0x | 1D strided |
...
Both versions compile to nearly identical assembly. Stride2DPolicy adds ~10% overhead from the branch at row boundaries.
...
| Operation | Manual | Stride1D | Stride2D | TensorStride |
|-----------|--------|----------|----------|--------------|
| Sequential | 0.89 ms | 0.89 ms | 0.98 ms | 1.52 ms |
| Stride-4 | 0.23 ms | 0.23 ms | — | 0.39 ms |
| 2D contiguous | 0.89 ms | — | 0.98 ms | 1.52 ms |
| 2D padded | 0.92 ms | — | 1.01 ms | 1.78 ms |
| 3D contiguous | 0.91 ms | — | — | 2.56 ms |

### Overhead Sources

| Source | Impact | Mitigation |
|--------|--------|------------|
| Odometer loop | 50-150% | Contiguous detection |
| SmallVector access | 10-20% | Inline storage |
| Branch at row end | 5-10% | Predictable pattern |
| Position tracking | 5-10% | Necessary for end detection |
