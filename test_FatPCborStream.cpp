...
{
    fat_p::DefaultStreamParser parser;

    auto data = encode_uint(42);
    auto result = parser.parse(data);

    SIMPLE_ASSERT(result.has_value(), "Parse succeeded");
    SIMPLE_ASSERT(result->is_unsigned(), "Is unsigned");
    SIMPLE_ASSERT(result->as_unsigned() == 42, "Value 42");
...
TEST_CASE(default_parser_array)
{
    ...
    SIMPLE_ASSERT(result->as_array().size() == 3, "Array size");
...
{
    fat_p::StrictStreamParser parser;

    auto data = encode_text("hello");
    auto result = parser.parse(data);

    SIMPLE_ASSERT(result.has_value(), "Parse succeeded");
    SIMPLE_ASSERT(result->is_string(), "Is string");
    SIMPLE_ASSERT(result->as_string() == "hello", "Value hello");
...
{
    fat_p::RelaxedStreamParser parser;

    auto data = encode_uint(12345);
    auto result = parser.parse(data);

    SIMPLE_ASSERT(result.has_value(), "Parse succeeded");
    SIMPLE_ASSERT(result->as_unsigned() == 12345, "Value 12345");
...
TEST_CASE(stream_parse_convenience)
{
    ...
    auto result = fat_p::stream_parse(data);

    SIMPLE_ASSERT(result.has_value(), "Parse succeeded");
    SIMPLE_ASSERT(result->as_unsigned() == 100, "Value 100");
...
TEST_CASE(stream_parse_strict_convenience)
{
    ...
    auto result = fat_p::stream_parse_strict(data);

    SIMPLE_ASSERT(result.has_value(), "Parse succeeded");
    SIMPLE_ASSERT(result->as_string() == "test", "Value test");
...
TEST_CASE(stream_parse_limited_convenience)
{
    ...
    auto result = fat_p::stream_parse_limited(data, limits);

    SIMPLE_ASSERT(result.has_value(), "Parse succeeded");
    SIMPLE_ASSERT(result->as_unsigned() == 999, "Value 999");
...
{
    fat_p::StrictStreamParser parser;  // max_depth = 32
    ...
    SIMPLE_ASSERT(result.error().code == ParseError::MaxDepthExceeded, "Depth error");
...
{
    fat_p::StrictStreamParser parser;  // max_string_bytes = 64KB
    ...
    SIMPLE_ASSERT(!result.has_value(), "Parse failed");
    SIMPLE_ASSERT(result.error().code == ParseError::MaxStringSizeExceeded, "String size error");
...
TEST_CASE(configurable_limits)
{
    ...
    fat_p::ConfigurableStreamParser parser(limits);
...
TEST_CASE(utf8_validation_invalid)
{
    ...
    auto result = parser.parse(data);
    SIMPLE_ASSERT(!result.has_value(), "Invalid UTF-8 rejected");
    SIMPLE_ASSERT(result.error().code == ParseError::InvalidUtf8, "UTF-8 error");
...
TEST_CASE(utf8_validation_overlong)
{
    ...
    auto result = parser.parse(data);
    SIMPLE_ASSERT(!result.has_value(), "Overlong rejected");
...
TEST_CASE(chunked_feeding)
{
    ...
    SIMPLE_ASSERT(*status == ParseStatus::Done, "Parsing done");

    // Verify result
    const auto& result = parser.result();
    SIMPLE_ASSERT(result.is_array(), "Is array");
    SIMPLE_ASSERT(result.as_array().size() == 2, "Array size 2");
...
TEST_CASE(chunked_feeding_error)
{
    ...
    for (size_t i = 0; i < data.size(); ++i)
    {
        ...
        {
            got_error = true;
            SIMPLE_ASSERT(status.error().code == ParseError::MaxDepthExceeded, "Depth error");
            break;
        }
    }

    SIMPLE_ASSERT(got_error, "Error detected during chunked feed");
...
TEST_CASE(error_information)
{
    ...
    SIMPLE_ASSERT(error.code == ParseError::MaxDepthExceeded, "Correct error code");
    ...
    SIMPLE_ASSERT(error_str.find("depth") != std::string::npos, "Error string has context");
...
TEST_CASE(using_macro)
{
    ...
    SIMPLE_ASSERT(result.has_value(), "Parse with macro succeeded");
    SIMPLE_ASSERT(result->as_unsigned() == 123, "Value 123");
...
void benchmark_policies()
{
    ...
    fat_p::DefaultStreamParser default_parser;
    ...
    fat_p::ValidatingStreamParser validating_parser;
    ...
    fat_p::StrictStreamParser strict_parser;
