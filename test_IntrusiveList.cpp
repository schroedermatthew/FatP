...
bool test_intrusive_list_empty() {
    ...
    SIMPLE_ASSERT(list.empty(), "List should be empty");
    SIMPLE_ASSERT(list.size() == 0, "List should have size 0");
...
bool test_intrusive_list_push_front() {
    ...
    list.push_front(n1);
    SIMPLE_ASSERT(list.size() == 1, "Size should be 1");
    SIMPLE_ASSERT(list.front().value == 1, "Front should be n1");
    
    list.push_front(n2);
    SIMPLE_ASSERT(list.size() == 2, "Size should be 2");
    SIMPLE_ASSERT(list.front().value == 2, "Front should be n2");
    
    list.push_front(n3);
    SIMPLE_ASSERT(list.size() == 3, "Size should be 3");
    SIMPLE_ASSERT(list.front().value == 3, "Front should be n3");
...
bool test_intrusive_list_push_back() {
    ...
    list.push_back(n1);
    SIMPLE_ASSERT(list.size() == 1, "Size should be 1");
    SIMPLE_ASSERT(list.back().value == 1, "Back should be n1");
    
    list.push_back(n2);
    SIMPLE_ASSERT(list.size() == 2, "Size should be 2");
    SIMPLE_ASSERT(list.back().value == 2, "Back should be n2");
    
    list.push_back(n3);
    SIMPLE_ASSERT(list.size() == 3, "Size should be 3");
    SIMPLE_ASSERT(list.back().value == 3, "Back should be n3");
...
bool test_intrusive_list_iteration() {
    ...
    SIMPLE_ASSERT(values.size() == 3, "Should iterate 3 times");
    SIMPLE_ASSERT(values[0] == 1, "First value should be 1");
    SIMPLE_ASSERT(values[1] == 2, "Second value should be 2");
    SIMPLE_ASSERT(values[2] == 3, "Third value should be 3");
...
bool test_intrusive_list_remove() {
    ...
    SIMPLE_ASSERT(values.size() == 2, "Should have 2 elements");
    SIMPLE_ASSERT(values[0] == 1, "First should be 1");
    SIMPLE_ASSERT(values[1] == 3, "Second should be 3");
    
    // Remove front
    list.remove(n1);
    SIMPLE_ASSERT(list.size() == 1, "Size should be 1");
    SIMPLE_ASSERT(list.front().value == 3, "Front should be 3");
...
bool test_intrusive_list_insert() {
    ...
    SIMPLE_ASSERT(values[2] == 3, "Third should be 3");
    
    // Insert at end
    list.insert(list.end(), n4);
    SIMPLE_ASSERT(list.size() == 4, "Size should be 4");
    SIMPLE_ASSERT(list.back().value == 4, "Back should be 4");
...
bool test_intrusive_list_erase() {
    ...
    SIMPLE_ASSERT(list.size() == 2, "Size should be 2");
    SIMPLE_ASSERT(next->value == 3, "Next should point to n3");
...
bool test_intrusive_list_clear() {
    ...
    SIMPLE_ASSERT(list.empty(), "List should be empty");
    SIMPLE_ASSERT(list.size() == 0, "Size should be 0");
    
    // Nodes should be unlinked
    SIMPLE_ASSERT(!n1.is_linked(), "n1 should not be linked");
    SIMPLE_ASSERT(!n2.is_linked(), "n2 should not be linked");
    SIMPLE_ASSERT(!n3.is_linked(), "n3 should not be linked");
...
bool test_intrusive_list_splice() {
    ...
    SIMPLE_ASSERT(list1.size() == 4, "list1 should have 4 elements");
    SIMPLE_ASSERT(list2.empty(), "list2 should be empty");
    ...
    SIMPLE_ASSERT(values[1] == 2, "Second should be 2");
    SIMPLE_ASSERT(values[2] == 3, "Third should be 3");
    SIMPLE_ASSERT(values[3] == 4, "Fourth should be 4");
...
bool test_intrusive_list_move() {
    ...
    SIMPLE_ASSERT(list1.empty(), "list1 should be empty after move");
    SIMPLE_ASSERT(list2.size() == 2, "list2 should have 2 elements");
    SIMPLE_ASSERT(list2.front().value == 1, "Front should be 1");
    
    // Move assignment
    IntrusiveList<TestNode> list3;
    list3 = std::move(list2);
    
    SIMPLE_ASSERT(list2.empty(), "list2 should be empty after move");
    SIMPLE_ASSERT(list3.size() == 2, "list3 should have 2 elements");
