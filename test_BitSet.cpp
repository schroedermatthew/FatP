...
TEST_CASE(basic_operations)
{
    ...
    SIMPLE_ASSERT(!bits.any(), "any() should return false for empty set");
    ...
    bits.set(10);
    bits.flip(10);
    SIMPLE_ASSERT(!bits.test(10), "Bit 10 should be flipped to 0");

    bits.flip(20);
    SIMPLE_ASSERT(bits.test(20), "Bit 20 should be flipped to 1");
...
TEST_CASE(set_with_value)
{
    ...
    bits.set(10, true);
    SIMPLE_ASSERT(bits.test(10), "Bit 10 should be set");

    bits.set(10, false);
    SIMPLE_ASSERT(!bits.test(10), "Bit 10 should be cleared");

    bits.set(20, true);
    bits.set(20, true);
    SIMPLE_ASSERT(bits.test(20), "Bit 20 should remain set");
...
TEST_CASE(subscript_operator)
{
    ...
    SIMPLE_ASSERT(bits[0], "bits[0] should be true");
    SIMPLE_ASSERT(bits[63], "bits[63] should be true");
    SIMPLE_ASSERT(bits[64], "bits[64] should be true");
    SIMPLE_ASSERT(bits[127], "bits[127] should be true");
    SIMPLE_ASSERT(!bits[1], "bits[1] should be false");
    SIMPLE_ASSERT(!bits[100], "bits[100] should be false");
...
TEST_CASE(unchecked_operations)
{
    ...
    SIMPLE_ASSERT(bits.test_unchecked(0), "Bit 0 should be set");
    SIMPLE_ASSERT(bits.test_unchecked(100), "Bit 100 should be set");
    SIMPLE_ASSERT(bits.test_unchecked(255), "Bit 255 should be set");
    SIMPLE_ASSERT(!bits.test_unchecked(50), "Bit 50 should not be set");

    bits.flip_unchecked(100);
    SIMPLE_ASSERT(!bits.test_unchecked(100), "Bit 100 should be flipped");

    bits.clear_unchecked(0);
    SIMPLE_ASSERT(!bits.test_unchecked(0), "Bit 0 should be cleared");
...
TEST_CASE(size_one)
{
    ...
    SIMPLE_ASSERT(bits.none(), "Should start empty");
    ...
    bits.flip_all();
    SIMPLE_ASSERT(bits.none(), "Should be empty after flip");
...
TEST_CASE(size_63)
{
    ...
    bits.clear(62);
    SIMPLE_ASSERT(!bits.all(), "all() should return false after clearing one bit");
...
TEST_CASE(size_65)
{
    ...
    bits.clear(64);
    ASSERT_EQ(bits.count(), 64u, "Count should be 64");
    SIMPLE_ASSERT(!bits.all(), "all() should return false");
...
TEST_CASE(bulk_operations)
{
    ...
    SIMPLE_ASSERT(!bits.test(10), "Bit 10 should be flipped");
    SIMPLE_ASSERT(!bits.test(20), "Bit 20 should be flipped");
    SIMPLE_ASSERT(!bits.test(30), "Bit 30 should be flipped");
...
TEST_CASE(reset_alias)
{
    ...
    bits.set_all();
    bits.reset();
    SIMPLE_ASSERT(bits.none(), "reset() should clear all bits");

    bits.set(10);
    bits.reset(10);
    SIMPLE_ASSERT(!bits.test(10), "reset(index) should clear that bit");
...
TEST_CASE(set_range)
{
    ...
    SIMPLE_ASSERT(bits.test(10), "Bit 10 should be set");
    SIMPLE_ASSERT(bits.test(19), "Bit 19 should be set");
    SIMPLE_ASSERT(!bits.test(9), "Bit 9 should not be set");
    SIMPLE_ASSERT(!bits.test(20), "Bit 20 should not be set");
...
TEST_CASE(clear_range)
{
    ...
    SIMPLE_ASSERT(!bits.test(10), "Bit 10 should be cleared");
    SIMPLE_ASSERT(!bits.test(19), "Bit 19 should be cleared");
    SIMPLE_ASSERT(bits.test(9), "Bit 9 should still be set");
    SIMPLE_ASSERT(bits.test(20), "Bit 20 should still be set");
...
TEST_CASE(iteration_empty)
{
    ...
    SIMPLE_ASSERT(indices.empty(), "Empty bitset should yield no iterations");
...
TEST_CASE(bitwise_and)
{
    ...
    SIMPLE_ASSERT(result.test(2), "Bit 2 should be set");
    SIMPLE_ASSERT(result.test(3), "Bit 3 should be set");
    SIMPLE_ASSERT(!result.test(1), "Bit 1 should not be set");
    SIMPLE_ASSERT(!result.test(4), "Bit 4 should not be set");
...
TEST_CASE(bitwise_or)
{
    ...
    SIMPLE_ASSERT(result.test(1), "Bit 1 should be set");
    SIMPLE_ASSERT(result.test(2), "Bit 2 should be set");
    SIMPLE_ASSERT(result.test(3), "Bit 3 should be set");
    SIMPLE_ASSERT(result.test(4), "Bit 4 should be set");
...
TEST_CASE(bitwise_xor)
{
    ...
    SIMPLE_ASSERT(result.test(1), "Bit 1 should be set");
    SIMPLE_ASSERT(result.test(4), "Bit 4 should be set");
    SIMPLE_ASSERT(!result.test(2), "Bit 2 should not be set");
    SIMPLE_ASSERT(!result.test(3), "Bit 3 should not be set");
...
TEST_CASE(bitwise_not)
{
    ...
    SIMPLE_ASSERT(!result.test(0), "Bit 0 should not be set");
    SIMPLE_ASSERT(!result.test(63), "Bit 63 should not be set");
    SIMPLE_ASSERT(result.test(1), "Bit 1 should be set");
    SIMPLE_ASSERT(result.test(32), "Bit 32 should be set");
...
TEST_CASE(compound_assignment)
{
    ...
    SIMPLE_ASSERT(test_and.test(2), "Bit 2 should be set");
...
TEST_CASE(is_subset_of)
{
    ...
    SIMPLE_ASSERT(a.is_subset_of(b), "a should be subset of b");
    SIMPLE_ASSERT(!b.is_subset_of(a), "b should not be subset of a");

    fat_p::BitSet<64> empty;
    SIMPLE_ASSERT(empty.is_subset_of(a), "Empty set is subset of any set");
    SIMPLE_ASSERT(a.is_subset_of(a), "Set is subset of itself");
...
TEST_CASE(intersects)
{
    ...
    SIMPLE_ASSERT(a.intersects(b), "a and b should intersect at bit 2");
    SIMPLE_ASSERT(!a.intersects(c), "a and c should not intersect");

    fat_p::BitSet<64> empty;
    SIMPLE_ASSERT(!a.intersects(empty), "No set intersects empty set");
...
TEST_CASE(equality)
{
    ...
    SIMPLE_ASSERT(a == b, "a and b should be equal");
    SIMPLE_ASSERT(!(a != b), "a and b should not be not-equal");
    SIMPLE_ASSERT(a != c, "a and c should not be equal");
    SIMPLE_ASSERT(!(a == c), "a and c should not be equal");

    fat_p::BitSet<64> empty1, empty2;
    SIMPLE_ASSERT(empty1 == empty2, "Empty sets should be equal");
...
TEST_CASE(initializer_list_constructor)
{
    ...
    SIMPLE_ASSERT(bits.test(5), "Bit 5 should be set");
    SIMPLE_ASSERT(bits.test(10), "Bit 10 should be set");
    SIMPLE_ASSERT(bits.test(15), "Bit 15 should be set");
    SIMPLE_ASSERT(bits.test(99), "Bit 99 should be set");
    SIMPLE_ASSERT(!bits.test(0), "Bit 0 should not be set");

    fat_p::BitSet<64> empty{};
    SIMPLE_ASSERT(empty.none(), "Empty initializer list should yield empty set");
...
TEST_CASE(copy_operations)
{
    ...
    SIMPLE_ASSERT(copied.test(10), "Copied bit 10");
    SIMPLE_ASSERT(copied.test(100), "Copied bit 100");

    copied.set(50);
    SIMPLE_ASSERT(!original.test(50), "Original should not be affected");
...
TEST_CASE(std_hash)
{
    ...
    SIMPLE_ASSERT(hasher(a) != hasher(c), "Different sets should likely have different hashes");
