...
bool test_boolean_fast_path() {
    ...
    SIMPLE_ASSERT(fast_ns < 100.0, "Boolean fast path should be under 100ns");
...
bool test_float_fast_path() {
    ...
    SIMPLE_ASSERT(fast_ns / baseline_ns < 2.0, 
...
bool test_is_stringifiable_trait() {
    ...
    // Arithmetic types
    SIMPLE_ASSERT(is_stringifiable_v<int>, "int should be stringifiable");
    SIMPLE_ASSERT(is_stringifiable_v<double>, "double should be stringifiable");
    SIMPLE_ASSERT(is_stringifiable_v<bool>, "bool should be stringifiable");
    
    // Enums (fixed in v2.2)
    SIMPLE_ASSERT(is_stringifiable_v<Color>, "enum with stringifier should be stringifiable");
    SIMPLE_ASSERT(is_stringifiable_v<Size>, "enum without stringifier should be stringifiable");
    
    // Custom types
    SIMPLE_ASSERT(is_stringifiable_v<CustomStringifiable>, 
                  "Custom with toString() should be stringifiable");
    SIMPLE_ASSERT(is_stringifiable_v<CustomStringifiable2>, 
                  "Custom with to_string() should be stringifiable");
    SIMPLE_ASSERT(is_stringifiable_v<CustomWithStreamOp>, 
                  "Custom with operator<< should be stringifiable");
    
    // Containers
    SIMPLE_ASSERT(is_stringifiable_v<std::vector<int>>, 
                  "vector should be stringifiable");
    SIMPLE_ASSERT((is_stringifiable_v<std::map<std::string, int>>), 
                  "map should be stringifiable");
    
    // Pair and tuple
    SIMPLE_ASSERT((is_stringifiable_v<std::pair<int, int>>), 
                  "pair should be stringifiable");
    SIMPLE_ASSERT((is_stringifiable_v<std::tuple<int, double>>), 
                  "tuple should be stringifiable");
    
    // Optional
    SIMPLE_ASSERT(is_stringifiable_v<std::optional<int>>, 
                  "optional should be stringifiable");
    
    // Non-stringifiable
    SIMPLE_ASSERT(!is_stringifiable_v<NonStreamable>, 
...
bool test_thread_safety_stress() {
    ...
    SIMPLE_ASSERT(error_count.load() == 0, "No exceptions in thread-safe operations");
    SIMPLE_ASSERT(success_count.load() == NUM_THREADS * ITERATIONS_PER_THREAD,
...
bool test_thread_local_error_independence() {
    ...
    SIMPLE_ASSERT(threads_with_correct_error.load() == NUM_THREADS,
