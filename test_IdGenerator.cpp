...
TEST_CASE(guard_default_ctor)
{
    ...
    Guard default_guard;
    SIMPLE_ASSERT(!default_guard, "Default guard should be invalid (operator bool)");
    ...
    // Move assignment from valid guard
    {
        ...
        SIMPLE_ASSERT(scoped.has_value(), "scoped_id should succeed");
        
        default_guard = std::move(scoped.value());
        SIMPLE_ASSERT(static_cast<bool>(default_guard), "Guard should be valid after move");
    ...
    SIMPLE_ASSERT(!default_guard, "Guard should be invalid after release_ownership");
...
TEST_CASE(random_small_type)
{
    ...
    for (size_t i = 0; i < 50; ++i)
    {
        ...
        if (id.has_value())
        {
            ...
            if (generated_ids.find(id.value()) != generated_ids.end())
            {
                ...
                SIMPLE_ASSERT(false, "Generator returned duplicate ID without error");
            }
            generated_ids.insert(id.value());
            ++success_count;
        }
        else
        {
            ...
            SIMPLE_ASSERT(id.error() == IdError::AlreadyInUse,
                          "Collision should return AlreadyInUse error");
    ...
    SIMPLE_ASSERT(success_count > 0, "Should generate at least some unique IDs");
...
TEST_CASE(random_seed_reproducibility)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        SIMPLE_ASSERT(id1.has_value() && id2.has_value(), "Generation should succeed");
        seq1.push_back(*id1);
        seq2.push_back(*id2);
    }
    ...
    SIMPLE_ASSERT(different.has_value(), "Generation should succeed");
    SIMPLE_ASSERT(*different != seq1[0], "Different seed should produce different sequence");
...
TEST_CASE(random_idgenerator_seeded)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        SIMPLE_ASSERT(id1.has_value() && id2.has_value(), "Seeded generation should succeed");
        seq1.push_back(*id1);
        seq2.push_back(*id2);
    }
    ...
    SIMPLE_ASSERT(different.has_value(), "Generation should succeed");
    SIMPLE_ASSERT(*different != seq1[0], "Different seed should produce different first ID");
...
TEST_CASE(concurrent_queries)
{
    ...
    SIMPLE_ASSERT(query_count > 0, "Query threads should have executed");
    SIMPLE_ASSERT(generate_count > 0, "Generate threads should have executed");
...
TEST_CASE(retry_logic_collision)
{
    ...
    for (size_t i = 0; i < max_attempts && success_count < 50; ++i)
    {
        ...
        if (id.has_value())
        {
            ...
            SIMPLE_ASSERT(generated.find(id.value()) == generated.end(),
    ...
    SIMPLE_ASSERT(success_count >= 20, "Retry loop should allow multiple successful generations");
...
TEST_CASE(basic_batch_generation)
{
    ...
    SIMPLE_ASSERT(batch.has_value(), "Batch generation should succeed");
    ...
    SIMPLE_ASSERT(empty_batch.has_value(), "Empty batch should succeed");
...
TEST_CASE(batch_with_recycling)
{
    ...
    SIMPLE_ASSERT(batch.has_value(), "Batch with recycling should succeed");
...
TEST_CASE(batch_overflow_rollback)
{
    ...
    SIMPLE_ASSERT(batch1.has_value(), "First batch should succeed");
    ...
    SIMPLE_ASSERT(!batch2.has_value(), "Batch exceeding space should fail");
    SIMPLE_ASSERT(batch2.error() == IdError::Overflow, "Error should be Overflow");

    // Enhanced verification: Check individual ID states after rollback
    for (uint8_t id : {250, 251, 252, 253, 254})
    {
        SIMPLE_ASSERT(gen.is_active(id), "Original IDs should remain active after rollback");
    }
    ...
    SIMPLE_ASSERT(last.has_value(), "Should still generate remaining ID");
...
TEST_CASE(release_batch_basic)
{
    ...
    SIMPLE_ASSERT(result.has_value(), "Batch release should succeed");
...
TEST_CASE(release_batch_error_handling)
{
    ...
    SIMPLE_ASSERT(!result.has_value(), "Should fail on invalid ID");
    SIMPLE_ASSERT(result.error() == IdError::InvalidRelease, "Error should be InvalidRelease");

    // First ID should be released, but second and third not processed
    SIMPLE_ASSERT(!gen.is_active(id1.value()), "First ID should be released before error");
    SIMPLE_ASSERT(gen.is_active(id2.value()), "Second ID should still be active");
    SIMPLE_ASSERT(gen.is_active(id3.value()), "Third ID should still be active (not processed)");
...
TEST_CASE(double_release)
{
    ...
    auto id = gen.generate();
    SIMPLE_ASSERT(id.has_value(), "Generation should succeed");

    auto release1 = gen.release(id.value());
    SIMPLE_ASSERT(release1.has_value(), "First release should succeed");

    auto release2 = gen.release(id.value());
    SIMPLE_ASSERT(!release2.has_value(), "Second release should fail");
    SIMPLE_ASSERT(release2.error() == IdError::InvalidRelease, "Should get InvalidRelease");
...
TEST_CASE(overflow_boundary)
{
    ...
    SIMPLE_ASSERT(!id4.has_value(), "Should fail on overflow");
    SIMPLE_ASSERT(id4.error() == IdError::Overflow, "Should get Overflow error");
...
TEST_CASE(overflow_exhaustion_tracking)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        SIMPLE_ASSERT(!overflow.has_value(), "Should fail on overflow");
        SIMPLE_ASSERT(overflow.error() == IdError::Overflow, 
    ...
    SIMPLE_ASSERT(!overflow2.has_value(), "Should still fail on overflow");
    SIMPLE_ASSERT(overflow2.error() == IdError::Overflow, "Should be Overflow");
...
TEST_CASE(batch_rollback_reverts_counter)
{
    ...
    SIMPLE_ASSERT(!batch.has_value(), "Batch should fail");
    SIMPLE_ASSERT(batch.error() == IdError::Overflow, "Should be overflow");
    ...
    SIMPLE_ASSERT(id251_retry.has_value(), "Should get recycled 251");
    ASSERT_EQ(id251_retry.value(), uint8_t(251), "Should be 251");

    auto id252 = gen.generate();
    SIMPLE_ASSERT(id252.has_value(), "Should get 252");
...
TEST_CASE(custom_allocation_policy)
{
    ...
    SIMPLE_ASSERT(id1.has_value(), "Custom policy generation should succeed");
    SIMPLE_ASSERT((id1.value() % 2) == 0, "ID should be even");
    SIMPLE_ASSERT((id2.value() % 2) == 0, "ID should be even");
    SIMPLE_ASSERT((id3.value() % 2) == 0, "ID should be even");
...
TEST_CASE(bounded_allocation)
{
    ...
    SIMPLE_ASSERT(!overflow.has_value(), "Should overflow after max bound");
...
TEST_CASE(dirty_max_smaller_id)
{
    ...
    SIMPLE_ASSERT(id5.has_value(), "Generate should succeed");
    ...
    SIMPLE_ASSERT(id25.has_value(), "Generate should succeed");
    ASSERT_EQ(id25.value(), uint64_t(25), "Fourth ID is 25");
    
    // Final state: {5, 10, 25}
    ASSERT_EQ(gen.active_count(), size_t(3), "Should have 3 active IDs");
    SIMPLE_ASSERT(gen.is_active(5), "5 should be active");
    SIMPLE_ASSERT(gen.is_active(10), "10 should be active");
    SIMPLE_ASSERT(gen.is_active(25), "25 should be active");
...
TEST_CASE(active_id_tracking)
{
    ...
    SIMPLE_ASSERT(id1.has_value() && id2.has_value() && id3.has_value(), "Generation should succeed");
    ...
    auto id4 = gen.generate();
    SIMPLE_ASSERT(id4.has_value(), "Generation after max release should succeed");
...
TEST_CASE(lazy_max_recompute)
{
    ...
    SIMPLE_ASSERT(new_id.has_value(), "Generation should succeed");
...
TEST_CASE(dirty_max_insert)
{
    ...
    SIMPLE_ASSERT(id10.has_value() && id11.has_value() && id12.has_value(), "Generate should succeed");
    ...
    SIMPLE_ASSERT(id14.has_value(), "Generate should succeed");
    ASSERT_EQ(id14.value(), uint64_t(14), "Policy continues advancing");
    
    // Verify correct tracking - should have {10, 14}
    ASSERT_EQ(gen.active_count(), size_t(2), "Should have 2 active IDs");
    SIMPLE_ASSERT(gen.is_active(10), "ID 10 should be active");
    SIMPLE_ASSERT(gen.is_active(14), "ID 14 should be active");
    SIMPLE_ASSERT(!gen.is_active(11), "ID 11 should not be active");
    SIMPLE_ASSERT(!gen.is_active(12), "ID 12 should not be active");
    SIMPLE_ASSERT(!gen.is_active(13), "ID 13 should not be active");
...
{
    std::cout << "\n" << colors::cyan() << "IdGenerator Benchmarks:" << colors::reset() << "\n\n";
    ...
    // Sequential generation benchmark
    {
        ...
        double gen_time = measure_perf(
            ...
            kStandardWarmup);
        std::cout << "Generate (sequential): " << format_time(gen_time) << "\n";
        ...
        double rel_time = measure_perf(
            ...
            kBenchmarkIterations,
            kStandardWarmup);
        std::cout << "Release: " << format_time(rel_time) << "\n";
    }

    // Recycled generation benchmark
    {
        SimpleIdGenerator<uint64_t> gen(1);
        std::vector<uint64_t> ids;
        ids.reserve(kBenchmarkIterations);

        // Pre-populate recycled pool
        ...
        double recycle_time = measure_perf(
            ...
            kBenchmarkIterations,
            kStandardWarmup);
        std::cout << "Generate (from recycle pool): " << format_time(recycle_time) << "\n";
    }

    // Thread-safe generator benchmark
    {
        ThreadSafeIdGenerator<uint64_t> safe_gen(1);
        double safe_time = measure_perf(
            ...
            kReducedIterations,
            kStandardWarmup);
        std::cout << "Generate (thread-safe): " << format_time(safe_time) << "\n";
    }

    // is_active query benchmark
    {
        SimpleIdGenerator<uint64_t> gen(1);
        std::vector<uint64_t> ids;
        ...
        double query_time = measure_perf(
            ...
            kBenchmarkIterations,
            kStandardWarmup);
        std::cout << "is_active query: " << format_time(query_time) << "\n";
    }

    // =========================================================================
    // Batch Operation Benchmarks
    // =========================================================================
    std::cout << "\n" << colors::cyan() << "Batch Operations:" << colors::reset() << "\n\n";

    // Batch generate benchmark
    {
        SimpleIdGenerator<uint64_t> gen(1);
        ...
        double batch_gen_time = measure_perf(
            ...
            kBatchIterations,
            kStandardWarmup / 10);
        std::cout << "Batch generate (100 IDs): " << format_time(batch_gen_time) << " per batch\n";
    }

    // Thread-safe batch generate benchmark
    {
        ThreadSafeIdGenerator<uint64_t> gen(1);
        constexpr size_t kBatchSize = 100;
        ...
        double ts_batch_gen_time = measure_perf(
            ...
            kBatchIterations,
            kStandardWarmup / 10);
        std::cout << "Batch generate (thread-safe, 100 IDs): " << format_time(ts_batch_gen_time) << " per batch\n";
    }

    // =========================================================================
    // Baseline Comparison: std::set vs ActiveIdTracker
    // =========================================================================
    std::cout << "\n" << colors::cyan() << "Baseline Comparison (std::set):" << colors::reset() << "\n\n";

    // std::set insert benchmark
    {
        ...
        double set_insert_time = measure_perf(
            ...
            kBenchmarkIterations,
            kStandardWarmup);
        std::cout << "std::set insert: " << format_time(set_insert_time) << "\n";
        ...
        double set_max_time = measure_perf(
            ...
            kBenchmarkIterations,
            kStandardWarmup);
        std::cout << "std::set max (rbegin): " << format_time(set_max_time) << "\n";
    }

    // ActiveIdTracker insert + max benchmark (direct comparison)
    {
        ActiveIdTracker<uint64_t> tracker;
        uint64_t counter = 1;
        ...
        double tracker_max_time = measure_perf(
            ...
            kBenchmarkIterations,
            kStandardWarmup);
        std::cout << "ActiveIdTracker max (cached): " << format_time(tracker_max_time) << "\n";
    }
}
