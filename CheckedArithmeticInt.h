    ...
    noexcept(PolicyTraits<Policy>::template is_noexcept<T>)
{
    ...
    {
        // Standard idiom for unsigned overflow detection
        overflow = (result < a);
    }
...
[[nodiscard]] PolicyReturnType<Policy, std::vector<T>> checked_add_vec(
    ...
    noexcept(PolicyTraits<Policy>::template is_noexcept<T>)
{
    ...
    {
        auto scalar_fn = [](T a, T b) { return checked_add<Policy>(a, b); };
        
        bool success = int_simd::checked_add_vec_simd<T, Policy>(
            vec_a.data(), vec_b.data(), result.data(), n, scalar_fn);
 ...
 */
template <typename Policy = ThrowOnErrorPolicy, typename T>
[[nodiscard]] PolicyReturnType<Policy, std::vector<T>> checked_sub_vec(
    const std::vector<T>& vec_a, 
    const std::vector<T>& vec_b)
    noexcept(PolicyTraits<Policy>::template is_noexcept<T>)
{
    ...
    if constexpr (std::is_same_v<T, int32_t> || std::is_same_v<T, uint32_t> || 
                  std::is_same_v<T, int64_t>)
    {
        auto scalar_fn = [](T a, T b) { return checked_sub<Policy>(a, b); };
        
        bool success = int_simd::checked_sub_vec_simd<T, Policy>(
            vec_a.data(), vec_b.data(), result.data(), n, scalar_fn);
 ...
 */
template <typename Policy = ThrowOnErrorPolicy, typename T>
[[nodiscard]] PolicyReturnType<Policy, std::vector<T>> checked_mul_vec(
    const std::vector<T>& vec_a, 
    const std::vector<T>& vec_b)
    noexcept(PolicyTraits<Policy>::template is_noexcept<T>)
{
    ...
    if constexpr (std::is_same_v<T, int32_t> || std::is_same_v<T, uint32_t>)
    {
        auto scalar_fn = [](T a, T b) { return checked_mul<Policy>(a, b); };
        
        bool success = int_simd::checked_mul_vec_simd<T, Policy>(
            vec_a.data(), vec_b.data(), result.data(), n, scalar_fn);
