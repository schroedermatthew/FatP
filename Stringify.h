...
namespace detail {
    ...
    template <typename T>
    using op_has_value = decltype(std::declval<T>().has_value());
#endif
...
#if STRINGIFY_HAS_OPTIONAL
    else if constexpr (is_detected_v<detail::op_has_value, PlainT>) {
        // std::optional detected
...
inline constexpr bool is_stringifiable_v = 
    ...
    #if STRINGIFY_HAS_OPTIONAL
    is_detected_v<detail::op_has_value, T> ||
    #endif
    #if FATP_HAS_CPP20
