...
{
    FATP_CONTEXTUAL_ENFORCE_3(&throwing_check_in_range, fat_p::InRangePredicate, 
                         value, min_val, max_val, "Value out of range");
}

void noexcept_check_in_range(int value, int min_val, int max_val) noexcept
{
    FATP_CONTEXTUAL_ENFORCE_3(&noexcept_check_in_range, fat_p::InRangePredicate,
                         value, min_val, max_val, "Value out of range");
}
...
{
    FATP_CONTEXTUAL_ENFORCE_2(&throwing_check_less_than, fat_p::IsLessThanPredicate,
                         a, b, "First value not less than second");
}

void noexcept_check_less_than(int a, int b) noexcept
{
    FATP_CONTEXTUAL_ENFORCE_2(&noexcept_check_less_than, fat_p::IsLessThanPredicate,
                         a, b, "First value not less than second");
}
...
{
    FATP_CONTEXTUAL_ABORT_3(&throwing_abort_in_range, fat_p::InRangePredicate,
                       value, min_val, max_val, "Value out of range (abort)");
}

void throwing_abort_less_than(int a, int b)
{
    FATP_CONTEXTUAL_ABORT_2(&throwing_abort_less_than, fat_p::IsLessThanPredicate,
...
FATP_TEST_CASE(contextual_expected_integration) {
    ...
    auto result1 = FATP_CONTEXTUAL_ENFORCE_EXPECTED(&test_contextual_expected_integration,
                                                true, "Should pass");
    ...
    auto result2 = FATP_CONTEXTUAL_ENFORCE_EXPECTED(&test_contextual_expected_integration,
                                                false, "Should fail");
...
FATP_TEST_CASE(expected_no_exceptions) {
    // Test 1: FATP_ENFORCE_EXPECTED with passing condition
    {
        ...
        FATP_ASSERT_TRUE(result.has_value(), 
                      "FATP_ENFORCE_EXPECTED should return Ok for true condition");
    }
    ...
    // Test 2: FATP_ENFORCE_EXPECTED with failing condition
    {
        auto result = FATP_ENFORCE_EXPECTED(false, "Should fail");
        FATP_ASSERT_TRUE(!result.has_value(), 
                      "FATP_ENFORCE_EXPECTED should return Err for false condition");
    ...
    // Test 3: FATP_ALWAYS_ENFORCE_EXPECTED
    {
        ...
        FATP_ASSERT_TRUE(result_pass.has_value(), 
                      "FATP_ALWAYS_ENFORCE_EXPECTED should return Ok for true");
        
        auto result_fail = FATP_ALWAYS_ENFORCE_EXPECTED(false, "Fail");
        FATP_ASSERT_TRUE(!result_fail.has_value(), 
                      "FATP_ALWAYS_ENFORCE_EXPECTED should return Err for false");
    }
    
    // Test 4: FATP_ENFORCE_PREDICATE_EXPECTED
    {
        int positive = 42;
        auto result = FATP_ENFORCE_PREDICATE_EXPECTED(fat_p::IsPositivePredicate, 
                                                 positive, "Should be positive");
        FATP_ASSERT_TRUE(result.has_value(), 
                      "FATP_ENFORCE_PREDICATE_EXPECTED should return Ok for positive value");
...
FATP_TEST_CASE(expected_contextual_noexcept) {
    ...
    FATP_ASSERT_TRUE(result.has_value(), 
                  "FATP_CONTEXTUAL_ENFORCE_EXPECTED in noexcept function should work");
...
FATP_TEST_CASE(expected_predicate_variants) {
    // Test 1: FATP_CONTEXTUAL_ENFORCE_EXPECTED_1 (single argument predicate)
    {
        ...
        FATP_ASSERT_TRUE(result.has_value(), 
                      "FATP_CONTEXTUAL_ENFORCE_EXPECTED_1 should return Ok");
        FATP_ASSERT_TRUE(result.value() == true,
                      "FATP_CONTEXTUAL_ENFORCE_EXPECTED_1 should return predicate result");
    }
    ...
    // Test 2: FATP_CONTEXTUAL_ENFORCE_EXPECTED_2 (two argument predicate)
    {
        ...
        auto test_func = [&]() noexcept -> Expected<bool, std::string> {
            return FATP_CONTEXTUAL_ENFORCE_EXPECTED_2(&test_func,
        ...
        auto result = test_func();
        FATP_ASSERT_TRUE(result.has_value(),
                      "FATP_CONTEXTUAL_ENFORCE_EXPECTED_2 should return Ok");
        FATP_ASSERT_TRUE(result.value() == true,
                      "FATP_CONTEXTUAL_ENFORCE_EXPECTED_2 should return predicate result");
    }
    ...
    // Test 3: FATP_CONTEXTUAL_ENFORCE_EXPECTED_3 (three argument predicate)
    {
        ...
        auto result = test_func();
        FATP_ASSERT_TRUE(result.has_value(),
                      "FATP_CONTEXTUAL_ENFORCE_EXPECTED_3 should return Ok");
        FATP_ASSERT_TRUE(result.value() == true,
                      "FATP_CONTEXTUAL_ENFORCE_EXPECTED_3 should return predicate result");
    }
...
FATP_TEST_CASE(performance_enforce_overhead) {
    ...
    for (int i = 0; i < 1000; ++i) {
        FATP_ENFORCE(true, "Warmup");
    }
    
    auto start = high_resolution_clock::now();
    for (int i = 0; i < iterations; ++i) {
        FATP_ENFORCE(true, "Performance test");
    }
    ...
    #ifdef NDEBUG
    FATP_ASSERT_TRUE(ns_per_call < 10.0, "FATP_ENFORCE() has minimal overhead in release build");
    std::cout << "    -> FATP_ENFORCE() overhead: " << ns_per_call << " ns/call (release)\n";
    #else
    FATP_ASSERT_TRUE(ns_per_call < 500.0, "FATP_ENFORCE() overhead acceptable in debug build");
    std::cout << "    -> FATP_ENFORCE() overhead: " << ns_per_call << " ns/call (debug)\n";
    #endif
...
FATP_TEST_CASE(performance_always_enforce_overhead) {
    ...
    auto start = high_resolution_clock::now();
    for (int i = 0; i < iterations; ++i) {
        FATP_ALWAYS_ENFORCE(true, "Performance test");
    }
    auto end = high_resolution_clock::now();
    auto duration = duration_cast<nanoseconds>(end - start).count();
    ...
    FATP_ASSERT_TRUE(ns_per_call < 500.0, "FATP_ALWAYS_ENFORCE() overhead is reasonable");
    std::cout << "    -> FATP_ALWAYS_ENFORCE() overhead: " << ns_per_call << " ns/call\n";
...
FATP_TEST_CASE(edge_case_multiple_types) {
    try {
        FATP_ALWAYS_ENFORCE(false, "Int:", 42, " Double:", 3.14, 
