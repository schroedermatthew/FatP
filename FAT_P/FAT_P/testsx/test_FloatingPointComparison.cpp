...
FATP_TEST_CASE(ulp_exact_boundaries)
{
        ...
        FATP_ASSERT_TRUE((floatEqual<float, UlpComparisonPolicy>(a, b, 1.0f)),
            "Exactly 1 ULP apart with tolerance 1.0");
        FATP_ASSERT_FALSE((floatEqual<float, UlpComparisonPolicy>(a, b, 0.5f)),
        ...
        FATP_ASSERT_TRUE((floatEqual<float, UlpComparisonPolicy>(a, c)),
            "Exactly 4 ULPs with default tolerance");

        // 5 ULPs apart - should fail with default
        float d = std::nextafter(c, 2.0f);
        FATP_ASSERT_FALSE((floatEqual<float, UlpComparisonPolicy>(a, d)),
            "5 ULPs exceeds default tolerance");

        // Double precision test
        double da = 1.0;
        double db = std::nextafter(da, 2.0);
        FATP_ASSERT_TRUE((floatEqual<double, UlpComparisonPolicy>(da, db, 1.0)),
...
FATP_TEST_CASE(ulp_negative_values)
{
        ...
        FATP_ASSERT_TRUE((floatEqual<double, UlpComparisonPolicy>(neg_a, neg_b, 1.0)),
        ...
        FATP_ASSERT_TRUE((floatEqual<double, UlpComparisonPolicy>(neg_a, neg_c, 3.0)),
            "Negative: 3 ULPs apart");
        FATP_ASSERT_FALSE((floatEqual<double, UlpComparisonPolicy>(neg_a, neg_c, 2.0)),
...
FATP_TEST_CASE(ulp_subnormals)
{
        ...
        FATP_ASSERT_TRUE((floatEqual<float, UlpComparisonPolicy>(denorm_f, denorm_f)),
            "Float subnormal equals itself");

        // Double subnormals
        constexpr double denorm_d = std::numeric_limits<double>::denorm_min();
        FATP_ASSERT_TRUE((floatEqual<double, UlpComparisonPolicy>(denorm_d, denorm_d)),
            "Double subnormal equals itself");

        // Near-zero hybrid handling
        FATP_ASSERT_TRUE((floatEqual<float, UlpComparisonPolicy>(0.0f, denorm_f)),
...
FATP_TEST_CASE(ulp_sign_sensitivity)
{
        ...
        FATP_ASSERT_FALSE((floatEqual<float, UlpComparisonPolicy>(a, b, 1000.0f)),
...
FATP_TEST_CASE(relative_default_epsilon)
{
        ...
        FATP_ASSERT_TRUE((floatEqual<double, RelativeComparisonPolicy>(a, b)),
...
FATP_TEST_CASE(relative_zero_comparisons)
{
        ...
        FATP_ASSERT_TRUE((floatEqual<double, RelativeComparisonPolicy>(0.0, 0.0)),
            "Zero equals zero");
        FATP_ASSERT_TRUE((floatEqual<double, RelativeComparisonPolicy>(0.0, -0.0)),
            "+0 equals -0");
        ...
        constexpr double eps = getDefaultEpsilon<double>();
        FATP_ASSERT_FALSE((floatEqual<double, RelativeComparisonPolicy>(0.0, eps)),
...
FATP_TEST_CASE(hybrid_near_zero_robustness)
{
        ...
        FATP_ASSERT_TRUE(approximateEqual(a, b, rel_eps, abs_eps),
...
FATP_TEST_CASE(hybrid_parameter_flexibility)
{
        ...
        FATP_ASSERT_TRUE(approximateEqual(a, b, eps, eps),
...
FATP_TEST_CASE(policy_differences)
{
        ...
        FATP_ASSERT_TRUE(floatEqual(a, b, 1e-6),
            "Standard allows sign crossing");
        FATP_ASSERT_TRUE(approximateEqual(a, b, 1e-6, 1e-6),
            "Hybrid allows sign crossing");

        // Relative/ULP are strict about signs
        FATP_ASSERT_FALSE((floatEqual<double, RelativeComparisonPolicy>(a, b, 1e-6)),
            "Relative rejects sign crossing");
        FATP_ASSERT_FALSE((floatEqual<double, UlpComparisonPolicy>(a, b, 1000.0)),
...
FATP_TEST_CASE(scale_transition_points)
{
        ...
        double tiny_a = 1e-14;
        double tiny_b = 2e-14;
        FATP_ASSERT_TRUE(approximateEqual(tiny_a, tiny_b, rel_eps, abs_eps),
            "Tiny scale: absolute dominates");

        // Medium: transition zone
        double med_a = 1e-9;
        double med_b = 1e-9 + 5e-13;
        FATP_ASSERT_TRUE(approximateEqual(med_a, med_b, rel_eps, abs_eps),
            "Medium scale: transition zone");

        // Large: relative dominates
        double large_a = 1e6;
        double large_b = 1e6 + 0.1;
        FATP_ASSERT_TRUE(approximateEqual(large_a, large_b, rel_eps, abs_eps),
...
FATP_TEST_CASE(mixed_signs_noise_floor)
{
        ...
        FATP_ASSERT_TRUE(approximateEqual(pos_noise, zero, noise_floor, noise_floor),
            "Positive noise equals zero");
        FATP_ASSERT_TRUE(approximateEqual(neg_noise, zero, noise_floor, noise_floor),
...
FATP_TEST_CASE(near_zero_crossing_stability)
{
        ...
        for (double val : oscillating) {
            FATP_ASSERT_TRUE(approximateEqual(val, 0.0, noise_floor, noise_floor),
...
FATP_TEST_CASE(approximate_equal_zero_handling)
{
        ...
        FATP_ASSERT_TRUE(approximateEqual(zero, tiny, eps, eps),
...
FATP_TEST_CASE(pathological_cases)
{
        ...
        FATP_ASSERT_TRUE(approximateEqual(third, approx_third, 1e-10, 1e-12),
            "1/3 approximation");

        // Values near sqrt(2)
        double sqrt2_calc = std::sqrt(2.0);
        double sqrt2_approx = 1.41421356237;
        FATP_ASSERT_TRUE(approximateEqual(sqrt2_calc, sqrt2_approx, 1e-10, 1e-11),
...
FATP_TEST_CASE(control_system_noise)
{
        ...
        FATP_ASSERT_TRUE(approximateEqual(frame_N, setpoint, noise_floor, noise_floor),
            "Control: +Noise equals setpoint");
        FATP_ASSERT_TRUE(approximateEqual(frame_N1, setpoint, noise_floor, noise_floor),
            "Control: -Noise equals setpoint");
