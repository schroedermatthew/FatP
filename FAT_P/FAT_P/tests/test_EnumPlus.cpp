...
bool test_has_flag_utility()
{
    ...
    ASSERT_TRUE(has_flag(perms, FilePermission::Read), "Should have Read flag");
    ASSERT_TRUE(has_flag(perms, FilePermission::Write), "Should have Write flag");
    ASSERT_TRUE(!has_flag(perms, FilePermission::Execute), "Should not have Execute flag");

    auto wrapped = EnumPlusWrapper<FilePermission>(perms);
    ASSERT_TRUE(has_flag(wrapped, FilePermission::Read), "Wrapped should have Read");
    ASSERT_TRUE(!has_flag(wrapped, FilePermission::Execute), "Wrapped should not have Execute");

    auto wrapped_flag = EnumPlusWrapper<FilePermission>(FilePermission::Read);
    ASSERT_TRUE(has_flag(wrapped, wrapped_flag), "Wrapper with wrapper flag should work");
    return true;
}
...
bool test_enum_plus_wrapper()
{
    ...
    ASSERT_TRUE(color == Color::Red, "Wrapper should equal Color::Red");
    ASSERT_TRUE(color != Color::Blue, "Wrapper should not equal Color::Blue");

    EnumPlusWrapper<Color> color2(Color::Red);
    ASSERT_TRUE(color == color2, "Two wrappers with same value should be equal");
    return true;
}
...
{
    ASSERT_TRUE(is_valid_enum<Color>(0), "0 should be valid Color (Red)");
    ASSERT_TRUE(is_valid_enum<Color>(1), "1 should be valid Color (Green)");
    ASSERT_TRUE(is_valid_enum<Color>(2), "2 should be valid Color (Blue)");
    ASSERT_TRUE(is_valid_enum<Color>(3), "3 should be valid Color (Yellow)");
    ASSERT_TRUE(!is_valid_enum<Color>(-1), "-1 should be invalid Color");
    ASSERT_TRUE(!is_valid_enum<Color>(4), "4 should be invalid Color");
    ASSERT_TRUE(!is_valid_enum<Color>(100), "100 should be invalid Color");

    ASSERT_TRUE(is_valid_enum(Color::Red), "Color::Red should be valid");
    ASSERT_TRUE(is_valid_enum(Color::Yellow), "Color::Yellow should be valid");

    Color invalid = static_cast<Color>(100);
    ASSERT_TRUE(!is_valid_enum(invalid), "Cast from 100 should be invalid");
    return true;
}
...
bool test_safe_enum_cast()
{
    ...
    ASSERT_TRUE(r1.has_value(), "0 should cast to valid Color");
    ASSERT_EQ(*r1, Color::Red, "0 should cast to Color::Red");

    auto r2 = safe_enum_cast<Color>(3);
    ASSERT_TRUE(r2.has_value(), "3 should cast to valid Color");
    ASSERT_EQ(*r2, Color::Yellow, "3 should cast to Color::Yellow");

    auto r3 = safe_enum_cast<Color>(-1);
    ASSERT_TRUE(!r3.has_value(), "-1 should not cast to valid Color");

    auto r4 = safe_enum_cast<Color>(4);
    ASSERT_TRUE(!r4.has_value(), "4 should not cast to valid Color");

    auto r5 = safe_enum_cast<Color>(100);
    ASSERT_TRUE(!r5.has_value(), "100 should not cast to valid Color");
    return true;
}
...
bool test_safe_to_underlying()
{
    ...
    ASSERT_TRUE(r1.has_value(), "Red should have valid underlying");
    ASSERT_EQ(*r1, 0, "Red underlying should be 0");

    auto r2 = safe_to_underlying(Color::Yellow);
    ASSERT_TRUE(r2.has_value(), "Yellow should have valid underlying");
    ASSERT_EQ(*r2, 3, "Yellow underlying should be 3");

    Color invalid = static_cast<Color>(100);
    auto r3 = safe_to_underlying(invalid);
    ASSERT_TRUE(!r3.has_value(), "Invalid enum should not have valid underlying");
    return true;
}
...
bool test_enum_index()
{
    ...
    ASSERT_TRUE(idx0.has_value(), "Red should have valid index");
    ASSERT_EQ(*idx0, 0u, "Red index should be 0");

    auto idx1 = enum_index(Color::Green);
    ASSERT_TRUE(idx1.has_value(), "Green should have valid index");
    ASSERT_EQ(*idx1, 1u, "Green index should be 1");

    auto idx2 = enum_index(Color::Blue);
    ASSERT_TRUE(idx2.has_value(), "Blue should have valid index");
    ASSERT_EQ(*idx2, 2u, "Blue index should be 2");

    auto idx3 = enum_index(Color::Yellow);
    ASSERT_TRUE(idx3.has_value(), "Yellow should have valid index");
    ASSERT_EQ(*idx3, 3u, "Yellow index should be 3");

    Color invalid = static_cast<Color>(100);
    auto idx_invalid = enum_index(invalid);
    ASSERT_TRUE(!idx_invalid.has_value(), "Invalid enum should not have index");

    auto status_idx = enum_index(Status::Running);
    ASSERT_TRUE(status_idx.has_value(), "Running should have valid index");
...
bool test_enum_value()
{
    ...
    ASSERT_TRUE(v0.has_value(), "Index 0 should be valid Color");
    ASSERT_EQ(*v0, Color::Red, "Index 0 should be Red");

    auto v1 = enum_value<Color>(1);
    ASSERT_TRUE(v1.has_value(), "Index 1 should be valid Color");
    ASSERT_EQ(*v1, Color::Green, "Index 1 should be Green");

    auto v2 = enum_value<Color>(2);
    ASSERT_TRUE(v2.has_value(), "Index 2 should be valid Color");
    ASSERT_EQ(*v2, Color::Blue, "Index 2 should be Blue");

    auto v3 = enum_value<Color>(3);
    ASSERT_TRUE(v3.has_value(), "Index 3 should be valid Color");
    ASSERT_EQ(*v3, Color::Yellow, "Index 3 should be Yellow");

    auto v4 = enum_value<Color>(4);
    ASSERT_TRUE(!v4.has_value(), "Index 4 should be invalid Color");

    auto v100 = enum_value<Color>(100);
    ASSERT_TRUE(!v100.has_value(), "Index 100 should be invalid Color");

    auto status_v = enum_value<Status>(2);
    ASSERT_TRUE(status_v.has_value(), "Index 2 should be valid Status");
...
bool test_enum_index_value_roundtrip()
{
    for (std::size_t i = 0; i < EnumSizeTrait<Color>::size; ++i) {
        ...
        ASSERT_TRUE(val.has_value(), "Index should produce valid value");
        auto idx = enum_index(*val);
        ASSERT_TRUE(idx.has_value(), "Value should produce valid index");
    ...
    for (auto v : values) {
        ...
        ASSERT_TRUE(idx.has_value(), "Value should have valid index");
...
{
    ASSERT_TRUE(enum_contains(Color::Red), "Red should be contained");
    ASSERT_TRUE(enum_contains(Color::Green), "Green should be contained");
    ASSERT_TRUE(enum_contains(Color::Blue), "Blue should be contained");
    ASSERT_TRUE(enum_contains(Color::Yellow), "Yellow should be contained");

    Color invalid = static_cast<Color>(100);
    ASSERT_TRUE(!enum_contains(invalid), "Invalid color should not be contained");

    Color negative = static_cast<Color>(-1);
    ASSERT_TRUE(!enum_contains(negative), "Negative should not be contained");

    ASSERT_TRUE(enum_contains(Status::Idle), "Idle should be contained");
    ASSERT_TRUE(enum_contains(Status::Running), "Running should be contained");
    return true;
}
...
bool test_from_string_icase()
{
    ...
    ASSERT_TRUE(s1.has_value(), "'RUNNING' should be valid Status");
