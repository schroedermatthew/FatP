...
void run_coroutine_benchmarks()
{
    ...
    out << "\n"
        << colors::cyan() << colors::bold() << "=== CoroutineTask Performance Benchmarks ===" << colors::reset()
        << "\n\n";
...
bool test_CoroutineTask()
{
    ...
    out << colors::yellow() << "Coroutines require C++20 or later. Current standard: C++"
        << (FATP_CPLUSPLUS / 100 % 100) << colors::reset() << "\n";
    return true;
}
