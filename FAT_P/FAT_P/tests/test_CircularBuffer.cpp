...
bool test_basic_construction()
{
    ...
    ASSERT_TRUE(buffer.empty(), "New buffer should be empty");
    ASSERT_TRUE(!buffer.full(), "New buffer should not be full");
    ASSERT_TRUE(buffer.size() == 0, "Size should be 0");
    ASSERT_TRUE(buffer.capacity() == 16, "Capacity should be 16");
...
bool test_capacity_one()
{
    ...
    ASSERT_TRUE(buffer.capacity() == 1, "Capacity should be 1");
    ASSERT_TRUE(buffer.empty(), "Should be empty");

    ASSERT_TRUE(buffer.push(42), "Push to capacity-1 buffer should succeed");
    ASSERT_TRUE(buffer.full(), "Should be full after one push");
    ASSERT_TRUE(!buffer.push(99), "Push to full capacity-1 buffer should fail");

    int val = 0;
    ASSERT_TRUE(buffer.pop(val), "Pop should succeed");
    ASSERT_TRUE(val == 42, "Popped value should be 42");
    ASSERT_TRUE(buffer.empty(), "Should be empty after pop");
...
bool test_push_pop_basic()
{
    ...
    ASSERT_TRUE(buffer.push(1), "First push should succeed");
    ASSERT_TRUE(buffer.push(2), "Second push should succeed");
    ASSERT_TRUE(buffer.push(3), "Third push should succeed");

    ASSERT_TRUE(buffer.size() == 3, "Size should be 3 after 3 pushes");

    int val = 0;
    ASSERT_TRUE(buffer.pop(val), "First pop should succeed");
    ASSERT_TRUE(val == 1, "First pop should return 1");

    ASSERT_TRUE(buffer.pop(val), "Second pop should succeed");
    ASSERT_TRUE(val == 2, "Second pop should return 2");

    ASSERT_TRUE(buffer.pop(val), "Third pop should succeed");
    ASSERT_TRUE(val == 3, "Third pop should return 3");

    ASSERT_TRUE(buffer.empty(), "Buffer should be empty");
    ASSERT_TRUE(!buffer.pop(val), "Pop from empty buffer should fail");
...
bool test_full_condition()
{
    ...
    ASSERT_TRUE(buffer.push(1), "Push 1 should succeed");
    ASSERT_TRUE(buffer.push(2), "Push 2 should succeed");
    ASSERT_TRUE(buffer.push(3), "Push 3 should succeed");
    ASSERT_TRUE(buffer.push(4), "Push 4 should succeed");

    ASSERT_TRUE(buffer.full(), "Buffer should be full");
    ASSERT_TRUE(!buffer.push(5), "Push to full buffer should fail");

    int val = 0;
    ASSERT_TRUE(buffer.pop(val), "Pop should succeed");
    ASSERT_TRUE(!buffer.full(), "Buffer should not be full after pop");
    ASSERT_TRUE(buffer.push(5), "Push after pop should succeed");
...
bool test_empty_condition()
{
    ...
    ASSERT_TRUE(buffer.push(1), "Push should succeed");
...
bool test_fifo_order()
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        ASSERT_TRUE(buffer.push(i * 10), "Push should succeed");
    }
    ...
    for (int i = 0; i < 8; ++i)
    {
        int val = 0;
        ASSERT_TRUE(buffer.pop(val), "Pop should succeed");
        ASSERT_TRUE(val == i * 10, "Values should be in FIFO order");
    }
...
bool test_wraparound()
{
    ...
    for (int round = 0; round < 5; ++round)
    {
        for (int i = 0; i < 4; ++i)
        {
            ASSERT_TRUE(buffer.push(round * 100 + i), "Push should succeed");
        }
        ...
        for (int i = 0; i < 4; ++i)
        {
            int val = 0;
            ASSERT_TRUE(buffer.pop(val), "Pop should succeed");
            ASSERT_TRUE(val == round * 100 + i, "Value should match");
        }
    }
...
bool test_move_semantics()
{
    ...
    ASSERT_TRUE(buffer.push(std::move(original)), "Push rvalue should succeed");

    std::string result;
    ASSERT_TRUE(buffer.pop(result), "Pop should succeed");
    ASSERT_TRUE(result == "test string", "String content should match");
...
bool test_emplace()
{
    ...
    ASSERT_TRUE(buffer.emplace(5, 'x'), "Emplace should succeed");

    std::string result;
    ASSERT_TRUE(buffer.pop(result), "Pop should succeed");
    ASSERT_TRUE(result == "xxxxx", "Emplaced string should be correct");
...
bool test_front()
{
    ...
    ASSERT_TRUE(buffer.front() == nullptr, "Front of empty buffer should be nullptr");

    ASSERT_TRUE(buffer.push(42), "Push should succeed");
    const int* frontPtr = buffer.front();
    ASSERT_TRUE(frontPtr != nullptr, "Front should not be nullptr");
    ASSERT_TRUE(*frontPtr == 42, "Front should point to first element");

    ASSERT_TRUE(buffer.push(99), "Second push should succeed");
    ASSERT_TRUE(*buffer.front() == 42, "Front should still be first element");

    int val = 0;
    ASSERT_TRUE(buffer.pop(val), "Pop should succeed");
    ASSERT_TRUE(*buffer.front() == 99, "Front should now be second element");
...
bool test_clear()
{
    ...
    ASSERT_TRUE(buffer.size() == 5, "Size should be 5 before clear");

    buffer.clear();

    ASSERT_TRUE(buffer.empty(), "Buffer should be empty after clear");
    ASSERT_TRUE(buffer.size() == 0, "Size should be 0 after clear");

    ASSERT_TRUE(buffer.push(100), "Push after clear should succeed");

    int val = 0;
    ASSERT_TRUE(buffer.pop(val), "Pop after clear should succeed");
    ASSERT_TRUE(val == 100, "Popped value should be correct");
...
bool test_clear_auto_destruct()
{
...

    {
        ...
        ASSERT_TRUE(destruct_count > before, "clear() should destruct non-trivial elements");
        ASSERT_TRUE(buffer.empty(), "Buffer should be empty after clear");
    }
...
bool test_size_tracking()
{
    ...
    ASSERT_TRUE(buffer.size() == 0, "Initial size should be 0");

    for (int i = 1; i <= 5; ++i)
    {
        (void)buffer.push(i);
        ASSERT_TRUE(buffer.size() == static_cast<size_t>(i),
...
bool test_static_capacity()
{
    ...
    ASSERT_TRUE(Buffer16::capacity() == 16, "Static capacity should be 16");
    ASSERT_TRUE(Buffer1::capacity() == 1, "Static capacity should be 1");
    ASSERT_TRUE(Buffer1024::capacity() == 1024, "Static capacity should be 1024");
...
bool test_buffer_size_power_of_two()
{
    ...
    ASSERT_TRUE(Buffer7::buffer_size() == 8, "Buffer size for capacity 7 should be 8");

    // Capacity 100: needs 101 slots minimum, next power of 2 is 128
    ASSERT_TRUE(Buffer100::buffer_size() == 128, "Buffer size for capacity 100 should be 128");
...
bool test_clear_and_destruct()
{
...

    {
        ...
        ASSERT_TRUE(destruct_count > before, "clear_and_destruct should destruct elements");
        ASSERT_TRUE(buffer.empty(), "Buffer should be empty after clear_and_destruct");
    }
...
bool test_thread_safety_spsc()
{
    ...
    ASSERT_TRUE(!order_error.load(), "SPSC values must be in order");
    ASSERT_TRUE(consumer_received.load() == NUM_ITEMS, "All items should be received");
    ASSERT_TRUE(buffer.empty(), "Buffer should be empty after test");
...
bool test_stress_wraparound()
{
    ...
    for (int i = 0; i < ITERATIONS; ++i)
    {
        ...
        for (int j = 0; j < 5; ++j)
        {
            ASSERT_TRUE(buffer.push(i * 10 + j), "Push should succeed");
        }
        ...
        for (int j = 0; j < 3; ++j)
        {
            int val = 0;
            ASSERT_TRUE(buffer.pop(val), "Pop should succeed");
            ASSERT_TRUE(val == i * 10 + j, "Value should match");
        }
