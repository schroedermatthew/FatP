...
TEST_CASE(case_insensitive_constructor_keeps_first)
{
    ...
    ASSERT_EQ(set.size(), size_t(2), "Should have 2 unique values under comparator");
    ASSERT_EQ(*set.begin(), std::string("Hello"),
              "Constructor should keep the first inserted representative for equivalent values");

    ASSERT_TRUE(set.contains("HELLO"), "Equivalent lookup should succeed");
    ASSERT_TRUE(set.contains("hello"), "Equivalent lookup should succeed");
...
TEST_CASE(merge)
{
    ...
    set1.merge(set1);
    ASSERT_EQ(set1.size(), selfSizeBefore, "Self-merge should not change size");
...
bool test_FlatSet()
{
    ...
    RUN_TEST_NS(runner, flatset, case_insensitive_constructor_keeps_first);
