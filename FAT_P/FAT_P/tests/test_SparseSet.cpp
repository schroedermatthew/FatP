...
void run_benchmarks()
{
    ...
    const double insertTimeSparse = measure_perf(
        [&set, i = uint32_t(0)]() mutable {
            set.insert(i);
            ++i;
        },
        n,
    ...
    const double insertTimeStd = measure_perf(
        [&stdSet, i = uint32_t(0)]() mutable {
            stdSet.insert(i);
            ++i;
        },
        n,
    ...
    std::cout << "  SparseSet:          " << format_time(insertTimeSparse) << "\n";
    std::cout << "  std::unordered_set: " << format_time(insertTimeStd) << "\n\n";
    ...
    bool sink = false;
    const double containsTimeSparse = measure_perf(
        [&set, &sink, i = uint32_t(0)]() mutable {
            sink = set.contains(i % static_cast<uint32_t>(n));
            ++i;
        },
        n,
        1000);
    ...
    const double containsTimeStd = measure_perf(
        [&stdSet, &sink, i = uint32_t(0)]() mutable {
            sink = stdSet.count(i % static_cast<uint32_t>(n)) > 0;
            ++i;
        },
        n,
        1000);
    ...
    std::cout << "Contains (existing):\n";
    std::cout << "  SparseSet:          " << format_time(containsTimeSparse) << "\n";
    std::cout << "  std::unordered_set: " << format_time(containsTimeStd) << "\n\n";

    const double eraseTimeSparse = measure_perf(
        [&set, i = uint32_t(0)]() mutable {
            set.erase(i);
            ++i;
        },
    ...
    std::cout << "Erase " << (n / 2) << " elements:\n";
    std::cout << "  SparseSet:          " << format_time(eraseTimeSparse) << "\n";
}
