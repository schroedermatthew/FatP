...
{
    std::cout << colors::cyan() << "\n[TEST] RCU Performance - Lock-Free Reads" << colors::reset() << std::endl;
    ...
    std::cout << colors::blue() << "  Read latency: " << std::fixed << std::setprecision(2) << ns_per_read << " ns/read"
              << colors::reset() << std::endl;

    // RCU reads should be very fast (typically < 100ns)
    FATP_ASSERT_TRUE(ns_per_read < 500.0, "RCU reads should be fast");

    std::cout << colors::green() << "[OK] RCU performance acceptable" << colors::reset() << std::endl;
    return true;
