...
bool test_memory_mapped_file_read_only_mapping() {
    ...
    ASSERT_TRUE(file.is_open(), "File should be open");
    ASSERT_TRUE(file.size() == content.size(), "Size should match");
    
    auto span = file.get_span<const char>();
    std::string result(span.data(), span.size());
    ASSERT_TRUE(result == content, "Content should match");
...
bool test_memory_mapped_file_read_write_mapping() {
    ...
    // Verify changes persisted
    {
        MemoryMappedFile file(filename, MemoryMappedFile::Mode::ReadOnly);
        auto span = file.get_span<const char>();
        ASSERT_TRUE(span[0] == 'M', "Changes should persist");
        ASSERT_TRUE(span[1] == 'o', "Changes should persist");
    }
...
bool test_memory_mapped_file_large_file() {
    ...
    ASSERT_TRUE(file.is_open(), "File should be open");
    ASSERT_TRUE(file.size() == SIZE, "Size should match");
    
    auto span = file.get_span<const char>();
    ASSERT_TRUE(span.size() == SIZE, "Span size should match");
    ASSERT_TRUE(span[0] == 'X', "Content should be accessible");
    ASSERT_TRUE(span[SIZE-1] == 'X', "End should be accessible");
...
bool test_memory_mapped_file_move_semantics() {
    ...
    ASSERT_TRUE(file1.is_open(), "file1 should be open");
    
    MemoryMappedFile file2 = std::move(file1);
    ASSERT_TRUE(!file1.is_open(), "file1 should be closed after move");
    ASSERT_TRUE(file2.is_open(), "file2 should be open after move");
...
bool test_memory_mapped_file_empty_file() {
    ...
    ASSERT_TRUE(file.is_open(), "Empty file should be open");
    ASSERT_TRUE(file.size() == 0, "Size should be zero");
    
    auto span = file.get_span<const char>();
    ASSERT_TRUE(span.size() == 0, "Span should be empty");
    ASSERT_TRUE(span.empty(), "Span should report empty");
...
bool test_memory_mapped_file_span_operations() {
    ...
    ASSERT_TRUE(span.size() == 10, "Span size should be 10");
    ASSERT_TRUE(!span.empty(), "Span should not be empty");
    ASSERT_TRUE(span.front() == '0', "Front should be '0'");
    ASSERT_TRUE(span.back() == '9', "Back should be '9'");
    ASSERT_TRUE(span[5] == '5', "Index access should work");
    
    // Test subviews
    auto first = span.first(3);
    ASSERT_TRUE(first.size() == 3, "First 3 elements");
    ASSERT_TRUE(first[0] == '0' && first[1] == '1' && first[2] == '2', "First subspan content");
    
    auto last = span.last(3);
    ASSERT_TRUE(last.size() == 3, "Last 3 elements");
    ASSERT_TRUE(last[0] == '7' && last[1] == '8' && last[2] == '9', "Last subspan content");
    
    auto sub = span.subspan(3, 4);
    ASSERT_TRUE(sub.size() == 4, "Subspan of 4 elements");
    ASSERT_TRUE(sub[0] == '3' && sub[3] == '6', "Subspan content");
    
    // Test iterators
    int count = 0;
    for (auto c : span) {
        ASSERT_TRUE(c == '0' + count, "Iterator should match content");
        ++count;
    }
    ASSERT_TRUE(count == 10, "Iterator should traverse all elements");
