...
{
    std::cout << "\n" << colors::cyan() << "StableHashMap Sanity Benchmark:" 
              << colors::reset() << "\n\n";
    ...
    if (speedup < 0.8)
    {
        std::cout << colors::yellow() << "  Warning: StableHashMap slower than expected"
                  << colors::reset() << "\n";
    }
    else
    {
        std::cout << colors::green() << "  ✓ Performance sanity check passed"
                  << colors::reset() << "\n";
    }
}
