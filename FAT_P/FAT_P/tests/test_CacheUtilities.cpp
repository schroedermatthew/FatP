...
bool test_cache_info() {
    ...
    // Sanity checks
    ASSERT_TRUE(CacheInfo::l1_line_size() >= 32, "Cache line should be at least 32 bytes");
    ASSERT_TRUE(CacheInfo::l1_line_size() <= 256, "Cache line should be at most 256 bytes");
    
    // Power of 2 check
    constexpr size_t line_size = CacheInfo::l1_line_size();
    ASSERT_TRUE((line_size & (line_size - 1)) == 0, "Cache line size should be power of 2");
    
    // Destructive >= constructive (typically equal)
    ASSERT_TRUE(CacheInfo::destructive_interference_size() >= CacheInfo::constructive_interference_size(),
                  "Destructive interference should be >= constructive");
    
    // Verify constexpr values match function returns
    ASSERT_TRUE(cache_constants::l1_line_size_v == CacheInfo::l1_line_size(),
                  "Constexpr value should match function");
    ASSERT_TRUE(cache_constants::destructive_interference_size_v == CacheInfo::destructive_interference_size(),
                  "Constexpr value should match function");
...
bool test_prefetch_operations() {
    ...
    ASSERT_TRUE(sum == expected, "Data should be accessible after prefetch");
    
    return true;
}

bool test_prefetch_range_zero_size() {
...
bool test_prefetch_ahead() {
    ...
    ASSERT_TRUE(sum > 0.0f, "Prefetch ahead should work");
    
    return true;
}

bool test_prefetch_ahead_overflow() {
    alignas(64) int data[64];
...
bool test_cache_flush() {
    ...
    ASSERT_TRUE(data[0] == 0, "Data should still be correct after flush");
    ASSERT_TRUE(data[63] == 63, "Last element should be correct after flush");
    
    return true;
}

bool test_flush_cache_range_zero_size() {
    alignas(64) int data[64];
...
bool test_stream_store() {
    ...
    for (int i = 0; i < 64; ++i) {
        ASSERT_TRUE(data[i] == i, "Stream store should write correct values");
    }
    
    return true;
}

bool test_stream_store_8byte() {
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        ASSERT_TRUE(data[i] == expected, "Stream store should work for 8-byte types");
    }
    
    return true;
}

bool test_stream_store_unaligned() {
    ...
    ASSERT_TRUE(loaded_int == 42, "Unaligned stream_store should work via fallback");
    ...
    ASSERT_TRUE(loaded_double == 3.14, "Unaligned double stream_store should work");
...
bool test_stream_copy() {
    ...
    for (int i = 0; i < 1024; ++i) {
        ASSERT_TRUE(dest[i] == src[i], "Stream copy should copy correctly");
    }
    
    return true;
}

bool test_stream_copy_unaligned() {
    alignas(64) char src[1024];
    alignas(64) char dest[1024];
    ...
    for (int i = 1; i < 1023; ++i) {
        ASSERT_TRUE(dest[i] == src[i], "Unaligned stream copy should work via fallback");
    }
    
    return true;
}

bool test_stream_copy_zero_size() {
...
bool test_cache_aligned() {
    ...
    ASSERT_TRUE(val1.get() == 42, "CacheAligned should store value");
    
    CacheAligned<double> val2;
    val2.get() = 3.14;
    ASSERT_TRUE(val2.get() == 3.14, "CacheAligned should be modifiable");
    
    // Check alignment
    ASSERT_TRUE(reinterpret_cast<uintptr_t>(&val1) % CacheInfo::destructive_interference_size() == 0,
                  "CacheAligned should be properly aligned");
    
    return true;
}

bool test_cache_aligned_array() {
    ...
    // Verify values
    for (int i = 0; i < 4; ++i) {
        ASSERT_TRUE(arr[i].get() == i * 10, "Array element should have correct value");
    }
    ...
    ASSERT_TRUE(spacing >= CacheInfo::destructive_interference_size(),
...
bool test_cache_line_padded() {
    ...
    ASSERT_TRUE(val1.get() == 100, "CacheLinePadded should store value");
    
    // Size should be at least cache line size
    ASSERT_TRUE(sizeof(CacheLinePadded<int>) >= CacheInfo::destructive_interference_size(),
...
bool test_cache_line_padded_alignment() {
    ...
    ASSERT_TRUE(addr0 % CacheInfo::destructive_interference_size() == 0,
    ...
    ASSERT_TRUE(arr[0].get() == 111, "First element should be correct");
    ASSERT_TRUE(arr[1].get() == 222, "Second element should be correct");
    
    return true;
}

bool test_cache_line_padded_various_sizes() {
    // Test with various small types
    CacheLinePadded<char> c(42);
    ASSERT_TRUE(sizeof(CacheLinePadded<char>) == CacheInfo::destructive_interference_size(),
                  "CacheLinePadded<char> should be cache line size");
    ASSERT_TRUE(c.get() == 42, "char value should be correct");
    
    CacheLinePadded<short> s(1000);
    ASSERT_TRUE(sizeof(CacheLinePadded<short>) == CacheInfo::destructive_interference_size(),
                  "CacheLinePadded<short> should be cache line size");
    ASSERT_TRUE(s.get() == 1000, "short value should be correct");
    
    CacheLinePadded<double> d(3.14159);
    ASSERT_TRUE(sizeof(CacheLinePadded<double>) == CacheInfo::destructive_interference_size(),
                  "CacheLinePadded<double> should be cache line size");
    ASSERT_TRUE(d.get() == 3.14159, "double value should be correct");
    
    return true;
}

bool test_cache_line_padded_exact_size() {
    ...
    ASSERT_TRUE(sizeof(CacheLinePadded<ExactSize>) == CacheInfo::destructive_interference_size(),
...
bool test_memory_barriers() {
    ...
    ASSERT_TRUE(data[0] == 1 && data[1] == 2, "Memory barrier should not corrupt data");
    ...
    ASSERT_TRUE(val == 3, "Fences should not corrupt data");
...
bool test_optimal_block_size() {
    ...
    ASSERT_TRUE(block_size * sizeof(double) <= 32768, "Block should fit in cache");
    
    // Power of 2 check
    ASSERT_TRUE((block_size & (block_size - 1)) == 0, "Block size should be power of 2");
    
    return true;
}

bool test_optimal_block_size_edge_cases() {
    // Zero elements
    size_t block = optimal_block_size(0, sizeof(int), 32768);
    ASSERT_TRUE(block >= 1, "Block size for 0 elements should be at least 1");
    
    // Zero element size
    block = optimal_block_size(1000, 0, 32768);
    ASSERT_TRUE(block >= 1, "Block size for 0-byte elements should be at least 1");
    
    // Huge elements that don't fit in cache
    block = optimal_block_size(1000, 1024 * 1024, 32768);
    ASSERT_TRUE(block >= 1, "Block size should be at least 1 even for huge elements");
...
bool test_block_iterator_2d() {
    ...
    while (iter.has_next()) {
        ...
        ASSERT_TRUE(ei > si, "Block should have positive height");
        ASSERT_TRUE(ej > sj, "Block should have positive width");
        ASSERT_TRUE(ei <= 100, "Block end_i should not exceed rows");
        ASSERT_TRUE(ej <= 100, "Block end_j should not exceed cols");
    ...
    ASSERT_TRUE(block_count == 49, "Should have exactly 49 blocks for 100x100/16x16");
    
    return true;
}

bool test_block_iterator_2d_non_divisible() {
    ...
    ASSERT_TRUE(block_count == 20, "Should have exactly 20 blocks for 50x70/16x16");
    
    return true;
}

bool test_block_iterator_2d_zero_blocks() {
    ...
    ASSERT_TRUE(count1 < 1000, "Zero block_rows should be clamped, no infinite loop");
    ASSERT_TRUE(count1 == 20, "Should iterate correctly with clamped block_rows=1");
    ...
    ASSERT_TRUE(count2 < 1000, "Zero block_cols should be clamped, no infinite loop");
    ASSERT_TRUE(count2 == 20, "Should iterate correctly with clamped block_cols=1");
    
    // Zero dimensions should terminate immediately
    BlockIterator2D iter_zero_dim(0, 10, 5, 5);
    ASSERT_TRUE(!iter_zero_dim.has_next(), "Zero rows should have no blocks");
    
    BlockIterator2D iter_zero_cols_dim(10, 0, 5, 5);
    ASSERT_TRUE(!iter_zero_cols_dim.has_next(), "Zero cols should have no blocks");
...
bool test_alignment_utilities() {
    ...
    // Test is_aligned
    ASSERT_TRUE(is_aligned(buffer, 64), "Buffer should be 64-byte aligned");
    ASSERT_TRUE(is_aligned(buffer, 32), "64-byte aligned is also 32-byte aligned");
    ASSERT_TRUE(is_aligned(buffer, 16), "64-byte aligned is also 16-byte aligned");
    ASSERT_TRUE(is_aligned(buffer, 1), "Any pointer is 1-byte aligned");
    
    // Test unaligned
    char* unaligned = buffer + 1;
    ASSERT_TRUE(!is_aligned(unaligned, 64), "buffer+1 should not be 64-byte aligned");
    ASSERT_TRUE(!is_aligned(unaligned, 2), "buffer+1 should not be 2-byte aligned");
    
    // Test align_up
    void* aligned_up = align_up(unaligned, 64);
    ASSERT_TRUE(is_aligned(aligned_up, 64), "align_up should produce aligned pointer");
    ASSERT_TRUE(aligned_up >= unaligned, "align_up should not go backward");
    
    // Test align_down
    void* aligned_down = align_down(unaligned, 64);
    ASSERT_TRUE(is_aligned(aligned_down, 64), "align_down should produce aligned pointer");
    ASSERT_TRUE(aligned_down <= unaligned, "align_down should not go forward");
    
    // Test alignment_offset
    size_t offset = alignment_offset(unaligned, 64);
    ASSERT_TRUE(offset == 63, "Offset should be 63 bytes");
    
    offset = alignment_offset(buffer, 64);
    ASSERT_TRUE(offset == 0, "Offset for aligned pointer should be 0");
    
    return true;
}

bool test_alignment_utilities_const() {
    ...
    ASSERT_TRUE(is_aligned(aligned_up, 64), "const align_up should work");
    
    const void* aligned_down = align_down(unaligned, 64);
    ASSERT_TRUE(is_aligned(aligned_down, 64), "const align_down should work");
...
bool test_CacheUtilities() {
    ...
    RUN_TEST(runner, prefetch_range_zero_size);
    RUN_TEST(runner, prefetch_ahead);
    RUN_TEST(runner, prefetch_ahead_overflow);
    
    // Cache flush tests
    RUN_TEST(runner, cache_flush);
    RUN_TEST(runner, flush_cache_range_zero_size);
    
    // Stream store tests
    RUN_TEST(runner, stream_store);
    RUN_TEST(runner, stream_store_8byte);
    RUN_TEST(runner, stream_store_unaligned);
    
    // Stream copy tests
    RUN_TEST(runner, stream_copy);
    RUN_TEST(runner, stream_copy_unaligned);
    RUN_TEST(runner, stream_copy_zero_size);
    
    // CacheAligned tests
    RUN_TEST(runner, cache_aligned);
    RUN_TEST(runner, cache_aligned_array);
    
    // CacheLinePadded tests
    RUN_TEST(runner, cache_line_padded);
    RUN_TEST(runner, cache_line_padded_alignment);
    RUN_TEST(runner, cache_line_padded_various_sizes);
    RUN_TEST(runner, cache_line_padded_exact_size);
    ...
    RUN_TEST(runner, optimal_block_size_edge_cases);
    RUN_TEST(runner, block_iterator_2d);
    RUN_TEST(runner, block_iterator_2d_non_divisible);
    RUN_TEST(runner, block_iterator_2d_zero_blocks);
    
    // Alignment utilities tests
    RUN_TEST(runner, alignment_utilities);
    RUN_TEST(runner, alignment_utilities_const);
