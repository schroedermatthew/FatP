...
bool test_reflection_field_names() {
    ...
    ASSERT_TRUE(x_name == "x", "Field 0 name should be x");
    ASSERT_TRUE(y_name == "y", "Field 1 name should be y");
    
    auto name_field = get_field_name<0, Person>();
    auto age_field = get_field_name<1, Person>();
    auto height_field = get_field_name<2, Person>();
    
    ASSERT_TRUE(name_field == "name", "Field 0 should be name");
    ASSERT_TRUE(age_field == "age", "Field 1 should be age");
    ASSERT_TRUE(height_field == "height", "Field 2 should be height");
...
bool test_reflection_visit_fields() {
    ...
    ASSERT_TRUE(field_names[0] == "name", "First field should be name");
    ASSERT_TRUE(field_names[1] == "age", "Second field should be age");
    ASSERT_TRUE(field_names[2] == "height", "Third field should be height");
    
    ASSERT_TRUE(field_values[0] == "Alice", "name value should be Alice");
    ASSERT_TRUE(field_values[1] == "30", "age value should be 30");
...
bool test_reflection_field_accessor() {
    ...
    ASSERT_TRUE((FieldAccessor<Person>::has_field("name")), "Should have name field");
    ASSERT_TRUE((FieldAccessor<Person>::has_field("age")), "Should have age field");
    ASSERT_TRUE((FieldAccessor<Person>::has_field("height")), "Should have height field");
    ASSERT_TRUE(!(FieldAccessor<Person>::has_field("weight")), "Should not have weight field");
    ...
    ASSERT_TRUE(retrieved_name == "Bob", "Retrieved name should be Bob");
...
bool test_reflection_hash_lookup_performance() {
    ...
    for (const char* field : {"name", "age", "height"}) {
        ASSERT_TRUE(FieldAccessor<Person>::has_field(field), 
                     "Lookup should find field");
    }
    
    // Test non-existent field
    ASSERT_TRUE(!FieldAccessor<Person>::has_field("nonexistent"), 
...
bool test_reflection_type_name() {
    ...
    ASSERT_TRUE(!point_name.empty(), "Point type name should not be empty");
    ASSERT_TRUE(!person_name.empty(), "Person type name should not be empty");
    ASSERT_TRUE(!int_name.empty(), "int type name should not be empty");
    ASSERT_TRUE(!double_name.empty(), "double type name should not be empty");
    
    // The actual names depend on compiler, but should contain the type name
    ASSERT_TRUE(point_name.find("Point") != std::string_view::npos, 
...
bool test_reflection_is_reflectable() {
    ASSERT_TRUE(is_reflectable<Point>(), "Point should be reflectable");
    ASSERT_TRUE(is_reflectable<Person>(), "Person should be reflectable");
    ASSERT_TRUE(is_reflectable<ComplexStruct>(), "ComplexStruct should be reflectable");
    ASSERT_TRUE(!is_reflectable<int>(), "int should not be reflectable");
    ASSERT_TRUE(!is_reflectable<std::string>(), "std::string should not be reflectable");
...
bool test_reflection_complex_types() {
    ...
    ASSERT_TRUE(c_val == "hello", "Field c should be hello");
    ...
    ASSERT_TRUE(name0 == "a", "Field 0 name should be a");
    ASSERT_TRUE(name1 == "b", "Field 1 name should be b");
    ASSERT_TRUE(name7 == "h", "Field 7 name should be h");
...
bool test_reflection_get_field_names() {
    ...
    ASSERT_TRUE(names[0] == "x", "First name should be x");
    ASSERT_TRUE(names[1] == "y", "Second name should be y");
    
    auto person_names = get_field_names<Person>();
    ASSERT_EQ(person_names.size(), 3u, "Should have 3 field names");
    ASSERT_TRUE(person_names[0] == "name", "First name should be name");
    ASSERT_TRUE(person_names[1] == "age", "Second name should be age");
    ASSERT_TRUE(person_names[2] == "height", "Third name should be height");
...
bool test_reflection_to_debug_string() {
    ...
    ASSERT_TRUE(debug.find("Point") != std::string::npos, 
                  "Debug string should contain type name Point");
    ASSERT_TRUE(debug.find("x") != std::string::npos, 
                  "Debug string should contain field name x");
    ASSERT_TRUE(debug.find("y") != std::string::npos, 
                  "Debug string should contain field name y");
    ASSERT_TRUE(debug.find("42") != std::string::npos, 
                  "Debug string should contain value 42");
    ASSERT_TRUE(debug.find("84") != std::string::npos, 
                  "Debug string should contain value 84");
    
    Person person{"Charlie", 35, 175.0};
    std::string person_debug = to_debug_string(person);
    
    ASSERT_TRUE(person_debug.find("Person") != std::string::npos, 
                  "Debug string should contain Person");
    ASSERT_TRUE(person_debug.find("Charlie") != std::string::npos, 
                  "Debug string should contain Charlie");
    ASSERT_TRUE(person_debug.find("35") != std::string::npos, 
...
bool test_reflection_const_access() {
    ...
    ASSERT_TRUE(visited, "Should be able to visit const object fields");
...
bool test_reflection_nested_structs() {
    ...
    ASSERT_TRUE(label == "Origin", "label should be Origin");
