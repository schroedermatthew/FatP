...
FATP_TEST_CASE(hash_lookup_performance) {
    ...
    for (const char* field : {"name", "age", "height"}) {
        FATP_ASSERT_TRUE(FieldAccessor<Person>::has_field(field), 
                     "Lookup should find field");
    }
    
    // Test non-existent field
    FATP_ASSERT_TRUE(!FieldAccessor<Person>::has_field("nonexistent"), 
...
FATP_TEST_CASE(type_name) {
    ...
    FATP_ASSERT_TRUE(point_name.find("Point") != std::string_view::npos, 
...
FATP_TEST_CASE(to_debug_string) {
    ...
    FATP_ASSERT_TRUE(debug.find("Point") != std::string::npos, 
                  "Debug string should contain type name Point");
    FATP_ASSERT_TRUE(debug.find("x") != std::string::npos, 
                  "Debug string should contain field name x");
    FATP_ASSERT_TRUE(debug.find("y") != std::string::npos, 
                  "Debug string should contain field name y");
    FATP_ASSERT_TRUE(debug.find("42") != std::string::npos, 
                  "Debug string should contain value 42");
    FATP_ASSERT_TRUE(debug.find("84") != std::string::npos, 
                  "Debug string should contain value 84");
    
    Person person{"Charlie", 35, 175.0};
    std::string person_debug = to_debug_string(person);
    
    FATP_ASSERT_TRUE(person_debug.find("Person") != std::string::npos, 
                  "Debug string should contain Person");
    FATP_ASSERT_TRUE(person_debug.find("Charlie") != std::string::npos, 
                  "Debug string should contain Charlie");
    FATP_ASSERT_TRUE(person_debug.find("35") != std::string::npos, 
