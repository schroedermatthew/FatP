...
FATP_TEST_CASE(fms)
{
    ...
    for (size_t i = 0; i < SimdVectorF::width; ++i)
    {
        FATP_ASSERT_LT(std::abs(result[i] - expected), 1e-6f, "FMS should compute a*b-c, not c-a*b");
    }
    ...
    for (size_t i = 0; i < SimdVectorF::width; ++i)
    {
        FATP_ASSERT_LT(std::abs(result2[i] - expected2), 1e-6f, "FMS(5,4,3) should be 17, not -17");
    }
...
FATP_TEST_CASE(horizontal_ops)
{
    ...
    FATP_ASSERT_LT(std::abs(max_val - static_cast<float>(SimdVectorF::width)), 1e-6f, "Horizontal max should be width");
...
FATP_TEST_CASE(unaligned_loads)
{
    ...
    for (size_t i = 0; i < SimdVectorF::width; ++i)
    {
        FATP_ASSERT_LT(std::abs(result[i] - static_cast<float>(i + 1)), 1e-6f, "Unaligned load should work correctly");
    }
...
FATP_TEST_CASE(double_precision)
{
    ...
    for (size_t i = 0; i < SimdVectorD::width; ++i)
    {
        FATP_ASSERT_LT(std::abs(result[i] - 6.0), 1e-12, "Double precision multiplication should work");
    }
...
FATP_TEST_CASE(select_blend)
{
    ...
    for (size_t i = 0; i < SimdVectorF::width; ++i)
    {
        FATP_ASSERT_LT(std::abs(result[i] - 2.0f), 1e-6f, "Select with all-true mask should return first operand");
    }
...
FATP_TEST_CASE(partial_mask_select)
{
    ...
    if constexpr (SimdVectorF::width < 2)
    {
        ...
        FATP_ASSERT_LT(std::abs(r1[0] - 42.0f), 1e-6f, "partial_mask_select: scalar select with true mask");
        FATP_ASSERT_LT(std::abs(r2[0] - (-1.0f)), 1e-6f, "partial_mask_select: scalar select with false mask");
    ...
    for (size_t i = 0; i < SimdVectorF::width; ++i)
    {
        const float expected = (i < SimdVectorF::width / 2) ? a_data[i] : b_data[i];
        ...
        FATP_ASSERT_LT(std::abs(result[i] - expected), 1e-6f, "partial_mask_select: lane value mismatch");
    }

    // Also verify popcount reflects number of true lanes
    FATP_ASSERT_EQ(mask.popcount(), expected_true, "partial_mask_select: mask popcount should match true lanes");
...
FATP_TEST_CASE(nan_propagation)
{
    ...
    for (size_t i = 0; i < SimdVectorF::width; ++i)
    {
        if (i == nan_lane)
        {
            FATP_ASSERT_TRUE(std::isnan(sum_result[i]), "nan_propagation: sum lane with NaN should be NaN");
            FATP_ASSERT_TRUE(std::isnan(prod_result[i]), "nan_propagation: prod lane with NaN should be NaN");
        }
        else
        {
            ...
            FATP_ASSERT_LT(std::abs(sum_result[i] - expected_sum), 1e-5f, "nan_propagation: finite sum mismatch");
            FATP_ASSERT_LT(std::abs(prod_result[i] - expected_prod), 1e-5f, "nan_propagation: finite prod mismatch");
        }
    }
...
FATP_TEST_CASE(mask_not_operator)
{
    ...
    FATP_ASSERT_EQ(original_count + flipped_count, SimdVectorF::width, "NOT should flip all bits");
...
void benchmark_simd_vector()
{
    ...
    std::cout << "SIMD addition (" << SimdVectorF::width << " wide): " << format_time(simd_time) << "\n";
