...
TEST_CASE(scalar_broadcast) {
    ...
    ASSERT_EQ(v[0], 42, "Scalar constructor should broadcast to v[0]");
    ASSERT_EQ(v[1], 42, "Scalar constructor should broadcast to v[1]");
    ASSERT_EQ(v[2], 42, "Scalar constructor should broadcast to v[2]");
    return true;
}

TEST_CASE(initializer_list_construction) {
    StaticTensor<double, Vector<3>, UncheckedPolicy> v{1.0, 2.0, 3.0};
    ASSERT_EQ(v[0], 1.0, "Initializer list v[0]");
    ASSERT_EQ(v[1], 2.0, "Initializer list v[1]");
    ASSERT_EQ(v[2], 3.0, "Initializer list v[2]");
    return true;
}

TEST_CASE(variadic_constructor) {
    StaticTensor<float, Vector<4>, UncheckedPolicy> v{1.0f, 2.0f, 3.0f, 4.0f};
    ASSERT_EQ(v[0], 1.0f, "Variadic constructor v[0]");
    ASSERT_EQ(v[3], 4.0f, "Variadic constructor v[3]");
    return true;
}

TEST_CASE(matrix_construction) {
    StaticTensor<int, Matrix<2, 3>, UncheckedPolicy> m{1, 2, 3, 4, 5, 6};
    ASSERT_EQ(m.at(0, 0), 1, "Matrix construction m(0,0)");
    ASSERT_EQ(m.at(1, 2), 6, "Matrix construction m(1,2)");
    return true;
}
...
TEST_CASE(type_aliases) {
    ...
    ASSERT_EQ(v[2], 3.0f, "Type alias Vec3f");
    ASSERT_EQ(m.at(1, 1), 4.0f, "Type alias Mat2x2f");
    return true;
}
...
TEST_CASE(shape_dimensions) {
    using S1 = Shape<3, 4, 5>;
    ASSERT_EQ(S1::dim<0>(), 3u, "Shape dim<0>");
    ASSERT_EQ(S1::dim<1>(), 4u, "Shape dim<1>");
    ASSERT_EQ(S1::dim<2>(), 5u, "Shape dim<2>");
    return true;
}

TEST_CASE(vector_shape) {
    using V = Vector<10>;
    ASSERT_EQ(V::rank, 1u, "Vector rank");
    ASSERT_EQ(V::size, 10u, "Vector size");
    return true;
}

TEST_CASE(matrix_shape) {
    using M = Matrix<3, 4>;
    ASSERT_EQ(M::rank, 2u, "Matrix rank");
    ASSERT_EQ(M::size, 12u, "Matrix size");
    return true;
}
...
TEST_CASE(vector_addition) {
    ...
    auto c = a + b;
    ASSERT_EQ(c[0], 5.0f, "Vector addition c[0]");
    ASSERT_EQ(c[1], 7.0f, "Vector addition c[1]");
    ASSERT_EQ(c[2], 9.0f, "Vector addition c[2]");
    return true;
}
...
TEST_CASE(vector_subtraction) {
    ...
    Vec3f b{4.0f, 5.0f, 6.0f};
    auto c = b - a;
    ASSERT_EQ(c[0], 3.0f, "Vector subtraction c[0]");
    ASSERT_EQ(c[1], 3.0f, "Vector subtraction c[1]");
    ASSERT_EQ(c[2], 3.0f, "Vector subtraction c[2]");
    return true;
}
...
TEST_CASE(hadamard_product) {
    ...
    Vec3f b{4.0f, 5.0f, 6.0f};
    auto c = a * b;
    ASSERT_EQ(c[0], 4.0f, "Hadamard product c[0]");
    ASSERT_EQ(c[1], 10.0f, "Hadamard product c[1]");
    ASSERT_EQ(c[2], 18.0f, "Hadamard product c[2]");
    return true;
}
...
TEST_CASE(element_wise_division) {
    ...
    Vec3f b{4.0f, 5.0f, 6.0f};
    auto c = b / a;
    ASSERT_EQ(c[0], 4.0f, "Element-wise division c[0]");
    ASSERT_EQ(c[1], 2.5f, "Element-wise division c[1]");
    ASSERT_EQ(c[2], 2.0f, "Element-wise division c[2]");
    return true;
}
...
TEST_CASE(scalar_multiplication) {
    Vec3f a{1.0f, 2.0f, 3.0f};
    auto c = a * 2.0f;
    ASSERT_EQ(c[0], 2.0f, "Scalar multiplication c[0]");
    ASSERT_EQ(c[1], 4.0f, "Scalar multiplication c[1]");
    ASSERT_EQ(c[2], 6.0f, "Scalar multiplication c[2]");
    return true;
}
...
TEST_CASE(scalar_multiplication_reversed) {
    Vec3f a{1.0f, 2.0f, 3.0f};
    auto c = 3.0f * a;
    ASSERT_EQ(c[0], 3.0f, "Reversed scalar multiplication c[0]");
    ASSERT_EQ(c[1], 6.0f, "Reversed scalar multiplication c[1]");
    ASSERT_EQ(c[2], 9.0f, "Reversed scalar multiplication c[2]");
    return true;
}
...
TEST_CASE(matrix_transpose) {
    ...
    ASSERT_EQ(result.at(0, 0), 1.0f, "Transpose (0,0)");
    ASSERT_EQ(result.at(0, 1), 4.0f, "Transpose (0,1)");
    ASSERT_EQ(result.at(1, 0), 2.0f, "Transpose (1,0)");
    ASSERT_EQ(result.at(1, 1), 5.0f, "Transpose (1,1)");
    ASSERT_EQ(result.at(2, 0), 3.0f, "Transpose (2,0)");
    ASSERT_EQ(result.at(2, 1), 6.0f, "Transpose (2,1)");
    return true;
}
...
TEST_CASE(outer_product) {
    ...
    ASSERT_EQ(result.at(0, 0), 3.0f, "Outer product (0,0)");
    ASSERT_EQ(result.at(0, 1), 4.0f, "Outer product (0,1)");
    ASSERT_EQ(result.at(1, 0), 6.0f, "Outer product (1,0)");
    ASSERT_EQ(result.at(1, 1), 8.0f, "Outer product (1,1)");
    return true;
}
...
bool test_TensorMath() {
    ...
    RUN_TEST_NS(runner, tensormath, default_construction);
    RUN_TEST_NS(runner, tensormath, scalar_broadcast);
    RUN_TEST_NS(runner, tensormath, initializer_list_construction);
    RUN_TEST_NS(runner, tensormath, variadic_constructor);
    RUN_TEST_NS(runner, tensormath, matrix_construction);
    RUN_TEST_NS(runner, tensormath, type_aliases);
    
    // Shape System
    out << "\n" << colors::blue() << "--- Compile-Time Shape System ---" << colors::reset() << "\n";
    RUN_TEST_NS(runner, tensormath, shape_rank);
    RUN_TEST_NS(runner, tensormath, shape_size);
    RUN_TEST_NS(runner, tensormath, shape_dimensions);
    RUN_TEST_NS(runner, tensormath, vector_shape);
    RUN_TEST_NS(runner, tensormath, matrix_shape);
    
    // Element-Wise Operations
    out << "\n" << colors::blue() << "--- Element-Wise Operations ---" << colors::reset() << "\n";
    RUN_TEST_NS(runner, tensormath, vector_addition);
    RUN_TEST_NS(runner, tensormath, vector_subtraction);
    RUN_TEST_NS(runner, tensormath, hadamard_product);
    RUN_TEST_NS(runner, tensormath, element_wise_division);
    RUN_TEST_NS(runner, tensormath, scalar_multiplication);
    RUN_TEST_NS(runner, tensormath, scalar_multiplication_reversed);
    
    // Policy Behavior
    out << "\n" << colors::blue() << "--- Arithmetic Policy Behavior ---" << colors::reset() << "\n";
    RUN_TEST_NS(runner, tensormath, unchecked_policy_allows_operations);
    RUN_TEST_NS(runner, tensormath, checked_policy_throws_on_overflow);
    RUN_TEST_NS(runner, tensormath, saturating_policy_clamps);
    
    // Linear Algebra
    out << "\n" << colors::blue() << "--- Linear Algebra ---" << colors::reset() << "\n";
    RUN_TEST_NS(runner, tensormath, dot_product);
    RUN_TEST_NS(runner, tensormath, matrix_vector_multiply);
    RUN_TEST_NS(runner, tensormath, matrix_matrix_multiply);
    RUN_TEST_NS(runner, tensormath, matrix_transpose);
    RUN_TEST_NS(runner, tensormath, outer_product);
    
    // Reduction Operations
    out << "\n" << colors::blue() << "--- Reduction Operations ---" << colors::reset() << "\n";
    RUN_TEST_NS(runner, tensormath, sum_reduction);
    RUN_TEST_NS(runner, tensormath, mean_reduction);
    RUN_TEST_NS(runner, tensormath, max_reduction);
    RUN_TEST_NS(runner, tensormath, min_reduction);
    RUN_TEST_NS(runner, tensormath, l2_norm);
    RUN_TEST_NS(runner, tensormath, normalize_produces_unit_vector);
    
#ifdef __AVX2__
    // SIMD Operations
    out << "\n" << colors::blue() << "--- SIMD Operations ---" << colors::reset() << "\n";
    RUN_TEST_NS(runner, tensormath, simd_addition);
    RUN_TEST_NS(runner, tensormath, simd_multiplication);
    RUN_TEST_NS(runner, tensormath, simd_dot_product);
    ...
    RUN_TEST_NS(runner, tensormath, tensor_3d_size);
    RUN_TEST_NS(runner, tensormath, tensor_3d_indexing);
    RUN_TEST_NS(runner, tensormath, tensor_4d_size);
    RUN_TEST_NS(runner, tensormath, tensor_4d_indexing);
