...
FATP_TEST_CASE(jsonc_strict_mode_rejection)
{
    ...
    FATP_ASSERT_THROWS(parse_json<StandardJsonPolicy>(json), std::runtime_error, "Standard policy should reject comments");
    return true;
}
...
FATP_TEST_CASE(uint64_boundaries)
{
    FATP_SUBTEST("UINT64_MAX large value") {
        ...
        uint64_t result = 0;
        FATP_ASSERT_THROWS(from_json(j, result), std::runtime_error, "Should reject UINT64_MAX due to precision loss beyond 2^53");
    }
    FATP_END_SUBTEST
    ...
    FATP_SUBTEST("Large positive int64 to uint64") {
        ...
        uint64_t result = 0;
        from_json(j, result);
        FATP_ASSERT_EQ(result, static_cast<uint64_t>(std::numeric_limits<int64_t>::max()), "Should convert INT64_MAX to uint64");
    }
    FATP_END_SUBTEST
...
FATP_TEST_CASE(unsigned_long_and_long_long_boundaries)
{
    FATP_SUBTEST("ULONG_MAX from int64") {
        ...
        {
            FATP_ASSERT_THROWS(from_json(j, result), std::runtime_error, "Should reject INT64_MAX for 32-bit unsigned long");
        }
    }
    FATP_END_SUBTEST
...
FATP_TEST_CASE(locale_independent_double_keys)
{
    ...
    try
    {
        ...
        FATP_ASSERT_TRUE(json_german.find("3.14") != std::string::npos || 
...
bool test_JsonLite()
{
    ...
    FATP_RUN_TEST_NS(runner, jsonlite, encode_decode_benchmark_vector_int);
    FATP_RUN_TEST_NS(runner, jsonlite, encode_decode_benchmark_map_string_int);
    FATP_RUN_TEST_NS(runner, jsonlite, encode_decode_benchmark_nested);
