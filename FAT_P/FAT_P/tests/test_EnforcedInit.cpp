...
public:
    int value;
    explicit MoveOnlyType(int v) noexcept
        : value(v)
    {
    }
...
public:
    int value;
    explicit NonCopyableType(int v) noexcept
        : value(v)
    {
    }
...
FATP_TEST_CASE(enforce_init_reset_not_allowed)
{
    // Test 1: Default policy disallows reset
    {
        ...
        FATP_ASSERT_TRUE(result.error().find("not allowed") != std::string::npos, "Error message explains policy");
...
FATP_TEST_CASE(enforce_init_reset_allowed)
{
    ...
    {
        using ResettableObj = EnforcedInit<TestObject, SingleThreadedPolicy, DefaultCheckPolicy, AllowResetPolicy>;
...
FATP_TEST_CASE(enforce_init_policy_pack_composition)
{
    ...
    using MultiCheck = EnforcedInit<int, SingleThreadedPolicy, PolicyPack<RangeCheckPolicy, PositiveCheckPolicy>>;
...
FATP_TEST_CASE(enforce_init_mutex_synchronization_policy)
{
    ...
    // Test 3: Concurrent access after init - FIX #2: Proper atomic operations
    {
        ...
        FATP_ASSERT_EQ(sum.load(),
                       static_cast<long long>(TEST_VALUE_DEFAULT) * CONCURRENT_THREAD_COUNT * CONCURRENT_ITERATIONS,
...
FATP_TEST_CASE(enforce_init_shared_mutex_policy)
{
    // Test 1: Shared read-write lock
    {
        ...
        FATP_ASSERT_EQ(read_sum.load(),
                       static_cast<long long>(TEST_VALUE_DEFAULT) * CONCURRENT_THREAD_COUNT,
...
FATP_TEST_CASE(enforce_init_condition_variable_policy)
{
    ...
    // Test 3: Wait returns immediately if already initialized
    {
        ...
        FATP_ASSERT_TRUE(duration < std::chrono::milliseconds(50), "No significant delay when already initialized");
    }
...
{
    using UnionInit = EnforcedInit<int, SingleThreadedPolicy, DefaultCheckPolicy, NoResetPolicy, UnionStoragePolicy>;
    using OptionalInit =
        EnforcedInit<int, SingleThreadedPolicy, DefaultCheckPolicy, NoResetPolicy, OptionalStoragePolicy>;
    ...
    // FIX #11: Verify size optimization
    {
        ...
        FATP_ASSERT_TRUE(sizeof(UnionInit) <= sizeof(OptionalInit), "Union storage is not larger than optional");
    }
    ...
    {
        using UnionObj =
            EnforcedInit<TestObject, SingleThreadedPolicy, DefaultCheckPolicy, NoResetPolicy, UnionStoragePolicy>;
        TestObject::reset_counts();
        {
            UnionObj obj;
            (void)obj.init(99);
...
FATP_TEST_CASE(enforce_init_stress_concurrent_access)
{
    ...
    const int num_threads = std::min(STRESS_THREAD_COUNT, static_cast<int>(std::thread::hardware_concurrency()));
    ...
    FATP_ASSERT_EQ(sum.load(),
                   static_cast<long long>(num_threads * iterations),
...
{
    std::cout << "\n"
              << colors::cyan() << colors::bold() << "=== Performance Benchmarks ===" << colors::reset() << "\n\n";
    ...
    // Benchmark 1: Basic initialization overhead - FIX #7
    {
        ...
        std::cout << "  Overhead: " << colors::yellow() << std::fixed << std::setprecision(1) << overhead << "%"
                  << colors::reset() << "\n";
    }

    // Benchmark 2: Lazy initialization - FIX #12: Isolated overhead
    {
        std::cout << "\n" << colors::yellow() << "Benchmark 2: Lazy Init vs Direct Init" << colors::reset() << "\n";
    ...
    // Benchmark 3: Storage policies
    {
        ...
        if (union_time < optional_time)
        {
            ...
            std::cout << "  " << colors::green() << "Union is " << std::fixed << std::setprecision(2) << speedup
                      << "x faster" << colors::reset() << "\n";
        }
    }
    ...
    // Benchmark 4: Concurrency policies
    {
        ...
        std::cout << "  Mutex overhead: " << colors::yellow() << std::fixed << std::setprecision(1) << overhead << "%"
                  << colors::reset() << "\n";
    }

    // Benchmark 5: Access patterns
    {
        std::cout << "\n" << colors::yellow() << "Benchmark 5: Operator-> vs get()" << colors::reset() << "\n";
...
bool test_EnforcedInit()
{
    ...
    // Test Suite 1: Basic Initialization and Access
    std::cout << "\n" << colors::cyan() << "Test Suite 1: Basic Initialization and Access" << colors::reset() << "\n";
    ...
    std::cout << "\n" << colors::cyan() << "Test Suite 2: Reset Policy" << colors::reset() << "\n";
    ...
    // Test Suite 3: Custom Check Policies
    std::cout << "\n" << colors::cyan() << "Test Suite 3: Custom Check Policies" << colors::reset() << "\n";
    ...
    // Test Suite 4: Lazy Initialization
    std::cout << "\n" << colors::cyan() << "Test Suite 4: Lazy Initialization" << colors::reset() << "\n";
    FATP_RUN_TEST_NS(runner, enforcedinit, enforce_init_lazy_init);

    // Test Suite 5: Copy and Move Semantics
    std::cout << "\n" << colors::cyan() << "Test Suite 5: Copy and Move Semantics" << colors::reset() << "\n";
    ...
    std::cout << "\n"
              << colors::cyan() << "Test Suite 6: Thread-Safety with Concurrency Policies" << colors::reset() << "\n";
    ...
    // Test Suite 7: Storage Policies
    std::cout << "\n" << colors::cyan() << "Test Suite 7: Storage Policies" << colors::reset() << "\n";
    ...
    // Test Suite 8: Lifecycle and RAII
    std::cout << "\n" << colors::cyan() << "Test Suite 8: Lifecycle and RAII" << colors::reset() << "\n";
    ...
    // Test Suite 9: Integration Patterns
    std::cout << "\n" << colors::cyan() << "Test Suite 9: Integration Patterns" << colors::reset() << "\n";
    ...
    std::cout << "\n" << colors::cyan() << "Test Suite 10: Edge Cases and Stress Tests" << colors::reset() << "\n";
