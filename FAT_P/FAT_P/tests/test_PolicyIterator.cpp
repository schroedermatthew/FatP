 ...
 *   Sanitizers: g++ -std=c++17 -DENABLE_TEST_APPLICATION -fsanitize=address,undefined -g -I. -o test
 * test_PolicyIterator.cpp
 */
/*
FATP_META:
  meta_version: 1
...
FATP_TEST_CASE(standard_policy_bidirectional)
{
    ...
    // Now backwards
    --it;
    result.push_back(*it); // 40
    --it;
    result.push_back(*it); // 30
    --it;
    result.push_back(*it); // 20
    --it;
...
FATP_TEST_CASE(filtering_policy_even)
{
    ...
    auto begin = PolicyIterator<int, Policy>::begin(data.data(), data.data() + data.size(), Policy{}, is_even);
    auto end = PolicyIterator<int, Policy>::end(data.data(), data.data() + data.size(), Policy{}, is_even);
...
FATP_TEST_CASE(filtering_policy_positive)
{
    ...
    auto begin = PolicyIterator<int, Policy>::begin(data.data(), data.data() + data.size(), Policy{}, is_positive);
    auto end = PolicyIterator<int, Policy>::end(data.data(), data.data() + data.size(), Policy{}, is_positive);
...
FATP_TEST_CASE(transform_policy_double)
{
    ...
    auto begin = PolicyIterator<int, Policy>::begin(data.data(), data.data() + data.size(), Policy{}, doubler);
    auto end = PolicyIterator<int, Policy>::end(data.data(), data.data() + data.size(), Policy{}, doubler);
...
FATP_TEST_CASE(transform_bidirectional)
{
    ...
    auto it = PolicyIterator<int, Policy>::begin(data.data(), data.data() + data.size(), Policy{}, doubler);
...
FATP_TEST_CASE(transform_type_conversion)
{
    ...
    auto begin = PolicyIterator<int, Policy>::begin(data.data(), data.data() + data.size(), Policy{}, to_double);
    auto end = PolicyIterator<int, Policy>::end(data.data(), data.data() + data.size(), Policy{}, to_double);
...
FATP_TEST_CASE(tensor_stride_policy)
{
    ...
    auto begin = PolicyIterator<int, TensorStridePolicy<int>>::begin(data.data(), data.data() + data.size(), policy);
    auto end = PolicyIterator<int, TensorStridePolicy<int>>::end(data.data(), data.data() + data.size(), policy);
...
FATP_TEST_CASE(tensor_retreat)
{
    ...
    auto it = PolicyIterator<int, TensorStridePolicy<int>>::begin(data.data(), data.data() + data.size(), policy);

    // Advance to middle
    for (int i = 0; i < 6; ++i)
    {
        ++it; // Now at position 6
    }
...
FATP_TEST_CASE(stl_iterator_category_standard)
{
    ...
    static_assert(std::is_same_v<Category, std::bidirectional_iterator_tag>, "StandardPolicy should be bidirectional");
    return true;
}
...
FATP_TEST_CASE(stl_iterator_category_filter)
{
    ...
    static_assert(std::is_same_v<Category, std::forward_iterator_tag>, "FilterPolicy should be forward");
    return true;
}
...
FATP_TEST_CASE(constructibility_filter_uses_factories)
{
    ...
    auto begin = PolicyIterator<int, Policy>::begin(data.data(), data.data() + data.size(), Policy{}, pred);
    auto end = PolicyIterator<int, Policy>::end(data.data(), data.data() + data.size(), Policy{}, pred);
...
FATP_TEST_CASE(constructibility_transform_uses_factories)
{
    ...
    auto begin = PolicyIterator<int, Policy>::begin(data.data(), data.data() + data.size(), Policy{}, xform);
    auto end = PolicyIterator<int, Policy>::end(data.data(), data.data() + data.size(), Policy{}, xform);
...
FATP_TEST_CASE(stride1d_column_sum)
{
    ...
    auto it = PolicyIterator<int64_t, Stride1DPolicy<int64_t>>::begin(data.data(), data.data() + data.size(), policy);
    auto end = PolicyIterator<int64_t, Stride1DPolicy<int64_t>>::end(data.data(), data.data() + data.size(), policy);
...
FATP_TEST_CASE(reverse_iterator_stride1d)
{
    ...
    std::vector<int> result;
    for (auto rit = std::make_reverse_iterator(end); rit != std::make_reverse_iterator(begin); ++rit)
    {
...
FATP_TEST_CASE(for_each_slice_zero_stride_contract)
{
    ...
    try
    {
        forEachSlice(data.data(), {3, 4, 5}, {0, 5, 1}, [](std::size_t, int*) {});
    }
    catch (const std::logic_error&)
    {
        caught = true;
...
FATP_TEST_CASE(contract_tensor_advance_past_end)
{
    ...
    auto it = PolicyIterator<int, TensorStridePolicy<int>>::end(data.data(), data.data() + data.size(), policy);
...
FATP_TEST_CASE(contract_transform_deref_end)
{
    ...
    auto it = PolicyIterator<int, Policy>::end(data.data(), data.data() + data.size(), Policy{}, doubler);
...
FATP_TEST_CASE(throwing_predicate)
{
    ...
    auto throwing_pred = [&call_count](const int& v) -> bool
    {
        ...
        if (v == 3)
        {
            throw std::runtime_error("Predicate threw");
        }
        return v % 2 == 0;
    };
    ...
    {
        auto begin =
            PolicyIterator<int, Policy>::begin(data.data(), data.data() + data.size(), Policy{}, throwing_pred);
        auto end = PolicyIterator<int, Policy>::end(data.data(), data.data() + data.size(), Policy{}, throwing_pred);
...
FATP_TEST_CASE(throwing_transformer)
{
    ...
    {
        auto begin =
            PolicyIterator<int, Policy>::begin(data.data(), data.data() + data.size(), Policy{}, throwing_xform);
        auto end = PolicyIterator<int, Policy>::end(data.data(), data.data() + data.size(), Policy{}, throwing_xform);
...
void run_benchmarks()
{
    ...
    for (int run = 0; run < kRuns; ++run)
    {
        ...
        double policy_ns =
            static_cast<double>(std::chrono::duration_cast<std::chrono::nanoseconds>(policy_time).count());
