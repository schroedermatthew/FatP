...
FATP_META:
  ...
  file_role: test
  path: tests/test_TensorEinsum.cpp
  namespace: fat_p
...
TEST_CASE(matmul) {
    ...
    ASSERT_EQ(C.shape()[0], 2u, "Result should have 2 rows");
    ASSERT_EQ(C.shape()[1], 2u, "Result should have 2 columns");
...
TEST_CASE(batch_matmul) {
    ...
    ASSERT_EQ(C.shape()[0], 2u, "Result should have batch_size=2");
    ASSERT_EQ(C.shape()[1], 2u, "Result should have 2 rows");
    ASSERT_EQ(C.shape()[2], 2u, "Result should have 2 columns");
...
TEST_CASE(outer) {
    ...
    ASSERT_EQ(C.shape()[0], 3u, "Result should have 3 rows");
    ASSERT_EQ(C.shape()[1], 4u, "Result should have 4 columns");
...
TEST_CASE(dot) {
    ...
    ASSERT_EQ(result.size(), 1u, "Result should be scalar (size 1)");
...
TEST_CASE(transpose) {
    ...
    ASSERT_EQ(AT.shape()[0], 3u, "Transposed should have 3 rows");
    ASSERT_EQ(AT.shape()[1], 2u, "Transposed should have 2 columns");
...
TEST_CASE(trace) {
    ...
    ASSERT_EQ(trace_result.size(), 1u, "Trace result should be scalar");
...
TEST_CASE(sum_axis) {
    ...
    ASSERT_EQ(row_sums.size(), 2u, "Should have 2 row sums");
    ...
    ASSERT_EQ(col_sums.size(), 3u, "Should have 3 column sums");
...
TEST_CASE(sum_all) {
    ...
    ASSERT_EQ(total_sum.size(), 1u, "Result should be scalar");
...
TEST_CASE(errors) {
    ...
    ASSERT_TRUE(caught, "Should throw on invalid notation");
    ...
    ASSERT_TRUE(caught, "Should throw on unsupported pattern");
...
bool test_TensorEinsum() {
    ...
    RUN_TEST_NS(runner, tensoreinsum, matmul);
    RUN_TEST_NS(runner, tensoreinsum, batch_matmul);
    RUN_TEST_NS(runner, tensoreinsum, outer);
    RUN_TEST_NS(runner, tensoreinsum, dot);
    RUN_TEST_NS(runner, tensoreinsum, transpose);
    RUN_TEST_NS(runner, tensoreinsum, trace);
    RUN_TEST_NS(runner, tensoreinsum, sum_axis);
    RUN_TEST_NS(runner, tensoreinsum, sum_all);
    RUN_TEST_NS(runner, tensoreinsum, elementwise);
    RUN_TEST_NS(runner, tensoreinsum, errors);
