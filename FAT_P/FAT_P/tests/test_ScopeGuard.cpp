...
FATP_META:
  ...
  file_role: test
  path: tests/test_ScopeGuard.cpp
  namespace: fat_p
...
{
    std::cout << "\n"
              << colors::bold() << colors::cyan() << "=== Performance Benchmarks ===" << colors::reset() << std::endl;
    ...
    std::cout << "\n" << colors::bold() << "Policy Overhead Comparison:" << colors::reset() << std::endl;
    ...
    std::cout << "\n"
              << colors::blue() << "[NOTE] Benchmarks show RAII overhead vs manual cleanup" << colors::reset()
              << std::endl;
    std::cout << colors::blue() << "[NOTE] NothrowPolicy should have lowest overhead (no exception handling)"
              << colors::reset() << std::endl;
}
...
FATP_TEST_CASE(type_traits)
{
    ...
    // Test 1: ScopeGuard detection
    {
        ...
        static_assert(is_scope_guard_v<BasicGuard>, "ScopeGuard should be detected");
        static_assert(is_scope_guard_v<PolicyGuard>, "ScopeGuard with policy should be detected");
        static_assert(!is_scope_guard_v<int>, "int should not be detected as ScopeGuard");
        static_assert(!is_scope_guard_v<std::function<void()>>, "std::function should not be detected as ScopeGuard");
    ...
    // Test 2: ScopeGuardOnFail detection
    {
        ...
        static_assert(is_scope_guard_v<FailGuard>, "ScopeGuardOnFail should be detected as scope guard");
    ...
    // Test 3: ScopeGuardOnSuccess detection
    {
        ...
        static_assert(is_scope_guard_v<SuccessGuard>, "ScopeGuardOnSuccess should be detected as scope guard");
...
bool test_ScopeGuard()
{
    ...
    scopeguard::run_scope_guard_benchmarks();
