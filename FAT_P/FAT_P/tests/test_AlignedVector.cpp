...
TEST_CASE(erase_last)
{
    ...
    ASSERT_EQ(vec.size(), 2u, "Size should be 2");
    ASSERT_TRUE(it == vec.end(), "Iterator should be end()");
...
TEST_CASE(erase_range_empty)
{
    ...
    ASSERT_EQ(vec.size(), 3u, "Size should be unchanged");
    ASSERT_TRUE(it == vec.begin() + 1, "Should return same position");
...
TEST_CASE(erase_to_empty)
{
    ...
    ASSERT_TRUE(vec.empty(), "Vector should be empty");
...
TEST_CASE(reserve_capacity)
{
    ...
    vec.reserve(100);
    ASSERT_TRUE(vec.capacity() >= 100, "Capacity should be at least 100");
...
TEST_CASE(max_size_and_overflow)
{
    ...
    ASSERT_TRUE(max_sz > 0, "max_size() should be positive");
    ...
    ASSERT_TRUE(threw_expected, "reserve() beyond max_size() should throw length_error or bad_alloc");
...
TEST_CASE(clear_and_pop_back)
{
    ...
    vec.clear();
    ASSERT_TRUE(vec.empty(), "After clear, vector should be empty");
...
TEST_CASE(equality_operators)
{
    ...
    ASSERT_TRUE(vec1 == vec2, "Equal vectors should be ==");
    ASSERT_TRUE(vec1 != vec3, "Different vectors should be !=");
...
TEST_CASE(ordering_operators)
{
    ...
    ASSERT_TRUE(vec1 < vec2, "{1,2,3} < {1,2,4}");
    ASSERT_TRUE(vec2 > vec1, "{1,2,4} > {1,2,3}");
    ASSERT_TRUE(vec1 <= vec2, "{1,2,3} <= {1,2,4}");
    ASSERT_TRUE(vec2 >= vec1, "{1,2,4} >= {1,2,3}");
...
TEST_CASE(allocator_comparison)
{
    ...
    ASSERT_TRUE(alloc1 == alloc2, "Same type/alignment allocators should be equal");
    ...
    ASSERT_TRUE(alloc1 == alloc3, "Same alignment allocators should be equal (rebindable)");
    
    // Different alignments should NOT be equal
    ASSERT_TRUE(!(alloc1 == alloc4), "Different alignment allocators should not be equal");
...
TEST_CASE(erase_calls_destructors)
{
    ...
    ASSERT_TRUE(DestructorCounter::count > count_before,
...
TEST_CASE(assign_exception_safety_strong_guarantee)
{
    ...
    ASSERT_TRUE(vec.size() == 0 || vec.size() == original_size,
...
TEST_CASE(self_insertion_safety)
{
    ...
    ASSERT_TRUE(all_valid, "All values should be in range [1,5]");
...
TEST_CASE(push_back_aliasing)
{
    ...
    ASSERT_TRUE(all_first, "All elements should be 'first'");
