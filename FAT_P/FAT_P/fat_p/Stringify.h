...
inline constexpr bool kIsOptionalLike =
    is_detected_v<op_has_value, std::decay_t<T>> && is_detected_v<op_deref, std::decay_t<T>>;
#endif
...
template <typename T>
struct is_ostreamable_impl<T, std::void_t<decltype(std::declval<std::ostringstream&>() << std::declval<const T&>())>>
    : std::true_type
{
};
...
template <typename T>
struct is_wostreamable_impl<T, std::void_t<decltype(std::declval<std::wostringstream&>() << std::declval<const T&>())>>
    : std::true_type
{
};
...
template <typename T>
struct has_to_string_method_impl<T, std::void_t<decltype(std::declval<const T&>().toString())>>
    : std::bool_constant<std::is_convertible_v<decltype(std::declval<const T&>().toString()), std::string>>
{
};
...
template <typename T>
struct has_to_string_snake_method_impl<T, std::void_t<decltype(std::declval<const T&>().to_string())>>
    : std::bool_constant<std::is_convertible_v<decltype(std::declval<const T&>().to_string()), std::string>>
{
};
 ...
 */
template <typename T>
[[nodiscard]] inline std::enable_if_t<std::is_floating_point_v<T>, std::string> fast_float_to_string(T value)
{
    return std::to_string(value);
}
...
inline std::string stringify_tuple_impl(const Tuple& t, const StringifyOptions& opts, std::index_sequence<Is...>)
{
    std::string result;
    result.reserve(64);
    result += "(";
...
template <typename T>
struct has_size_method<T, std::void_t<decltype(std::declval<const T&>().size())>> : std::true_type
{
};
...
[[nodiscard]] inline std::size_t estimate_container_size(const Container& container, const StringifyOptions& opts)
{
    std::size_t count = 0;
 ...
 */
template <typename Container>
[[nodiscard]] inline std::string stringify_container(const Container& container, const StringifyOptions& opts)
{
 ...
 */
template <typename Container>
[[nodiscard]] inline std::string stringify_map(const Container& container, const StringifyOptions& opts)
{
    StringifyDepthGuard guard;
    if (guard.exceeded_limit())
    {
...
[[nodiscard]] inline std::string toString(T&& value, const StringifyOptions& opts)
{
    ...
    else if constexpr (is_detected_v<detail::op_tuple_size, PlainT>)
    {
        ...
        return detail::stringify_tuple_impl(value, opts, std::make_index_sequence<std::tuple_size_v<PlainT>>{});
    }
#if FATP_STRINGIFY_HAS_OPTIONAL
    ...
    else if constexpr (detail::has_to_string_method<PlainT>::value || detail::has_to_string_snake_method<PlainT>::value)
    {
    ...
    else if constexpr (is_map_like_v<PlainT> && !std::is_convertible_v<PlainT, std::string> &&
                       !std::is_convertible_v<PlainT, const char*>)
    {
        return detail::stringify_map(value, opts);
    }
#if FATP_HAS_CPP20
    else if constexpr (detail::is_iterable_concept<PlainT> && !std::is_convertible_v<PlainT, std::string_view> &&
                       !std::is_convertible_v<PlainT, const char*>)
    {
        return detail::stringify_container(value, opts);
    }
#else
    else if constexpr (detail::is_iterable<PlainT>::value && !std::is_convertible_v<PlainT, std::string> &&
                       !std::is_convertible_v<PlainT, const char*>)
    {
        return detail::stringify_container(value, opts);
    }
...
inline constexpr bool is_stringifiable_v =
    std::is_enum_v<std::decay_t<T>> || std::is_arithmetic_v<std::decay_t<T>> || detail::is_ostreamable<T>::value ||
    detail::has_to_string_method<T>::value || detail::has_to_string_snake_method<T>::value ||
    is_detected_v<detail::op_first_type, T> || is_detected_v<detail::op_tuple_size, T> ||
#if FATP_STRINGIFY_HAS_OPTIONAL
    detail::kIsOptionalLike<T> ||
#endif
#if FATP_HAS_CPP20
    (detail::is_iterable_concept<T> && !std::is_convertible_v<T, std::string_view> &&
     !std::is_convertible_v<T, const char*>);
#else
    (detail::is_iterable<T>::value && !std::is_convertible_v<T, std::string> && !std::is_convertible_v<T, const char*>);
#endif
 ...
 */
template <typename T>
[[nodiscard]] inline std::string toStringPadded(T&& value, std::size_t width, char align = '>', char pad_char = ' ')
{
...
toStringPointer(T* ptr, const StringifyOptions& opts = {}, const char* null_placeholder = "nullptr")
{
    if (ptr == nullptr)
    {
