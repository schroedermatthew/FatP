 ...
 */
using AnyDefaultRegistryFactory = LegacyVariadicFactory<
 ...
 */
using AnyExplicitRegistryFactory = LegacyVariadicFactory<
    std::pair<std::type_index, std::type_index>,
    bool,
    true,
    AnyFallbackPolicy,
...
{
    auto key =
        std::make_pair(std::type_index(typeid(T)), std::type_index(typeid(Policy)));

    // Register default-epsilon version (uses policy/type defaults)
    (void)getAnyDefaultRegistry().registerType(
        key,
...
struct EqualDispatcher<std::any, Policy>
{
    ...
    static bool compare(const std::any& a, const std::any& b, EpsParams... eps)
    {
        static_assert(sizeof...(EpsParams) <= 2,
                      "std::any comparison accepts at most 2 epsilon parameters");
...
bool validateAndHandleNested(const std::any& a,
                             const std::any& b,
                             bool& result,
                             EpsParams... eps)
{
    ...
    {
        LOG_ERROR(std::string("Recursion depth exceeded (") +
                  std::to_string(depth) + " > " +
                  std::to_string(kMaxAnyRecursionDepth) +
                  ") in std::any comparison. Possible excessive nesting.");
        result = false;
        return false;
    }
    ...
    {
        LOG_ERROR(std::string("Type mismatch in std::any comparison. a: ") +
                  (a.has_value() ? a.type().name() : "(empty)") + ", b: " +
    ...
    if (valueType == std::type_index(typeid(std::any)))
    {
        try
        {
            result = areEqual<Policy>(std::any_cast<const std::any&>(a),
        ...
        {
            LOG_ERROR(std::string("Bad any_cast for nested std::any: ") + e.what());
            result = false;
            return false;
        }
    }
...
{
    LOG_ERROR(std::string("No comparison handler registered for type '") +
              valueType.name() + "' with policy '" + policyType.name() +
              "'. Use registerAnyType<T, Policy>() to register.");
}
