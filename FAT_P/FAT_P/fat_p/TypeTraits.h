...
template <typename T>
struct has_data<T, detail::void_t<decltype(std::declval<T&>().data())>> : std::true_type
{
};
...
template <typename T>
struct has_reserve<T, detail::void_t<decltype(std::declval<T&>().reserve(std::declval<std::size_t>()))>>
    : std::true_type
{
};
...
template <typename T>
struct has_rbegin<T, detail::void_t<decltype(std::rbegin(std::declval<T&>()))>> : std::true_type
{
};
...
template <typename T>
struct has_rend<T, detail::void_t<decltype(std::rend(std::declval<T&>()))>> : std::true_type
{
};
...
template <typename T>
struct has_subscript<T, detail::void_t<decltype(std::declval<T&>()[std::declval<std::size_t>()])>> : std::true_type
{
};
...
template <typename T>
struct has_at<T, detail::void_t<decltype(std::declval<T&>().at(std::declval<std::size_t>()))>> : std::true_type
{
};
...
template <typename T>
struct has_push_back<T, detail::void_t<decltype(std::declval<T&>().push_back(std::declval<typename T::value_type>()))>>
    : std::true_type
{
};
...
template <typename T>
struct has_emplace_back<T, detail::void_t<decltype(std::declval<T&>().emplace_back())>> : std::true_type
{
};
...
template <typename T>
struct has_push_front<T,
                      detail::void_t<decltype(std::declval<T&>().push_front(std::declval<typename T::value_type>()))>>
    : std::true_type
{
};
...
{
    static constexpr bool value = is_detected_v<op_value_type, T> && is_pair<detected_t<op_value_type, T>>::value;
};
 ...
 */
template <typename T>
struct is_map_like : std::conjunction<is_iterable<T>, detail::has_pair_value_type<std::decay_t<T>>>
{
};
...
template <typename T>
struct is_equality_comparable<T, detail::void_t<decltype(std::declval<T>() == std::declval<T>())>> : std::true_type
{
};
...
template <typename T>
struct is_inequality_comparable<T, detail::void_t<decltype(std::declval<T>() != std::declval<T>())>> : std::true_type
{
};
...
template <typename T>
struct is_streamable<T, detail::void_t<decltype(std::declval<std::ostream&>() << std::declval<const T&>())>>
    : std::true_type
{
};
...
template <typename T>
struct has_less<T, detail::void_t<decltype(std::declval<T>() < std::declval<T>())>> : std::true_type
{
};
...
template <typename T>
struct has_less_equal<T, detail::void_t<decltype(std::declval<T>() <= std::declval<T>())>> : std::true_type
{
};
...
template <typename T>
struct has_greater<T, detail::void_t<decltype(std::declval<T>() > std::declval<T>())>> : std::true_type
{
};
...
template <typename T>
struct has_greater_equal<T, detail::void_t<decltype(std::declval<T>() >= std::declval<T>())>> : std::true_type
{
};
 ...
 */
template <typename T>
struct is_fully_ordered : std::conjunction<has_less<T>, has_less_equal<T>, has_greater<T>, has_greater_equal<T>>
{
};
 ...
 */
template <typename T>
struct is_less_comparable : std::conjunction<has_less<T>, has_less_equal<T>, has_greater<T>, has_greater_equal<T>>
{
};
...
template <typename T>
struct has_less_than<T, detail::void_t<decltype(std::declval<T>() < std::declval<T>())>> : std::true_type
{
};
...
struct is_valid_comparator<Comp,
                           T,
                           detail::void_t<decltype(std::declval<Comp>()(std::declval<T>(), std::declval<T>()))>>
    : std::true_type
{
};
...
template <typename T>
struct is_transparent<T, detail::void_t<typename T::is_transparent>> : std::true_type
{
};
 ...
 */
template <typename T>
using has_serialize_t = decltype(std::declval<T&>().serialize(std::declval<std::ostream&>()));
 ...
 */
template <typename T>
struct is_tuple_like : std::conjunction<has_tuple_size<T>, has_tuple_element<T>, has_get<T>>
{
};
 ...
 */
template <typename T>
struct is_optional_like : std::conjunction<has_has_value<T>, has_value_method<T>>
{
};
...
{
    static constexpr const char* reason = !has_begin_v<T>  ? "Type lacks begin() method or std::begin support"
...
struct why_not_hashable
{
    static constexpr const char* reason =
        !is_hashable_v<T> ? "std::hash<T> specialization not found or not valid" : "Type is hashable";
};
...
{
    static constexpr const char* reason = !has_serialize_v<T> ? "Type lacks serialize(std::ostream&) method"
                                          : !has_deserialize_v<T>
                                              ? "Type lacks static deserialize(std::istream&) method"
...
{
    static constexpr const char* reason = !has_less_v<T> ? "Type lacks operator<" : "Type is comparable";
};
...
{
    static_assert(is_iterable_v<T>, "[CONTRACT VIOLATION] Type must be iterable (have begin/end)");
}
...
{
    static_assert(is_sized_v<T>, "[CONTRACT VIOLATION] Type must have size() method");
}
...
{
    static_assert(is_container_v<T>, "[CONTRACT VIOLATION] Type must be a container (iterable + sized)");
}
...
{
    static_assert(is_hashable_v<T>, "[CONTRACT VIOLATION] Type must be hashable (std::hash<T> must exist)");
}
...
{
    static_assert(has_less_than_v<T>, "[CONTRACT VIOLATION] Type must be comparable (operator< must exist)");
}
...
{
    static_assert(is_invocable_v<F, Args...>, "[CONTRACT VIOLATION] Function must be invocable with given arguments");
}
...
{
    static_assert(is_allocator_v<T>, "[CONTRACT VIOLATION] Type must be an allocator");
}
...
{
    static_assert(is_serializable_v<T>, "[CONTRACT VIOLATION] Type must be serializable (have serialize/deserialize)");
}
