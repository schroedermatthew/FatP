...
constexpr OverflowType detect_mul_overflow(T a, T b, T result) noexcept {
    ...
    if (a == 0 || b == 0) return OverflowType::None;
...
inline bool neon_any_differ_i32(int32x4_t wrap, int32x4_t sat) noexcept {
    ...
    return (m0 != std::numeric_limits<uint64_t>::max()) || 
           (m1 != std::numeric_limits<uint64_t>::max());
#else
    // AArch32: check 32-bit lanes individually
    return (vgetq_lane_u32(eq, 0) != ~0U) || (vgetq_lane_u32(eq, 1) != ~0U) ||
           (vgetq_lane_u32(eq, 2) != ~0U) || (vgetq_lane_u32(eq, 3) != ~0U);
#endif
}
