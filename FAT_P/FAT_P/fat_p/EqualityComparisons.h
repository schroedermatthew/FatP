			...
			EpsParams... eps)
		{
			...
			(
				[&]()
				{
					...
					{
						success = false;
						FATP_LOG_ERROR(std::string("Tuple element mismatch at index ") +
		...
		static bool compare(const T& a, const T& b, EpsParams... eps)
		{
			...
			else if constexpr (IsIterable<T>::value)
			{
				...
				if constexpr (HasSize<T>::value)
				{
					if (a.size() != b.size())
					{
						FATP_LOG_ERROR(std::string("Container size mismatch: ") + std::to_string(a.size()) +
				...
				if constexpr (IsUnorderedAssociative<T>::value)
				{
					...
					for (const auto& elemA : a)
					{
						...
						if constexpr (HasMappedType<T>::value)
						{
							...
							if (itB == b.end())
							{
								FATP_LOG_ERROR(std::string("Key not found in second container: ") +
									safeToString(keyA));
								if (kStopOnFirstError)
								{
									return false;
						...
						else
						{
							...
							if (itB == b.end())
							{
								FATP_LOG_ERROR(std::string("Element not found in second set: ") +
									safeToString(keyA));
								if (kStopOnFirstError)
								{
									return false;
							...
							{
								success = false;
								FATP_LOG_ERROR(std::string("Set element value mismatch: ") +
									safeToString(valA) + " != " + safeToString(valB));
								if (kStopOnFirstError)
								{
									return false;
				...
				else
				{
					...
					while (it1 != std::end(a))
					{
						...
						if constexpr (!HasSize<T>::value)
						{
							if (it2 == std::end(b))
							{
								FATP_LOG_ERROR(std::string("Container b ended early at index ") +
									std::to_string(index));
								return false;
							}
						}
						...
						{
							success = false;
							FATP_LOG_ERROR(std::string("Element mismatch at index ") +
