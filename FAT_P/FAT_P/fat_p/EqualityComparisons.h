...
struct HasToString<T,
                   std::enable_if_t<
                       std::is_convertible_v<
                           decltype(toString(std::declval<const T&>())),
                           std::string>>>
    : std::true_type
{
};
...
struct IsUnorderedAssociative<
    ...
    std::void_t<typename T::key_type,
                ...
                decltype(std::declval<const T&>().find(
...
struct IsUnorderedMultiAssociative<
    ...
    std::void_t<typename T::key_type,
                typename T::key_equal,
                decltype(std::declval<const T&>().equal_range(
                    std::declval<const typename T::key_type&>())),
                decltype(std::declval<const T&>().count(
                    std::declval<const typename T::key_type&>())),
...
inline std::string truncateForLog(const std::string& s,
                                  std::size_t headLen = 32,
                                  std::size_t tailLen = 16)
{
...
bool tupleAreEqualImpl(const Tuple& a,
                       const Tuple& b,
                       ...
                       EpsParams... eps)
{
    ...
    (
        [&]()
        {
            ...
            bool result = EqualDispatcher<ElemT, Policy>::compare(std::get<I>(a),
                                                                  std::get<I>(b),
                                                                  eps...);
            if (!result)
            {
                success = false;
                LOG_ERROR(std::string("Tuple element mismatch at index ") +
                          std::to_string(I) + ": " + safeToString(std::get<I>(a)) +
                          " != " + safeToString(std::get<I>(b)));
            }
        }(),
        ...);
 ...
 */
template <typename Policy, typename Tuple, std::size_t... I, typename... EpsParams>
bool tupleAreEqualStopFirstImpl(const Tuple& a,
                                const Tuple& b,
                                std::index_sequence<I...>,
                                EpsParams... eps)
{
...
bool compareTuple(const Tuple& a, const Tuple& b, EpsParams... eps)
{
    ...
    {
        return tupleAreEqualStopFirstImpl<Policy>(
            a,
            b,
...
struct EqualDispatcher
{
    ...
    static bool compare(const T& a, const T& b, EpsParams... eps)
    {
        ...
        {
            bool firstOk = EqualDispatcher<typename T::first_type, Policy>::compare(a.first,
                                                                                    b.first,
                                                                                    eps...);
            if (!firstOk)
            {
                LOG_ERROR(std::string("Pair.first mismatch: ") + safeToString(a.first) +
            ...
            bool secondOk = EqualDispatcher<typename T::second_type, Policy>::compare(a.second,
                                                                                      b.second,
                                                                                      eps...);
            if (!secondOk)
            {
                LOG_ERROR(std::string("Pair.second mismatch: ") + safeToString(a.second) +
        ...
        else if constexpr (IsIterable<T>::value)
        {
            ...
            if constexpr (HasSize<T>::value)
            {
                if (a.size() != b.size())
                {
                    LOG_ERROR(std::string("Container size mismatch: ") + std::to_string(a.size()) +
            ...
            if constexpr (IsUnorderedAssociative<T>::value)
            {
                ...
                if constexpr (IsUnorderedMultiAssociative<T>::value)
                {
                    ...
                    std::unordered_set<KeyT, KeyHasher, KeyEqual> visitedKeys(
                        0,
                        KeyHasher{keyHash},
                        KeyEqual{keyEq});
                    ...
                    for (const auto& elemA : a)
                    {
                        ...
                        {
                            LOG_ERROR(
                                std::string("Multi-container multiplicity mismatch for key ") +
                        ...
                        if constexpr (HasMappedType<T>::value)
                        {
                            ...
                            for (auto itA = rangeA.first; itA != rangeA.second; ++itA)
                            {
                                ...
                                for (std::size_t i = 0; i < bIters.size(); ++i)
                                {
                                    ...
                                    if (EqualDispatcher<ValT, Policy>::compare(itA->second,
                                                                               bIters[i]->second,
                                                                               eps...))
                                    {
                                        used[i] = true;
                                ...
                                {
                                    LOG_ERROR(
                                        std::string("Multimap: no matching value for key ") +
                                        safeToString(keyA) + " and value " +
                                        safeToString(itA->second));
                                    if (kStopOnFirstError)
                                    {
                                        return false;
                                    }
                        ...
                        else
                        {
                            ...
                            for (auto itA = rangeA.first; itA != rangeA.second; ++itA)
                            {
                                ...
                                for (std::size_t i = 0; i < bIters.size(); ++i)
                                {
                                    ...
                                    if (EqualDispatcher<ValT, Policy>::compare(*itA,
                                                                               *bIters[i],
                                                                               eps...))
                                    {
                                        used[i] = true;
                                ...
                                {
                                    LOG_ERROR(
                                        std::string("Multiset: no matching element for key ") +
                                        safeToString(keyA) + " and element " +
                                        safeToString(*itA));
                                    if (kStopOnFirstError)
                                    {
                                        return false;
                    ...
                    if constexpr (!HasSize<T>::value)
                    {
                        for (const auto& elemB : b)
                        {
                            ...
                            if (a.count(keyB) == 0)
                            {
                                LOG_ERROR(std::string("Extra key present only in second container: ") +
                                          safeToString(keyB));
                                if (kStopOnFirstError)
                                {
                                    return false;
                ...
                for (const auto& elemA : a)
                {
                    ...
                    if constexpr (HasMappedType<T>::value)
                    {
                        ...
                        {
                            success = false;
                            LOG_ERROR(std::string("Value mismatch for key '") + safeToString(keyA) +
                    ...
                    else
                    {
                        ...
                        if (itB == b.end())
                        {
                            LOG_ERROR(std::string("Element not found in second set: ") +
                                      safeToString(keyA));
                            if (kStopOnFirstError)
                            {
                                return false;
                        ...
                        {
                            success = false;
                            LOG_ERROR(std::string("Set element value mismatch: ") +
                                      safeToString(valA) + " != " + safeToString(valB));
                            if (kStopOnFirstError)
                            {
                                return false;
            ...
            else
            {
                ...
                while (it1 != std::end(a))
                {
                    ...
                    if constexpr (!HasSize<T>::value)
                    {
                        if (it2 == std::end(b))
                        {
                            LOG_ERROR(std::string("Container b ended early at index ") +
                                      std::to_string(index));
                            return false;
                        }
                    }
                    ...
                    {
                        success = false;
                        LOG_ERROR(std::string("Element mismatch at index ") +
                                  std::to_string(index) + ": " + safeToString(*it1) + " != " +
                                  safeToString(*it2));
                        if (kStopOnFirstError)
                        {
                            return false;
                ...
                if constexpr (!HasSize<T>::value)
                {
                    if (it2 != std::end(b))
                    {
                        LOG_ERROR(std::string("Container b has extra elements starting at index ") +
                                  std::to_string(index));
                        return false;
                    }
                }
        ...
        {
            LOG_ERROR(std::string("Unsupported type for equality comparison: ") + typeid(T).name());
            return false;
        }
    }
};
