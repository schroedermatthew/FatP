...
private:
    ...
    [[nodiscard]] Expected<void, std::string> add_relationship_unlocked(
        ...
        for (const auto& [name, node] : mFeatures) {
            ...
            if (req_it != node.relationships.end()) {
                for (const auto& required : req_it->second) {
                    ...
                    if (!it->second.enabled) {
                        return unexpected("'" + name + "' requires '" + required + 
                                         "' but it's disabled");
                    }
                }
            }
            ...
            if (impl_it != node.relationships.end()) {
                for (const auto& implied : impl_it->second) {
                    ...
                    if (!it->second.enabled) {
                        return unexpected("'" + name + "' implies '" + implied + 
                                         "' but it's disabled");
                    }
                }
            }
...
Expected<void, std::string> enable_feature(const std::string& name,
        ...
        struct ChainGuard {
            ...
            explicit ChainGuard(std::vector<std::string>& c) : chain(c) {}
            ~ChainGuard() { if (!dismissed) chain.pop_back(); }
