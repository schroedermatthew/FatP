...
template <typename T, size_t N, size_t Alignment>
using PartialBuffer =
    std::conditional_t<(Alignment > 16), HeapAlignedBuffer<T, N, Alignment>, AlignedBuffer<T, N, Alignment>>;
#else
...
public:
    ...
    static SimdVector zero() noexcept
    {
        return SimdVector(T{0});
    }
    static SimdVector ones() noexcept
    {
        return SimdVector(T{1});
    }
    ...
    SimdVector operator-() const noexcept
    {
        return zero() - *this;
    }
    ...
    // Compound operators
    SimdVector& operator+=(const SimdVector& r) noexcept
    {
        *this = *this + r;
        return *this;
    }
    ...
    SimdVector abs() const noexcept
    {
        return select(*this >= zero(), *this, -*this);
    }
    SimdVector max(const SimdVector& other) const noexcept
    {
        return select(*this > other, *this, other);
    }
    SimdVector min(const SimdVector& other) const noexcept
    {
        return select(*this < other, *this, other);
    }
    SimdVector clamp(const SimdVector& lo, const SimdVector& hi) const noexcept
    {
