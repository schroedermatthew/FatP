...
namespace fat_p {
    ...
    struct UnionStoragePolicy {
        ...
        struct type {
            ...
            constexpr type() noexcept : init(false) {}
            ...
            explicit operator bool() const noexcept { return init; }
    ...
    struct AtomicPolicy {
        ...
        std::atomic<bool>& getLock() const noexcept { return init_flag_; }
    };
#endif
    ...
    private:
        void notify_init() noexcept;
        typename StoragePolicy::template type<T> m_value;
    };
    ...
    Expected<void, std::string> EnforcedInit<T, ConcurrencyPolicy, CheckPolicy, ResetPolicy, StoragePolicy>::init(Args&&... args) {
        ...
        CheckPolicy::template post_init_check<T>(m_value.value());
        notify_init();
        return {};
    }
    ...
    bool EnforcedInit<T, ConcurrencyPolicy, CheckPolicy, ResetPolicy, StoragePolicy>::wait_for_init(const Duration& timeout) const {
        if constexpr (std::is_base_of_v<ConditionVarPolicy, CheckPolicy>) {
            ...
            return static_cast<const ConditionVarPolicy*>(
                static_cast<const ConcurrencyPolicy*>(this))->wait_for_init(timeout);
        } else {
    ...
    void EnforcedInit<T, ConcurrencyPolicy, CheckPolicy, ResetPolicy, StoragePolicy>::notify_init() noexcept {
        if constexpr (std::is_base_of_v<ConditionVarPolicy, CheckPolicy>) {
            static_cast<ConditionVarPolicy*>(
                static_cast<CheckPolicy*>(this))->notify_init();
        } else if constexpr (std::is_base_of_v<ConditionVarPolicy, ConcurrencyPolicy>) {
            static_cast<ConditionVarPolicy*>(
                static_cast<ConcurrencyPolicy*>(this))->notify_init();
        }
    }
