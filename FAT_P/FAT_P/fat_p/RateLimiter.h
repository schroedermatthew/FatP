...
public:
     ...
     */
    template<typename Rep, typename Period>
    SlidingWindowRateLimiter(size_t max_requests, 
    ...
    Clock::duration window_duration() const { return m_window_duration; }
    
private:
    void cleanup_old_timestamps(TimePoint now) {
        auto threshold = now - m_window_duration;
        m_timestamps.erase(
            std::remove_if(m_timestamps.begin(), m_timestamps.end(),
                [threshold](TimePoint tp) { return tp < threshold; }),
            m_timestamps.end()
        );
    }
