...
template <typename T,
          ...
          template <typename, typename> class BackendPolicy>
...
template <typename T>
using binary_serialize_t = decltype(std::declval<T&>().binary_serialize(std::declval<std::vector<uint8_t>&>()));

template <typename T>
using binary_deserialize_t = decltype(T::binary_deserialize(std::declval<const std::vector<uint8_t>&>()));

template <typename T>
using benchmark_interface_t = decltype(std::declval<T&>().benchmark_setup(),
...
{
    static constexpr bool value = is_small_vector_v<T> || is_circular_buffer_v<T> || is_flat_map_v<T> ||
                                  is_flat_set_v<T> || is_sorted_container_v<T> || is_sparse_set_v<T> ||
...
{
    static constexpr bool value = is_tensor_v<T> || is_fixed_tensor_v<T> || is_csr_matrix_v<T> || is_simd_vector_v<T>;
};
...
struct is_parallel_algorithm_compatible<
    T,
    std::enable_if_t<(
        is_iterable_v<T> && is_detected_v<cpp_util_detail::container_value_type_t, T> &&
...
template <typename T>
inline constexpr bool is_parallel_algorithm_compatible_v = is_parallel_algorithm_compatible<T>::value;
...
struct is_expected_like
    : std::conjunction<is_detected<cpp_util_detail::value_method_t, T>, is_detected<cpp_util_detail::error_method_t, T>>
{
};
...
{
    static_assert(has_validate_v<T>, "[CONTRACT VIOLATION] Type must have validate() method");
}
...
{
    static_assert(is_expected_v<T>, "[CONTRACT VIOLATION] Type must be cpp_utilities::Expected");
}
...
{
    static_assert(is_tensor_v<T>, "[CONTRACT VIOLATION] Type must be cpp_utilities::Tensor");
}
...
{
    static_assert(is_parallel_algorithm_compatible_v<T>, "[CONTRACT VIOLATION] Type must support parallel algorithms");
}
...
{
    static_assert(is_binary_serializable_v<T>, "[CONTRACT VIOLATION] Type must support binary serialization");
}
...
{
    static_assert(is_library_container_v<T>, "[CONTRACT VIOLATION] Type must be a cpp_utilities container");
}
...
{
    static_assert(is_concurrent_container_v<T>, "[CONTRACT VIOLATION] Type must be a concurrent container");
}
...
{
    static_assert(is_tensor_type_v<T>, "[CONTRACT VIOLATION] Type must be a tensor type");
}
...
{
    return is_expected_v<T>                                     ? "Type is specialized as Expected"
           ...
           : !is_detected_v<cpp_util_detail::error_type_t, T>
               ? "Missing error_type typedef"
...
{
    return is_tensor_v<T>                                ? "Type is specialized as Tensor"
...
{
    return is_binary_serializable_v<T>    ? "Type is fully binary serializable"
...
{
    return is_small_vector_v<T>             ? "Type is SmallVector"
...
{
    static constexpr const char* reason = !is_detected_v<cpp_util_detail::shape_t, T>
...
{
    static constexpr const char* reason = !has_binary_serialize_v<T> ? "Missing binary_serialize() method"
                                          : !has_binary_deserialize_v<T>
                                              ? "Missing static binary_deserialize() method"
