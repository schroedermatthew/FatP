...
TEST_CASE(numa_info)
{
    ...
    SIMPLE_ASSERT(NumaInfo::current_node() >= 0, "Current node should be non-negative");
    SIMPLE_ASSERT(NumaInfo::current_node() < NumaInfo::num_nodes(),
                  "Current node should be within range");

    int cpus = NumaInfo::cpus_on_node(0);
    SIMPLE_ASSERT(cpus >= 0, "CPUs on node 0 should be non-negative");
...
TEST_CASE(allocator_zero_size)
{
    ...
    SIMPLE_ASSERT(ptr == nullptr, "allocate(0) should return nullptr");
...
TEST_CASE(allocator_equality)
{
    ...
    SIMPLE_ASSERT(local1 == local2, "NumaLocalPolicy allocators should be equal");

    NumaAllocator<int, NumaInterleavedPolicy> interleaved1;
    NumaAllocator<int, NumaInterleavedPolicy> interleaved2;
    SIMPLE_ASSERT(interleaved1 == interleaved2, "NumaInterleavedPolicy allocators should be equal");

    SIMPLE_ASSERT(!(local1 == interleaved1), "Different policy types should not be equal");
    ...
    SIMPLE_ASSERT(pref0 == pref0_copy, "Same node preferred allocators should be equal");

    if (NumaInfo::num_nodes() > 1)
    {
        NumaAllocator<int, NumaPreferredPolicy> pref1(NumaPreferredPolicy{1});
        SIMPLE_ASSERT(pref0 != pref1, "Different node preferred allocators should not be equal");
    }
...
TEST_CASE(allocator_rebind)
{
    ...
    SIMPLE_ASSERT(ptr != nullptr, "Rebound allocator should work");
...
TEST_CASE(numa_memory_stats)
{
    ...
    for (int node = 0; node < NumaInfo::num_nodes(); ++node)
    {
        ...
        if (stats.valid)
        {
            ...
            if (stats.has_total)
            {
                SIMPLE_ASSERT(stats.total_bytes >= stats.free_bytes,
                              "Total should be >= free");
#if defined(__linux__)
                SIMPLE_ASSERT(stats.total_bytes > 0, "Total bytes should be positive on Linux");
    ...
    SIMPLE_ASSERT(!invalid_stats.valid, "Invalid node should return invalid stats");

    auto out_of_range_stats = get_node_memory_stats(1000);
    SIMPLE_ASSERT(!out_of_range_stats.valid, "Out of range node should return invalid stats");
...
TEST_CASE(thread_local_numa_pool_basic)
{
    ...
    SIMPLE_ASSERT(used_before > 0, "Used should reflect allocation");

    int* ptr2 = ThreadLocalNumaPool<int>::allocate(5);
    SIMPLE_ASSERT(ptr2 != nullptr, "Second allocation should succeed");
    ...
    SIMPLE_ASSERT(ptr2 != ptr1, "Pointers from separate allocations should be distinct");
...
TEST_CASE(thread_local_numa_pool_large)
{
    ...
    SIMPLE_ASSERT(large_ptr != nullptr, "Large allocation should succeed");
...
TEST_CASE(thread_local_numa_pool_reset)
{
    ...
    SIMPLE_ASSERT(ptr != nullptr, "Allocation after reset should succeed");
    ...
    SIMPLE_ASSERT(ptr2 != nullptr, "Allocation after reset should succeed");
...
TEST_CASE(thread_local_numa_pool_multithread)
{
    ...
    SIMPLE_ASSERT(success.load(), "All threads should succeed");
...
TEST_CASE(thread_local_numa_pool_cross_thread_dealloc)
{
    ...
    SIMPLE_ASSERT(consumer_done.load(), "Consumer should complete successfully with valid data");
...
TEST_CASE(thread_local_numa_pool_aligned_types)
{
    ...
    SIMPLE_ASSERT(ptr != nullptr, "Aligned allocation should succeed");

    // Verify alignment
    std::uintptr_t addr = reinterpret_cast<std::uintptr_t>(ptr);
    SIMPLE_ASSERT(addr % 64 == 0, "Pointer must be 64-byte aligned");

    // Verify each element is aligned
    for (int i = 0; i < 10; ++i)
    {
        std::uintptr_t elem_addr = reinterpret_cast<std::uintptr_t>(&ptr[i]);
        SIMPLE_ASSERT(elem_addr % 64 == 0, "Each element must be 64-byte aligned");
    }
...
TEST_CASE(numa_allocator_aligned_types)
{
    ...
    SIMPLE_ASSERT(local_ptr != nullptr, "Local policy aligned allocation should succeed");

    std::uintptr_t local_addr = reinterpret_cast<std::uintptr_t>(local_ptr);
    SIMPLE_ASSERT(local_addr % 64 == 0, "NumaLocalPolicy must return 64-byte aligned pointer");
    ...
    SIMPLE_ASSERT(interleaved_ptr != nullptr, "Interleaved policy aligned allocation should succeed");

    std::uintptr_t interleaved_addr = reinterpret_cast<std::uintptr_t>(interleaved_ptr);
    SIMPLE_ASSERT(interleaved_addr % 64 == 0,
...
TEST_CASE(bind_thread_to_node_validation)
{
    ...
    SIMPLE_ASSERT(!result_invalid, "Binding to invalid node should fail");

    bool result_out_of_range = bind_thread_to_node(1000);
    SIMPLE_ASSERT(!result_out_of_range, "Binding to out-of-range node should fail");
...
TEST_CASE(allocator_exception_safety)
{
    ...
    SIMPLE_ASSERT(threw, "Oversized allocation should throw bad_alloc");
...
void benchmark_numa_allocator()
{
    ...
    double ratio = local_time / std_time;
    ...
    std::cout << "NumaLocalVector push_back (1000 elements, no reserve): " << format_time(vec_numa_time) << "\n";

    double vec_ratio = vec_numa_time / vec_std_time;
    std::cout << "Ratio (NUMA/std): " << std::fixed << std::setprecision(2) << vec_ratio << "x\n";
    std::cout << "Note: Use reserve() to avoid repeated NUMA syscalls during growth.\n";
    ...
    // Reset pool before benchmark to ensure clean state
    ...
    double pool_time = measure_perf(
        []() {
            ...
            for (int i = 0; i < 100; ++i)
            {
                int* ptr = ThreadLocalNumaPool<int>::allocate(10);
                DoNotOptimize(ptr);
                // deallocate is no-op for pool allocations
                ThreadLocalNumaPool<int>::deallocate(ptr, 10);
            }
    ...
    pool_time = pool_time / 100.0;

    std::cout << "Direct NumaAllocator (10 ints): " << format_time(direct_time) << "\n";
    std::cout << "ThreadLocalNumaPool (10 ints, pool hit): " << format_time(pool_time) << "\n";

    if (pool_time > 0 && direct_time > 0)
    {
        double speedup = direct_time / pool_time;
        std::cout << "Pool speedup: " << std::fixed << std::setprecision(1) << speedup << "x faster\n";
    }
}
