...
#include "test_FatPBinary.h"
#endif

namespace fat_p::testing::fatpbinary
{

using fat_p::BinaryBuffer;
using fat_p::BinaryWriter;
using fat_p::BinaryReader;
using fat_p::BinaryResult;
using fat_p::BinaryError;
using fat_p::binary_encode_to;
using fat_p::binary_decode_from;
...
TEST_CASE(buffer_alignment)
{
    ...
    SIMPLE_ASSERT(addr % 64 == 0, "BinaryBuffer must be 64-byte aligned");
...
TEST_CASE(int_roundtrip)
{
    ...
    for (int v : values)
    {
        ...
        SIMPLE_ASSERT(enc.has_value(), "Encode int failed");

        auto dec = binary_decode_from<int>(buf);
        SIMPLE_ASSERT(dec.has_value(), dec.error().message.c_str());
...
TEST_CASE(uint64_roundtrip)
{
    ...
    for (std::uint64_t v : values)
    {
        BinaryBuffer buf;
        auto enc = binary_encode_to(buf, v);
        SIMPLE_ASSERT(enc.has_value(), "Encode uint64 failed");
...
{
    {
        ...
        SIMPLE_ASSERT(enc.has_value(), "Encode true failed");

        auto dec = binary_decode_from<bool>(buf);
        SIMPLE_ASSERT(dec.has_value() && *dec == true, "true roundtrip");
    }

    {
        BinaryBuffer buf;
        auto enc = binary_encode_to(buf, false);
        SIMPLE_ASSERT(enc.has_value(), "Encode false failed");

        auto dec = binary_decode_from<bool>(buf);
        SIMPLE_ASSERT(dec.has_value() && *dec == false, "false roundtrip");
    }
...
TEST_CASE(float_roundtrip)
{
    ...
    for (float v : values)
    {
        BinaryBuffer buf;
        auto enc = binary_encode_to(buf, v);
        SIMPLE_ASSERT(enc.has_value(), "Encode float failed");
...
TEST_CASE(double_roundtrip)
{
    ...
    for (double v : values)
    {
        BinaryBuffer buf;
        auto enc = binary_encode_to(buf, v);
        SIMPLE_ASSERT(enc.has_value(), "Encode double failed");
...
TEST_CASE(string_roundtrip)
{
    ...
    for (const auto& s : values)
    {
        ...
        SIMPLE_ASSERT(enc.has_value(), "Encode string failed");
...
TEST_CASE(vector_int_roundtrip)
{
    ...
    BinaryBuffer buf;
    auto enc = binary_encode_to(buf, v);
    SIMPLE_ASSERT(enc.has_value(), "Encode vector failed");
...
TEST_CASE(nested_vector_roundtrip)
{
    ...
    BinaryBuffer buf;
    auto enc = binary_encode_to(buf, v);
    SIMPLE_ASSERT(enc.has_value(), "Encode nested vector failed");
...
TEST_CASE(empty_vector_roundtrip)
{
    ...
    BinaryBuffer buf;
    auto enc = binary_encode_to(buf, v);
    SIMPLE_ASSERT(enc.has_value(), "Encode empty vector failed");

    auto dec = binary_decode_from<std::vector<int>>(buf);
    SIMPLE_ASSERT(dec.has_value(), dec.error().message.c_str());
    SIMPLE_ASSERT(dec->empty(), "empty vector roundtrip");
...
TEST_CASE(vector_string_roundtrip)
{
    ...
    BinaryBuffer buf;
    auto enc = binary_encode_to(buf, v);
    SIMPLE_ASSERT(enc.has_value(), "Encode vector<string> failed");
...
TEST_CASE(map_roundtrip)
{
    ...
    SIMPLE_ASSERT(enc.has_value(), "Encode map failed");
...
TEST_CASE(nested_map_roundtrip)
{
    ...
    BinaryBuffer buf;
    auto enc = binary_encode_to(buf, m);
    SIMPLE_ASSERT(enc.has_value(), "Encode nested map failed");
...
TEST_CASE(empty_map_roundtrip)
{
    ...
    BinaryBuffer buf;
    auto enc = binary_encode_to(buf, m);
    SIMPLE_ASSERT(enc.has_value(), "Encode empty map failed");

    auto dec = binary_decode_from<std::map<std::string, int>>(buf);
    SIMPLE_ASSERT(dec.has_value(), dec.error().message.c_str());
    SIMPLE_ASSERT(dec->empty(), "empty map roundtrip");
...
TEST_CASE(enum_roundtrip)
{
    ...
    for (TestEnum v : values)
    {
        BinaryBuffer buf;
        auto enc = binary_encode_to(buf, v);
        SIMPLE_ASSERT(enc.has_value(), "Encode enum failed");

        auto dec = binary_decode_from<TestEnum>(buf);
        SIMPLE_ASSERT(dec.has_value(), dec.error().message.c_str());
        SIMPLE_ASSERT(*dec == v, "enum roundtrip");
    }
...
TEST_CASE(cross_container)
{
    ...
    SIMPLE_ASSERT(enc.has_value(), "Encode failed");

    std::vector<std::uint8_t> sbuf(hbuf.begin(), hbuf.end());

    auto r1 = binary_decode_from<std::string>(hbuf);
    SIMPLE_ASSERT(r1.has_value() && *r1 == "test123", "Decode from HpcVector");

    auto r2 = binary_decode_from<std::string>(sbuf);
    SIMPLE_ASSERT(r2.has_value() && *r2 == "test123", "Decode from std::vector");
...
TEST_CASE(decode_invalid_tag)
{
    ...
    SIMPLE_ASSERT(!dec.has_value(), "Invalid tag should fail");
    SIMPLE_ASSERT(!dec.error().message.empty(), "Error message should exist");
...
TEST_CASE(decode_truncated_string)
{
    ...
    SIMPLE_ASSERT(!dec.has_value(), "Truncated string should fail");
...
TEST_CASE(decode_truncated_vector)
{
    ...
    for (std::size_t cut = 1; cut < buf.size(); ++cut)
    {
        ...
        SIMPLE_ASSERT(!dec.has_value(), "Truncated vector should fail");
    }
...
TEST_CASE(decode_type_mismatch_string_as_int)
{
    ...
    auto dec = binary_decode_from<int>(buf);
    SIMPLE_ASSERT(!dec.has_value(), "Type mismatch should fail");
...
TEST_CASE(decode_empty_buffer)
{
    ...
    SIMPLE_ASSERT(!dec_int.has_value(), "Empty buffer should fail for int");

    auto dec_str = binary_decode_from<std::string>(empty);
    SIMPLE_ASSERT(!dec_str.has_value(), "Empty buffer should fail for string");

    auto dec_vec = binary_decode_from<std::vector<int>>(empty);
    SIMPLE_ASSERT(!dec_vec.has_value(), "Empty buffer should fail for vector");
...
TEST_CASE(decode_impossible_length)
{
    ...
    SIMPLE_ASSERT(!dec.has_value(), "Impossible length should fail");
...
TEST_CASE(decode_partial_map)
{
    ...
    SIMPLE_ASSERT(!dec.has_value(), "Partial map should fail");
...
TEST_CASE(decode_nested_truncation)
{
    ...
    SIMPLE_ASSERT(!dec.has_value(), "Truncated nested should fail");
...
TEST_CASE(decode_huge_length_protection)
{
    ...
    auto dec = binary_decode_from<std::vector<int>>(buf);
    SIMPLE_ASSERT(!dec.has_value(), "Huge length should fail");
...
TEST_CASE(fuzz_nested_structures)
{
    ...
    for (int iter = 0; iter < 300; ++iter)
    {
        ...
        BinaryBuffer buf;
        auto enc = binary_encode_to(buf, v);
        SIMPLE_ASSERT(enc.has_value(), "Encode nested failed");
