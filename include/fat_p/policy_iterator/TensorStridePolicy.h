...
FATP_META:
  ...
  path: include/fat_p/policy_iterator/TensorStridePolicy.h
  namespace: fat_p
  layer: Domain
  summary: "Internal implementation for Tensor stride iteration policies."
 ...
 *       TensorStridePolicy is more general and performs rank-generic state
 *       updates. Benchmark the specialized and generic policies against the
 *       corresponding manual loop on the target compiler and workload.
 *
 ...
 * auto it = PolicyIterator<int, Stride1DPolicy<int>>::begin(data, allocationEnd, policy);
 * @endcode
 */
template <typename T>
struct Stride1DPolicy
{
    ...
    /// PolicyIterator passes the real allocation end so the final increment can clamp safely.
...
private:
    ...
    pointer mLast = nullptr; ///< Last logical element, initialized on reaching/constructing end
...
public:
    ...
    void advance(pointer& ptr, pointer bufferEnd)
    {
        FATP_ENFORCE(!atEnd(), "Cannot advance past end");

        if (mPosition + 1 == mCount)
        {
            mLast = ptr;
            ptr = bufferEnd;
            ++mPosition;
        ...
        FATP_ENFORCE(bufferEnd - ptr > mStride, "Stride1D buffer does not contain the next logical element");
        ptr += mStride;
        ++mPosition;
    }
    ...
    void retreat(pointer& ptr)
    {
        ...
        if (atEnd())
        {
            FATP_ENFORCE(mLast != nullptr, "Stride1D end iterator is not initialized");
            ptr = mLast;
            --mPosition;
            return;
        }
    ...
    {
        // The caller supplies a real one-past-allocation pointer. It may be
        // farther than the last logical element; it must never be a fabricated
        // strided sentinel outside the allocation.
#ifndef NDEBUG
        auto span = bufferEnd - base;
        auto minimumSpan = static_cast<std::ptrdiff_t>(mCount - 1) * mStride + 1;
        FATP_ENFORCE(span >= minimumSpan, "Stride1D: buffer does not contain every logical element");
        ...
        (void)bufferEnd;
#endif
        mLast = base + static_cast<std::ptrdiff_t>(mCount - 1) * mStride;
        mPosition = mCount;
    }
};
 ...
 *   - The end pointer passed to PolicyIterator is the real one-past-allocation
 *     pointer and the allocation contains every logical element.
 *
...
public:
    ...
    void retreat(pointer& ptr)
    {
        ...
        if (atEnd())
        {
            FATP_ENFORCE(mLast != nullptr, "Stride2D end iterator is not initialized");
            ptr = mLast;
            mRow = mRows - 1;
            mCol = mCols - 1;
            --mPosition;
    ...
    void setToEnd([[maybe_unused]] pointer& ptr, pointer base, pointer bufferEnd)
    {
        // Validate against the last logical element while retaining the real
        // one-past-allocation pointer as the iterator sentinel.
#ifndef NDEBUG
        auto span = bufferEnd - base;
        auto minimumSpan = static_cast<std::ptrdiff_t>(mRows - 1) * mRowStride +
                           static_cast<std::ptrdiff_t>(mCols - 1) * mColStride + 1;
        FATP_ENFORCE(span >= minimumSpan, "Stride2D: buffer does not contain every logical element");
        (void)base;      // Used only in debug
        (void)bufferEnd; // Used only in debug
#else
        (void)base;
        (void)bufferEnd;
#endif
        mLast = base + static_cast<std::ptrdiff_t>(mRows - 1) * mRowStride +
                static_cast<std::ptrdiff_t>(mCols - 1) * mColStride;
        mRow = mRows;
        mCol = 0;
        mPosition = mTotal;
    }
