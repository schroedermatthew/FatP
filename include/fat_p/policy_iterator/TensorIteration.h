...
FATP_META:
  ...
  path: include/fat_p/policy_iterator/TensorIteration.h
  namespace: fat_p
  layer: Domain
  summary: "Internal implementation for Tensor iteration composition helpers."
 ...
 * tensors using counted loops over explicit shapes and strides.
 *
 * Traversal strategy:
 * - 1D/2D tensors use direct counted loops.
 * - 3D+ tensors use scalar outer loops and recurse over the inner dimensions.
 *
 * This eliminates nested-loop boilerplate while keeping traversal explicit.
 * Benchmark target workloads when iteration cost is material.
 *
...
// ============================================================================
//
// TensorIteration provides high-level counted-loop helpers for explicit shape/stride metadata.
//
// EXPECTATIONS:
//   - For 1D runtime-stride walks (e.g., scanning a column): the helper uses a counted loop.
//   - For 2D row-major traversal (contiguous or padded rows): the helper uses nested loops.
//   - For arbitrary N-D / permuted axes / complex stride patterns: use TensorStridePolicy directly.
//
// LIMITATIONS / CONTRACT:
//   These helpers accept positive strides and visit logical indices in row-major order. The caller
//   owns the buffer-size precondition: every indexed element must be inside the same allocation.
//
// NOTE:
 ...
 *
 * Strategy:
 * - 1D/2D: Direct counted loops form pointers only for logical elements.
...
void iterateNDImpl(T* base, const std::size_t* shape, const std::ptrdiff_t* strides, std::size_t ndim, Func& fn)
{
    ...
    else if (ndim == 2)
    {
        for (std::size_t row = 0; row < shape[0]; ++row)
        {
            ...
            {
                fn(base[static_cast<std::ptrdiff_t>(row) * strides[0] + static_cast<std::ptrdiff_t>(col) * strides[1]]);
            }
        }
    }
    else
 ...
 *
 * @par Performance
 * Equivalent to hand-written counted nested loops. Benchmark the target
 * layout and compiler when traversal performance is material.
 *
...
void iterateND(T* base,
               ...
               Func&& fn)
{
    FATP_ALWAYS_ENFORCE(base != nullptr, "iterateND requires a non-null base pointer");
...
void forEachSlice(T* base,
                  ...
                  SliceFunc&& fn)
{
    FATP_ALWAYS_ENFORCE(base != nullptr, "forEachSlice requires a non-null base pointer");
