...
FATP_META:
  ...
  path: include/fat_p/tensor/TensorEinsum.h
  namespace: fat_p
  layer: Domain
  summary: "Internal implementation for the supported Tensor einsum patterns."
...
inline std::tuple<std::string, std::string, std::string> parse_einsum_notation(const std::string& notation)
{
    ...
    if (notation.find("->", arrow_pos + 2) != std::string::npos)
    {
        throw std::invalid_argument("Invalid einsum notation: multiple '->' separators");
    }
    ...
    inputs.erase(std::remove_if(inputs.begin(), inputs.end(), is_space), inputs.end());
    output.erase(std::remove_if(output.begin(), output.end(), is_space), output.end());

    const auto valid_labels = [](const std::string& labels) {
        return std::all_of(labels.begin(), labels.end(), [](unsigned char value) {
            return std::isalpha(value) != 0;
        });
    };
    if (!valid_labels(output))
    {
        throw std::invalid_argument("Invalid einsum notation: output labels must be alphabetic");
    }
    ...
    {
        if (inputs.find(',', comma_pos + 1) != std::string::npos)
        {
            throw std::invalid_argument("Invalid einsum notation: more than two inputs");
        }
        // Two inputs
        std::string input1 = inputs.substr(0, comma_pos);
        std::string input2 = inputs.substr(comma_pos + 1);
        if (input1.empty() || input2.empty() || !valid_labels(input1) || !valid_labels(input2))
        {
            throw std::invalid_argument("Invalid einsum notation: input labels must be non-empty and alphabetic");
        }
    ...
    {
        // Single input
        if (inputs.empty() || !valid_labels(inputs))
        {
            throw std::invalid_argument("Invalid einsum notation: input labels must be non-empty and alphabetic");
        }
 ...
 */
template <ReadableTensor A>
Tensor<typename A::value_type> einsum(const std::string& notation, const A& a)
{
    ...
    if (in1.size() != a.rank())
    {
        throw std::invalid_argument("einsum: unary label count must match tensor rank");
    }
    ...
    if (in1.length() == 2 && out.length() == 2 && in1[0] == out[1] && in1[1] == out[0])
    {
        ...
        size_t rows = a.extents()[0];
        size_t cols = a.extents()[1];
    ...
    // Extract a repeated-label diagonal: ii->i
    if (in1.length() == 2 && in1[0] == in1[1] && out.length() == 1 && out[0] == in1[0])
    {
        if (a.rank() != 2 || a.extents()[0] != a.extents()[1])
        {
            throw std::invalid_argument("einsum diagonal: input must be a square rank-2 tensor");
        }
        Tensor<T> result({a.extents()[0]});
        for (size_t i = 0; i < a.extents()[0]; ++i)
        {
            result[i] = a(i, i);
        }
...
Tensor<typename A::value_type> einsum(const std::string& notation, const A& a, const B& b)
{
    using T = typename A::value_type;
    auto [in1, in2, out] = detail::parse_einsum_notation(notation);
    ...
    if (in1.size() != a.rank() || in2.size() != b.rank())
    {
        throw std::invalid_argument("einsum: input label counts must match tensor ranks");
    }
    for (char label : out)
    {
        if (in1.find(label) == std::string::npos && in2.find(label) == std::string::npos)
        {
            throw std::invalid_argument("einsum: output label does not appear in an input");
        }
    }
    ...
    if (in1 == "ij" && in2 == "jk" && out == "ik")
    {
        ...
        if (a.extents()[1] != b.extents()[0])
        {
            throw std::invalid_argument("einsum matmul: incompatible dimensions");
        }

        size_t m = a.extents()[0];
        size_t n = a.extents()[1];
        size_t p = b.extents()[1];
    ...
    if (in1 == "bij" && in2 == "bjk" && out == "bik")
    {
        ...
        if (a.extents()[2] != b.extents()[1])
        {
            throw std::invalid_argument("einsum batch matmul: incompatible dimensions");
        }

        size_t batch = a.extents()[0];
        size_t m = a.extents()[1];
        size_t n = a.extents()[2];
        size_t p = b.extents()[2];
    ...
    if (in1 == "ij" && in2 == "ij" && out.empty())
    {
        if (a.extents() != b.extents())
        {
            throw std::invalid_argument("einsum Frobenius product: matrices must have the same shape");
        }
    ...
    if (in1 == in2 && in1 == out)
    {
        for (size_t i = 0; i < in1.size(); ++i)
        {
            if (in1.find(in1[i]) != i)
            {
                throw std::invalid_argument("einsum element-wise: repeated labels are not supported");
            }
        }
        if (a.rank() != b.rank())
        {
