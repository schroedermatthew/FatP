...
FATP_META:
  ...
  path: include/fat_p/SortedContainer.h
  namespace: fat_p
  layer: Containers
...
#include "Expected.h"
#include "Concepts.h" // For concepts::reservable
#include "FatPConcepts.h" // For concepts::has_shrink_to_fit
...
// NOTE: concepts::has_shrink_to_fit and concepts::reservable provided by Concepts.h
...
struct is_fuzzy_unique_policy<Policy, std::void_t<typename Policy::is_fuzzy_policy>> : std::true_type
{
};
...
struct FatPLoggingPolicy
{
    ...
    static void log(const char* action, const T& value)
    {
        ...
        conditionalPrintError([msg = oss.str()] {
            return msg;
        });
    }
...
{
    static constexpr double get() noexcept
    {
 ...
 */
template <typename BasePolicy, typename Logger = FatPLoggingPolicy, bool IsFuzzy = is_fuzzy_unique_policy_v<BasePolicy>>
...
struct LoggingUniquePolicy<BasePolicy, Logger, false> : BasePolicy
{
    ...
    static bool insert(Vector& vec, T&& value, Compare comp, Args&&... args)
    {
        ...
        return BasePolicy::insert(vec, std::forward<T>(value), comp, std::forward<Args>(args)...);
    }
};
    ...
    [[nodiscard]] Expected<bool, std::string> insert(U&& value, EpsParams... eps)
    {
        ...
        try
        {
            ...
            if constexpr (is_fuzzy_unique_policy_v<UniquenessPolicy>)
            {
                ...
                {
                    inserted = UniquenessPolicy::insert(mInternalContainer.get(),
                                                        std::forward<U>(value),
                                                        mCompare,
                                                        default_epsilon_v<T>(),
                                                        default_epsilon_v<T>());
                ...
                {
                    inserted =
                        UniquenessPolicy::insert(mInternalContainer.get(), std::forward<U>(value), mCompare, eps...);
                }
            }
            else
            {
                inserted = UniquenessPolicy::insert(mInternalContainer.get(), std::forward<U>(value), mCompare);
            }
            validateInvariant_unlocked();
        ...
        try
        {
            ...
            if constexpr (std::is_base_of_v<std::forward_iterator_tag, iter_category>)
            {
                ...
                if (range_size < 16)
                {
                    ...
                    if (start_it < mInternalContainer.get().begin() || start_it > container_end)
                    {
                    ...
                    {
                        auto insertion_point = std::upper_bound(mInternalContainer.get().begin(), it, *it, mCompare);
                        std::rotate(insertion_point, it, it + 1);
                    }
                }
                else
                {
                    std::stable_sort(mInternalContainer.get().begin(), mInternalContainer.get().end(), mCompare);
                }
            }
            else
            {
...
private:
    ...
    {
        auto it = std::lower_bound(mInternalContainer.get().begin(), mInternalContainer.get().end(), value, mCompare);
        if (it != mInternalContainer.get().end() && !mCompare(*it, value) && !mCompare(value, *it))
        {
            return it;
        }
        return mInternalContainer.get().end();
    ...
    {
        FATP_ENFORCE(std::is_sorted(mInternalContainer.get().begin(), mInternalContainer.get().end(), mCompare),
    ...
    void applyUniquenessPolicy_unlocked(EpsParams... eps)
    {
        if constexpr (std::is_base_of_v<OnlyUniquePolicy, UniquenessPolicy>)
        {
            ...
            auto lastUnique = std::unique(mInternalContainer.get().begin(), mInternalContainer.get().end(), equiv);
        ...
        else if constexpr (is_fuzzy_unique_policy_v<UniquenessPolicy>)
        {
            ...
            if (!container.empty())
            {
                ...
                for (auto read_it = container.begin() + 1; read_it != container.end(); ++read_it)
                {
...
bool operator==(const SortedContainer<T, UP, CP, A, ConP, BP>& lhs, const SortedContainer<T, UP, CP, A, ConP, BP>& rhs)
{
