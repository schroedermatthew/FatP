...
#elif defined(SIMD_NEON) && SIMD_NEON_AARCH64
    using storage_type = std::conditional_t<std::is_same_v<T, float>, uint32x4_t, uint64x2_t>;
#elif defined(SIMD_NEON)
    // ARM32: float uses NEON, double uses scalar bool
    using storage_type = std::conditional_t<std::is_same_v<T, float>, uint32x4_t, bool>;
#else
...
public:
    ...
    using storage_type = std::conditional_t<std::is_same_v<T, float>, float32x4_t, double>;
#else
