...
#include "ComparisonTolerances.h" // For default epsilon values
...
#include "ScopeGuardPolicies.h" // For conditionalPrintError in LoggingUniquePolicy
...
namespace fat_p {
     ...
     * or zero for non-floating-point types.
     */
    template <typename T>
    ...
    struct default_epsilon<float> {
        static constexpr float value = kDefaultFloatEpsilon;
    };
    
    template <>
    struct default_epsilon<double> {
        static constexpr double value = kDefaultDoubleEpsilon;
    };
    
    template <typename T>
    inline constexpr T default_epsilon_v = default_epsilon<T>::value;
    ...
    public:
         ...
         * @param eps Optional epsilon params for fuzzy policies. If omitted for
         *            FuzzyUniquePolicy with floating-point T, uses default_epsilon_v<T>.
         * @return Expected<bool, std::string> True if inserted (or skipped), error on failure.
         */
            ...
            try {
                ...
                if constexpr (is_fuzzy_unique_policy<UniquenessPolicy>::value) {
                    if constexpr (sizeof...(eps) == 0) {
                        // No epsilon provided; use type-appropriate default
                        inserted = UniquenessPolicy::insert(
                            internalContainer_.get(), std::forward<U>(value), compare_,
                            default_epsilon_v<T>, default_epsilon_v<T>);
                    } else {
                        inserted = UniquenessPolicy::insert(
                            internalContainer_.get(), std::forward<U>(value), compare_, eps...);
                    }
                }
                else {
        ...
        [[nodiscard]] Expected<void, std::string> insertRange(InputIt first, InputIt last, EpsParams... eps) {
            ...
            try {
                ...
                if constexpr (std::is_base_of_v<std::forward_iterator_tag, iter_category>) {
                    ...
                    // Just append and sort â€” no optimization possible
                    internalContainer_.get().insert(internalContainer_.get().end(), first, last);
                    std::stable_sort(internalContainer_.get().begin(), 
                                     internalContainer_.get().end(), compare_);
                }
                ...
                if constexpr (std::is_same_v<UniquenessPolicy, OnlyUniquePolicy>) {
                    ...
                    // Custom fuzzy unique: greedy forward scan comparing against last representative.
                    // We avoid std::unique because fuzzy equality is non-transitive, and std::unique
                    // technically requires an equivalence relation (though most implementations work).
                    // This explicit algorithm compares each element against the last KEPT element,
                    // which correctly handles non-transitive predicates.
