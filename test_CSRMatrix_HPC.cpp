...
TEST_CASE(is_symmetric_structural_check)
{
    ...
    SIMPLE_ASSERT(!A.is_symmetric(), "Non-symmetric matrix identified as symmetric");
    ...
    SIMPLE_ASSERT(B.is_symmetric(), "Symmetric matrix not recognized");
...
TEST_CASE(is_symmetric_diagonal)
{
    ...
    SIMPLE_ASSERT(D.is_symmetric(), "Diagonal matrix should be symmetric");
...
TEST_CASE(is_symmetric_empty)
{
    ...
    Matrix E(5, 5);
    SIMPLE_ASSERT(E.is_symmetric(), "Empty matrix should be symmetric");
...
TEST_CASE(aligned_accessors)
{
    ...
    SIMPLE_ASSERT(v != nullptr, "values_aligned() returned null");
    SIMPLE_ASSERT(c != nullptr, "col_indices_aligned() returned null");

    // Const access
    const double* vals_c = A_const.values_aligned();
    const int32_t* cols_c = A_const.col_indices_aligned();
    SIMPLE_ASSERT(vals_c != nullptr, "const values_aligned() returned null");
    SIMPLE_ASSERT(cols_c != nullptr, "const col_indices_aligned() returned null");
...
void benchmark_spmv()
{
    std::cout << "\n" << colors::cyan() << "SpMV Benchmarks:" << colors::reset() << "\n\n";

    using Matrix = fat_p::HpcCSRMatrix<double>;

    // Build a sparse diagonal-dominant matrix
    constexpr std::size_t N = 1000;
    constexpr std::size_t nnz_per_row = 7;

    std::vector<int32_t> rows, cols;
    std::vector<double> vals;
    rows.reserve(N * nnz_per_row);
    cols.reserve(N * nnz_per_row);
    vals.reserve(N * nnz_per_row);
...
{
    std::cout << "\n" << colors::cyan() << "Transpose Benchmarks:" << colors::reset() << "\n\n";
    ...
    for (std::size_t i = 0; i < N; ++i)
    {
        for (std::size_t j = 0; j < N; ++j)
        {
            if ((i + j) % 10 == 0)
            {
                rows.push_back(static_cast<int32_t>(i));
                cols.push_back(static_cast<int32_t>(j));
                vals.push_back(static_cast<double>(i * N + j));
            }
        }
    }
...
bool test_CSRMatrix_HPC()
{
    ...
    csrmatrix_hpc::benchmark_transpose();
