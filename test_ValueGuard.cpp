...
public:
    ...
    explicit ThrowOnAssign(int v = 0, int throws_remaining = 0) 
        : value(v), throw_countdown(throws_remaining) {}
    
    ThrowOnAssign(const ThrowOnAssign& other) 
        : value(other.value), throw_countdown(other.throw_countdown) {}
...
bool test_custom_restorer() {
    ...
    {
        auto guard = make_value_guard_custom(value, 100, 
...
bool test_custom_restorer_with_logging() {
    ...
    {
        auto guard = make_value_guard_custom(value, std::string("temporary"),
...
bool test_custom_restorer_incremental() {
    ...
    {
        auto guard = make_value_guard_custom(counter, 10,
 ...
 */
bool test_constructor_exception_safety() {
 ...
 */
bool test_deduction_guide_disambiguation() {
    int value = 42;
...
bool test_performance_custom_policy() {
    ...
    for (int i = 0; i < ITERATIONS; ++i) {
        auto guard = make_value_guard_custom(value, i,
            [](int& t, int&& o) noexcept { t = o; });
        DoNotOptimize(value);
    }
...
int main() {
    bool success = fat_p::testing::test_ValueGuard();
