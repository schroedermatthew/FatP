...
TEST_CASE(parse_unsigned_integers)
{
    ...
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse uint 0");
    SIMPLE_ASSERT(parser.result().is_unsigned(), "Is unsigned");
    SIMPLE_ASSERT(parser.result().as_unsigned() == 0, "Value 0");

    parser.reset();
    data = encode_uint(23);
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse uint 23");
    SIMPLE_ASSERT(parser.result().as_unsigned() == 23, "Value 23");

    // 1-byte integer
    parser.reset();
    data = encode_uint(100);
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse uint 100");
    SIMPLE_ASSERT(parser.result().as_unsigned() == 100, "Value 100");

    // 2-byte integer
    parser.reset();
    data = encode_uint(1000);
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse uint 1000");
    SIMPLE_ASSERT(parser.result().as_unsigned() == 1000, "Value 1000");

    // 4-byte integer
    parser.reset();
    data = encode_uint(100000);
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse uint 100000");
    SIMPLE_ASSERT(parser.result().as_unsigned() == 100000, "Value 100000");

    // 8-byte integer
    parser.reset();
    data = encode_uint(0x123456789ABCDEFULL);
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse uint large");
    SIMPLE_ASSERT(parser.result().as_unsigned() == 0x123456789ABCDEFULL, "Value large");
...
TEST_CASE(parse_negative_integers)
{
    ...
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse -1");
    SIMPLE_ASSERT(parser.result().is_signed(), "Is signed");
    SIMPLE_ASSERT(parser.result().as_signed() == -1, "Value -1");

    // -100 is encoded as negint(99)
    parser.reset();
    data = encode_negint(99);
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse -100");
    SIMPLE_ASSERT(parser.result().as_signed() == -100, "Value -100");

    // -1000 is encoded as negint(999)
    parser.reset();
    data = encode_negint(999);
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse -1000");
    SIMPLE_ASSERT(parser.result().as_signed() == -1000, "Value -1000");
...
TEST_CASE(parse_byte_strings)
{
    ...
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse empty bytes");
    SIMPLE_ASSERT(parser.result().is_bytes(), "Is bytes");
    SIMPLE_ASSERT(parser.result().as_bytes().empty(), "Empty bytes");

    // Short byte string
    parser.reset();
    data = encode_bytes({0x01, 0x02, 0x03, 0x04});
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse bytes");
    auto bytes = parser.result().as_bytes();
    SIMPLE_ASSERT(bytes.size() == 4, "Bytes size");
    SIMPLE_ASSERT(bytes[0] == 1 && bytes[3] == 4, "Bytes content");
...
TEST_CASE(parse_text_strings)
{
    ...
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse empty text");
    SIMPLE_ASSERT(parser.result().is_string(), "Is string");
    SIMPLE_ASSERT(parser.result().as_string().empty(), "Empty string");

    // Regular string
    parser.reset();
    data = encode_text("hello");
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse hello");
    SIMPLE_ASSERT(parser.result().as_string() == "hello", "String content");

    // Longer string
    parser.reset();
    std::string longstr(300, 'x');
    data = encode_text(longstr);
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse long string");
    SIMPLE_ASSERT(parser.result().as_string() == longstr, "Long string content");
...
TEST_CASE(parse_simple_values)
{
    ...
    // false
    SIMPLE_ASSERT(parser.feed(encode_false()) == ParseStatus::Done, "Parse false");
    SIMPLE_ASSERT(parser.result().is_bool(), "Is bool");
    SIMPLE_ASSERT(parser.result().as_bool() == false, "Value false");

    // true
    parser.reset();
    SIMPLE_ASSERT(parser.feed(encode_true()) == ParseStatus::Done, "Parse true");
    SIMPLE_ASSERT(parser.result().as_bool() == true, "Value true");

    // null
    parser.reset();
    SIMPLE_ASSERT(parser.feed(encode_null()) == ParseStatus::Done, "Parse null");
    SIMPLE_ASSERT(parser.result().is_null(), "Is null");
...
TEST_CASE(parse_floats)
{
    ...
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse double");
    SIMPLE_ASSERT(parser.result().is_float(), "Is float");
    SIMPLE_ASSERT(std::fabs(parser.result().as_float() - pi) < 1e-15, "Double value");
...
TEST_CASE(parse_arrays)
{
    ...
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse empty array");
    SIMPLE_ASSERT(parser.result().is_array(), "Is array");
    SIMPLE_ASSERT(parser.result().as_array().empty(), "Empty array");
    ...
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse array [1,2,3]");
    auto& arr = parser.result().as_array();
    SIMPLE_ASSERT(arr.size() == 3, "Array size");
    SIMPLE_ASSERT(arr[0].as_unsigned() == 1, "arr[0]");
    SIMPLE_ASSERT(arr[1].as_unsigned() == 2, "arr[1]");
    SIMPLE_ASSERT(arr[2].as_unsigned() == 3, "arr[2]");
...
TEST_CASE(parse_maps)
{
    ...
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse empty map");
    SIMPLE_ASSERT(parser.result().is_map(), "Is map");
    SIMPLE_ASSERT(parser.result().as_map().empty(), "Empty map");
    ...
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse map");
    auto& m = parser.result().as_map();
    SIMPLE_ASSERT(m.size() == 2, "Map size");
...
TEST_CASE(parse_nested)
{
    ...
    SIMPLE_ASSERT(parser.feed(data) == ParseStatus::Done, "Parse nested");
    auto& outer = parser.result().as_array();
    SIMPLE_ASSERT(outer.size() == 2, "Outer size");
    SIMPLE_ASSERT(outer[0].is_array(), "Inner 0 is array");
    SIMPLE_ASSERT(outer[1].is_array(), "Inner 1 is array");
    SIMPLE_ASSERT(outer[0].as_array()[0].as_unsigned() == 1, "[[1,_],[_,_]]");
    SIMPLE_ASSERT(outer[1].as_array()[1].as_unsigned() == 4, "[[_,_],[_,4]]");
...
TEST_CASE(chunked_input)
{
    ...
    SIMPLE_ASSERT(status == ParseStatus::Done, "Done on last byte");

    // Verify result
    auto& arr = parser.result().as_array();
    SIMPLE_ASSERT(arr.size() == 3, "Array size");
    SIMPLE_ASSERT(arr[0].as_string() == "hello", "First element");
    SIMPLE_ASSERT(arr[1].as_unsigned() == 12345, "Second element");
    SIMPLE_ASSERT(arr[2].as_bytes().size() == 4, "Third element size");
...
TEST_CASE(chunked_random_sizes)
{
    ...
    while (pos < data.size())
    {
        ...
        {
            SIMPLE_ASSERT(pos + chunk_size == data.size(), "Done at end");
            break;
        }
    ...
    SIMPLE_ASSERT(parser.is_done(), "Parser done");
    SIMPLE_ASSERT(parser.result().is_map(), "Result is map");
...
TEST_CASE(limit_max_depth)
{
    ...
    SIMPLE_ASSERT(status == ParseStatus::Error, "Depth exceeded");
    SIMPLE_ASSERT(parser.error() == ParseError::MaxDepthExceeded, "Correct error");
...
TEST_CASE(limit_string_size)
{
    ...
    auto status = parser.feed(data);
    SIMPLE_ASSERT(status == ParseStatus::Error, "String too large");
    SIMPLE_ASSERT(parser.error() == ParseError::MaxStringSizeExceeded, "Correct error");
...
TEST_CASE(limit_total_size)
{
    ...
    auto status = parser.feed(data);
    SIMPLE_ASSERT(status == ParseStatus::Error, "Total size exceeded");
    SIMPLE_ASSERT(parser.error() == ParseError::MaxTotalSizeExceeded, "Correct error");
...
TEST_CASE(error_truncated_input)
{
    ...
    auto status = parser.feed(data);
    SIMPLE_ASSERT(status == ParseStatus::NeedMoreData, "Waiting for more");
...
TEST_CASE(error_reserved_ai)
{
    ...
    auto status = parser.feed(data);
    SIMPLE_ASSERT(status == ParseStatus::Error, "Reserved AI");
    SIMPLE_ASSERT(parser.error() == ParseError::ReservedAdditionalInfo, "Correct error");
...
TEST_CASE(error_indefinite_length)
{
    ...
    auto status = parser.feed(data);
    SIMPLE_ASSERT(status == ParseStatus::Error, "Indefinite not supported");
    SIMPLE_ASSERT(parser.error() == ParseError::IndefiniteLengthNotSupported, "Correct error");
...
TEST_CASE(statistics)
{
    ...
    SIMPLE_ASSERT(stats.bytes_consumed == data.size(), "Bytes consumed");
    SIMPLE_ASSERT(stats.max_depth_seen == 2, "Max depth");
    SIMPLE_ASSERT(stats.values_parsed >= 4, "Values parsed");
