...
namespace fat_p {
    ...
    using FatPJsonArray =
        SmallVector<JsonValue, DEFAULT_INLINE_ARRAY_SIZE, std::allocator<JsonValue>>;
    ...
    namespace json_detail {
        ...
        inline size_t extract_position_from_error(std::string_view msg) noexcept
        {
            ...
            std::from_chars_result parse_result =
                std::from_chars(msg.data() + pos_idx, msg.data() + end_idx, result);
    ...
    try_parse_json(std::string_view json) noexcept
    {
        try
        {
            size_t pos = 0;
    ...
    template <typename Policy = StandardJsonPolicy>
    [[nodiscard]] inline Expected<JsonValue, JsonError>
    try_load_json(const std::string& filename) noexcept
    {
        try
        {
            ...
            {
                return unexpected(
                    JsonError{JsonErrorCode::FileError, "Failed to open file", 0, filename});
            }
            ...
            {
                return unexpected(
                    JsonError{JsonErrorCode::FileError, "Error reading file", 0, filename});
            }
    ...
    template <typename Policy = StandardJsonPolicy>
    [[nodiscard]] inline Expected<JsonValue, JsonError>
    load_json_mmap(const std::string& filename) noexcept
    {
        try
        {
            ...
            {
                return unexpected(
                    JsonError{JsonErrorCode::FileError, "Failed to memory-map file", 0, filename});
            }
        ...
        catch (const std::exception& e)
        {
            ...
            JsonErrorCode code = json_detail::classify_exception(full_msg);
            return unexpected(
                JsonError{code, "Memory-mapped file operation failed.", 0, full_msg});
        }
    }
    ...
    try_save_json(const std::string& filename,
                  ...
                  bool pretty = Policy::pretty_print) noexcept
    {
        try
        {
            ...
            {
                return unexpected(JsonError{
                    JsonErrorCode::FileError, "Failed to open file for writing", 0, filename});
            }
            ...
            {
                return unexpected(
                    JsonError{JsonErrorCode::FileError, "Error writing to file", 0, filename});
            }
    ...
    template <typename Policy = StandardJsonPolicy>
    [[nodiscard]] inline Expected<void, JsonError>
    try_save_atomic(const std::string& filename,
                    const JsonValue& value,
                    ...
                    int initial_delay_ms = 1) noexcept
    {
        ...
        return unexpected(JsonError{
            ...
            "Atomic rename failed after " + std::to_string(max_retries + 1) + 
            " attempts: " + ec.message(), 
            0, 
            filename});
    }
            ...
            if (j.is_int())
            {
                ...
                if constexpr (std::is_integral_v<T>)
                {
                    if constexpr (std::is_same_v<T, uint64_t>)
                    {
                        ...
                        {
                            return unexpected(
                                JsonError{JsonErrorCode::NumericOverflow,
                    ...
                    else
                    {
                        ...
                        if (i64 < static_cast<int64_t>(to_min) ||
                            i64 > static_cast<int64_t>(to_max))
                        {
                            return unexpected(
                                JsonError{JsonErrorCode::NumericOverflow,
                                          "Integer value out of range for target type",
    ...
    struct JsonStats
    {
        ...
        {
            return (parse_count > 0) ? (total_parse_time_ms / static_cast<double>(parse_count))
                                     : 0.0;
        }

        [[nodiscard]] double avg_serialize_time_ms() const noexcept
        {
            return (serialize_count > 0)
                       ? (total_serialize_time_ms / static_cast<double>(serialize_count))
                       : 0.0;
        }
        ...
        [[nodiscard]] double parse_throughput_mb_per_sec() const noexcept
        {
            ...
            return (static_cast<double>(total_bytes_parsed) / 1024.0 / 1024.0) /
                   (total_parse_time_ms / 1000.0);
        }
        ...
        [[nodiscard]] double serialize_throughput_mb_per_sec() const noexcept
        {
            ...
            return (static_cast<double>(total_bytes_serialized) / 1024.0 / 1024.0) /
                   (total_serialize_time_ms / 1000.0);
        }
    };
 ...
 *
 * @details Brings common FatPJsonLite symbols into scope. Expands to:
...
#define USING_FATP_JSON_LITE()              \
    ...
    to_json_object(const FatPJsonObject<Compare, Allocator>& obj)
    {
        JsonObject result;
        for (const auto& [key, value] : obj)
        {
     ...
     * instead of individual insertions which would be O(NÂ²).
     */
    [[nodiscard]] inline FatPJsonObject<> from_json_object(const JsonObject& obj)
    {
        // Use range constructor for O(N log N) instead of O(NÂ²) individual inserts
    ...
    try_query_json_pointer(const JsonValue& root, std::string_view pointer) noexcept
    {
        try
        {
     ...
     */
    [[nodiscard]] inline Expected<JsonValue*, JsonError>
    try_query_json_pointer(JsonValue& root, std::string_view pointer) noexcept
    {
        try
        {
    ...
    try_query_json_as(const JsonValue& root, std::string_view pointer) noexcept
    {
    ...
    template <typename T>
    [[nodiscard]] inline T
    query_json_as_or(const JsonValue& root, std::string_view pointer, T default_value) noexcept
    {
