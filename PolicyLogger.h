...
struct alignas(CACHE_LINE_SIZE) LogRecord {
    ...
    void setLocation(const SourceLocation& loc) {
        ...
        line = loc.line;  // ✅ C++17: MEMBER ACCESS (NO PARENTHESES!)
#endif
...
private:
    ...
    std::conditional_t<
        ...
        SPSCRingBuffer<LogRecord, CPP_UTIL_LOG_BUFFER_SIZE>,
        std::monostate
    > ringBuffer_;
    ...
    void workerLoop() {
        ...
        if constexpr (ModePolicy::NeedsRingBuffer) {
            ...
            // ✅ FIX: Use heap allocation to avoid stack overflow (16KB on stack was too large)
