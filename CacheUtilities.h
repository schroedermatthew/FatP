 ...
 */
inline constexpr size_t destructive_interference_size_v =
#ifdef __cpp_lib_hardware_interference_size
    std::hardware_destructive_interference_size
#elif defined(__APPLE__) && defined(__aarch64__)
    128 // Apple Silicon coherency granule
#else
    64  // x86 and most ARM
#endif
;
 ...
 */
inline constexpr size_t constructive_interference_size_v =
#ifdef __cpp_lib_hardware_interference_size
...
struct CacheLinePaddedImpl<T, true> {
    ...
    static_assert(sizeof(T) == cache_line_size,
                  "Internal error: exact-fit specialization used for non-exact type");
    ...
    template<typename... Args>
    explicit CacheLinePaddedImpl(Args&&... args) : value(std::forward<Args>(args)...) {}
...
template<typename T>
struct alignas(cache_constants::destructive_interference_size_v) CacheLinePadded 
    : detail::CacheLinePaddedImpl<T> 
{
    ...
    template<typename... Args>
    explicit CacheLinePadded(Args&&... args) : Base(std::forward<Args>(args)...) {}
...
inline size_t optimal_block_size(size_t elements, size_t element_size, 
    ...
    if (elements == 0 || element_size == 0) return 1;
...
struct BlockIterator2D {
    ...
    size_t block_start_i() const noexcept { return block_i; }
    size_t block_start_j() const noexcept { return block_j; }
    size_t block_end_i() const noexcept { return std::min(block_i + block_rows, rows); }
    size_t block_end_j() const noexcept { return std::min(block_j + block_cols, cols); }
};
