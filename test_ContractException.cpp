...
TEST_CASE(out_of_range_contract_error)
{
    ...
    {
        std::string msg = e.what();
        SIMPLE_ASSERT(msg.find("Index 10") != std::string::npos, "Message contains index info");
    }
...
TEST_CASE(overflow_underflow_contract_errors)
{
    ...
    {
        std::string msg = e.what();
        SIMPLE_ASSERT(msg.find("overflow") != std::string::npos, "Message contains 'overflow'");
    }
    ...
    {
        std::string msg = e.what();
        SIMPLE_ASSERT(msg.find("underflow") != std::string::npos, "Message contains 'underflow'");
    }
...
TEST_CASE(bad_alloc_inheritance)
{
    ...
    SIMPLE_ASSERT(caught_as_bad_alloc, "AllocContractError should be catchable as std::bad_alloc");
    return true;
}
...
TEST_CASE(message_with_variables)
{
    ...
    {
        std::string msg = e.what();
        SIMPLE_ASSERT(msg.find("42") != std::string::npos, "Message contains interpolated value");
        SIMPLE_ASSERT(msg.find("x") != std::string::npos, "Message contains variable name");
    }
...
TEST_CASE(long_message)
{
    ...
    {
        std::string msg = e.what();
        SIMPLE_ASSERT(msg.length() > 1000, "Long message preserved (with prefix)");
        SIMPLE_ASSERT(msg.find('x') != std::string::npos, "Long message content preserved");
    }
...
TEST_CASE(stream_operator_logic)
{
    ...
    SIMPLE_ASSERT(output.find("[Logic]") != std::string::npos,
                  "Stream output contains category in brackets");
    SIMPLE_ASSERT(output.find("Contract Violation:") != std::string::npos,
                  "Stream output contains message");
    SIMPLE_ASSERT(output.find("precondition failed") != std::string::npos,
...
TEST_CASE(stream_operator_runtime)
{
    ...
    SIMPLE_ASSERT(output.find("[Runtime]") != std::string::npos,
                  "Stream output contains Runtime category");
    SIMPLE_ASSERT(output.find("connection timeout") != std::string::npos,
                  "Stream output contains user text");
...
TEST_CASE(stream_operator_alloc)
{
    ...
    SIMPLE_ASSERT(output.find("[Allocation]") != std::string::npos,
                  "Stream output contains Allocation category");
    SIMPLE_ASSERT(output.find("pool exhausted") != std::string::npos,
                  "Stream output contains user text");
...
TEST_CASE(stream_operator_all_types)
{
    ...
    SIMPLE_ASSERT(output.find("[Logic]") != std::string::npos, "Output contains Logic category");
    SIMPLE_ASSERT(output.find("[Runtime]") != std::string::npos,
...
TEST_CASE(move_constructor_logic)
{
    ...
    SIMPLE_ASSERT(msg.find("movable message") != std::string::npos,
...
TEST_CASE(move_constructor_alloc)
{
    ...
    std::string msg = e2.what();
    SIMPLE_ASSERT(msg.find("alloc movable") != std::string::npos,
...
{
    SIMPLE_ASSERT(std::is_nothrow_move_constructible_v<AllocContractError>,
                  "AllocContractError should be nothrow move constructible");
    SIMPLE_ASSERT(std::is_nothrow_move_assignable_v<AllocContractError>,
...
TEST_CASE(noexcept_methods)
{
    ...
    SIMPLE_ASSERT(noexcept(e.category()), "category() should be noexcept");
    SIMPLE_ASSERT(noexcept(e.message()), "message() should be noexcept");
    SIMPLE_ASSERT(noexcept(ae.what()), "AllocContractError::what() should be noexcept");
    SIMPLE_ASSERT(noexcept(ae.category()), "AllocContractError::category() should be noexcept");
    SIMPLE_ASSERT(noexcept(ae.message()), "AllocContractError::message() should be noexcept");
...
TEST_CASE(rethrow_preserves_message)
{
    ...
    {
        SIMPLE_ASSERT(std::string(e.message()).find("inner throw message") != std::string::npos,
...
TEST_CASE(nested_exception_pattern)
{
    ...
    {
        SIMPLE_ASSERT(std::string(e.what()).find("inner") != std::string::npos,
                      "Outer exception contains inner message");

        try
        {
            std::rethrow_if_nested(e);
            SIMPLE_ASSERT(false, "Should have rethrown nested exception");
        }
        catch (const ContractViolationBase& nested)
        {
            SIMPLE_ASSERT(std::string(nested.message()).find("outer") != std::string::npos,
