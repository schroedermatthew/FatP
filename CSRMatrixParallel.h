 ...
 * equal work (measured by nnz). This is key for good parallel scaling.
 *
 * @tparam PtrType Pointer type from the matrix (typically size_t)
 * @param row_ptrs Row pointer array from CSR matrix
 * @param num_rows Number of rows
 * @param num_partitions Desired number of partitions
 * @return Vector of (start_row, end_row) pairs using size_t
 */
template <typename PtrType>
std::vector<std::pair<std::size_t, std::size_t>> compute_balanced_partitions(
    ...
    std::size_t num_partitions)
{
    ...
    std::size_t total_nnz = static_cast<std::size_t>(row_ptrs[num_rows]);
    ...
    {
        std::size_t row_nnz = static_cast<std::size_t>(row_ptrs[i + 1] - row_ptrs[i]);
        current_nnz += row_nnz;
