...
using fat_p::ExpectedErrorPolicy;
using fat_p::ThrowingErrorPolicy;
using fat_p::DefaultErrorPolicy;
...
TEST_CASE(database_connection_lambda_capture) {
    ...
    (void)db_factory.registerType("postgres-dev", 
        makeDbCreator("PostgreSQL", "localhost", 5432));
    (void)db_factory.registerType("postgres-prod",
        makeDbCreator("PostgreSQL", "prod-server", 5432));
    (void)db_factory.registerType("mysql-dev",
...
TEST_CASE(throwing_make) {
    ...
    SIMPLE_ASSERT(ok_result.has_value(), "Make should succeed when not throwing");
    
    ThrowingWidget::should_throw = true;
    auto fail_result = factory.make("thrower");
    SIMPLE_ASSERT(!fail_result.has_value(), "Make should fail when throwing");
...
TEST_CASE(throwing_error_policy) {
    ...
    try {
        ...
        threw = true;
        SIMPLE_ASSERT(std::string(e.what()).find("not found") != std::string::npos,
                      "Exception message should mention 'not found'");
    }
    SIMPLE_ASSERT(threw, "Should throw on missing key");
...
TEST_CASE(hpc_factory) {
    ...
    (void)factory.registerType("widget", []{ return Widget(42); });
    (void)factory.registerType("another", []{ return Widget(99); });
...
TEST_CASE(singleton_lifetime_policy) {
    ...
    SIMPLE_ASSERT(&factory1 == &factory2, "Should return same instance");
    
    (void)factory1.registerType("singleton_test", []{ return Widget(123); });
    SIMPLE_ASSERT(factory2.hasType("singleton_test"), 
...
TEST_CASE(variadic_parameters) {
    ...
    SIMPLE_ASSERT(result.has_value(), "Should create with parameters");
...
TEST_CASE(reentrant_factory_access) {
    ...
    SIMPLE_ASSERT(result.has_value(), "Re-entrant make should succeed");
...
TEST_CASE(simple_variadic_factory_basic) {
    ...
    SIMPLE_ASSERT(registered, "Should register in legacy factory");
...
TEST_CASE(concurrent_read_write) {
    ...
    for (int i = 0; i < 100; ++i) {
        (void)factory.registerType(i, [i] { return Widget(i); });
    }
    ...
    for (int i = 0; i < NUM_WRITERS; ++i) {
        writers.emplace_back([&, i] {
            ...
            for (int j = 0; j < WRITES_PER_WRITER; ++j) {
                ...
                (void)factory.registerType(key, [i]{ return Widget(i); });
    ...
    SIMPLE_ASSERT(factory.size() >= 100 + NUM_WRITERS * WRITES_PER_WRITER,
...
TEST_CASE(shared_mutex_concurrency) {
    ...
    SIMPLE_ASSERT(read_count.load() > 100, 
...
TEST_CASE(transparent_lookup) {
    ...
    SIMPLE_ASSERT(lit_result.has_value(), "Should work with string literal");
    ASSERT_EQ(lit_result->value_, 42, "Value should be correct");
    
    // Test hasType with literal
    SIMPLE_ASSERT(factory.hasType("widget"), "hasType should work with literal");
    SIMPLE_ASSERT(!factory.hasType("nonexistent"), "hasType should return false for missing");
    
    // Test with explicit std::string
    std::string key = "widget";
    auto str_result = factory.make(key);
    SIMPLE_ASSERT(str_result.has_value(), "Should work with std::string");
...
TEST_CASE(empty_key) {
    ...
    SIMPLE_ASSERT(registered, "Empty key should be valid");
    
    auto result = factory.make("");
    SIMPLE_ASSERT(result.has_value(), "Should make with empty key");
...
void run_benchmarks() {
    ...
    out << "\n" << colors::cyan() << "Factory Benchmarks:" 
        << colors::reset() << "\n\n";
    ...
    // Basic make
    {
        ...
        double time = measure_perf([&factory] {
            auto result = factory.make("widget");
    ...
    // Registration
    {
        ...
        double time = measure_perf([&factory, &counter] {
    ...
    // Direct creation vs Factory
    {
        ...
        double factory_time = measure_perf([&factory] {
            auto result = factory.make("widget");
            DoNotOptimize(result);
        }, 1000000, 10000);
        out << "  Factory make: " << format_time(factory_time) << "\n";
        out << "  Overhead: " << std::fixed << std::setprecision(1) 
            << (factory_time / direct_time) << "x\n";
    }
    ...
    // Map vs UnorderedMap with 1000 items
    {
        ...
        for (int i = 0; i < 1000; ++i) {
            ...
            (void)map_factory.registerType(key, [i] { return Widget(i); });
            (void)unordered_factory.registerType(key, [i] { return Widget(i); });
        }
        
        out << "\nMap vs UnorderedMap (1000 items):\n";
        
        double map_time = measure_perf([&map_factory] {
            auto r = map_factory.make("widget500");
            DoNotOptimize(r);
        }, 100000, 1000);
        out << "  Map storage: " << format_time(map_time) << "\n";
        
        double unordered_time = measure_perf([&unordered_factory] {
    ...
    // Size impact
    {
        ...
        double small_time = measure_perf([&small_factory] {
            auto r = small_factory.make("widget5");
            DoNotOptimize(r);
        }, 100000, 1000);
        out << "  10 items: " << format_time(small_time) << "\n";
        
        double large_time = measure_perf([&large_factory] {
    ...
    // Stats policy overhead comparison
    {
        ...
        (void)atomic_factory.registerType("widget", []{ return Widget(42); });
        
        HPCFactory<std::string, Widget> no_stats_factory;
        (void)no_stats_factory.registerType("widget", []{ return Widget(42); });
        
        double atomic_time = measure_perf([&atomic_factory] {
            auto r = atomic_factory.make("widget");
            DoNotOptimize(r);
        }, 1000000, 10000);
        out << "  Atomic stats: " << format_time(atomic_time) << "\n";
        
        double no_stats_time = measure_perf([&no_stats_factory] {
            try {
