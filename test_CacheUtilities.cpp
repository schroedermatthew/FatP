...
bool test_cache_info() {
    ...
    // Sanity checks
    SIMPLE_ASSERT(CacheInfo::l1_line_size() >= 32, "Cache line should be at least 32 bytes");
    ...
    SIMPLE_ASSERT((line_size & (line_size - 1)) == 0, "Cache line size should be power of 2");
    
    // Destructive >= constructive (typically equal)
    SIMPLE_ASSERT(CacheInfo::destructive_interference_size() >= CacheInfo::constructive_interference_size(),
                  "Destructive interference should be >= constructive");
    
    // Verify constexpr values match function returns
    SIMPLE_ASSERT(cache_constants::l1_line_size_v == CacheInfo::l1_line_size(),
                  "Constexpr value should match function");
    SIMPLE_ASSERT(cache_constants::destructive_interference_size_v == CacheInfo::destructive_interference_size(),
                  "Constexpr value should match function");
...
bool test_prefetch_operations() {
    ...
    SIMPLE_ASSERT(sum == expected, "Data should be accessible after prefetch");
...
bool test_cache_flush() {
    ...
    SIMPLE_ASSERT(data[63] == 63, "Last element should be correct after flush");
...
bool test_stream_store_8byte() {
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        SIMPLE_ASSERT(data[i] == expected, "Stream store should work for 8-byte types");
    }
...
bool test_stream_store_unaligned() {
    ...
    SIMPLE_ASSERT(loaded_int == 42, "Unaligned stream_store should work via fallback");
    ...
    SIMPLE_ASSERT(loaded_double == 3.14, "Unaligned double stream_store should work");
...
bool test_stream_copy_unaligned() {
    ...
    for (int i = 1; i < 1023; ++i) {
        SIMPLE_ASSERT(dest[i] == src[i], "Unaligned stream copy should work via fallback");
    }
...
bool test_cache_aligned_array() {
    ...
    // Verify values
    for (int i = 0; i < 4; ++i) {
        SIMPLE_ASSERT(arr[i].get() == i * 10, "Array element should have correct value");
    }
    ...
    SIMPLE_ASSERT(spacing >= CacheInfo::destructive_interference_size(),
...
bool test_cache_line_padded_alignment() {
    ...
    SIMPLE_ASSERT(addr0 % CacheInfo::destructive_interference_size() == 0,
    ...
    SIMPLE_ASSERT(arr[0].get() == 111, "First element should be correct");
    SIMPLE_ASSERT(arr[1].get() == 222, "Second element should be correct");
...
bool test_cache_line_padded_various_sizes() {
    ...
    SIMPLE_ASSERT(sizeof(CacheLinePadded<char>) == CacheInfo::destructive_interference_size(),
                  "CacheLinePadded<char> should be cache line size");
    SIMPLE_ASSERT(c.get() == 42, "char value should be correct");
    
    CacheLinePadded<short> s(1000);
    SIMPLE_ASSERT(sizeof(CacheLinePadded<short>) == CacheInfo::destructive_interference_size(),
                  "CacheLinePadded<short> should be cache line size");
    SIMPLE_ASSERT(s.get() == 1000, "short value should be correct");
    
    CacheLinePadded<double> d(3.14159);
    SIMPLE_ASSERT(sizeof(CacheLinePadded<double>) == CacheInfo::destructive_interference_size(),
                  "CacheLinePadded<double> should be cache line size");
    SIMPLE_ASSERT(d.get() == 3.14159, "double value should be correct");
...
bool test_cache_line_padded_exact_size() {
    ...
    SIMPLE_ASSERT(sizeof(CacheLinePadded<ExactSize>) == CacheInfo::destructive_interference_size(),
...
bool test_optimal_block_size() {
    ...
    // Power of 2 check
    SIMPLE_ASSERT((block_size & (block_size - 1)) == 0, "Block size should be power of 2");
...
bool test_optimal_block_size_edge_cases() {
    ...
    SIMPLE_ASSERT(block >= 1, "Block size for 0 elements should be at least 1");
    
    // Zero element size
    block = optimal_block_size(1000, 0, 32768);
    SIMPLE_ASSERT(block >= 1, "Block size for 0-byte elements should be at least 1");
    
    // Huge elements that don't fit in cache
    block = optimal_block_size(1000, 1024 * 1024, 32768);
    SIMPLE_ASSERT(block >= 1, "Block size should be at least 1 even for huge elements");
...
bool test_block_iterator_2d() {
    ...
    while (iter.has_next()) {
        ...
        SIMPLE_ASSERT(ei <= 100, "Block end_i should not exceed rows");
        SIMPLE_ASSERT(ej <= 100, "Block end_j should not exceed cols");
    ...
    SIMPLE_ASSERT(block_count == 49, "Should have exactly 49 blocks for 100x100/16x16");
...
bool test_block_iterator_2d_non_divisible() {
    ...
    SIMPLE_ASSERT(block_count == 20, "Should have exactly 20 blocks for 50x70/16x16");
...
bool test_block_iterator_2d_zero_blocks() {
    ...
    SIMPLE_ASSERT(count1 < 1000, "Zero block_rows should be clamped, no infinite loop");
    SIMPLE_ASSERT(count1 == 20, "Should iterate correctly with clamped block_rows=1");
    ...
    SIMPLE_ASSERT(count2 < 1000, "Zero block_cols should be clamped, no infinite loop");
    SIMPLE_ASSERT(count2 == 20, "Should iterate correctly with clamped block_cols=1");
    
    // Zero dimensions should terminate immediately
    BlockIterator2D iter_zero_dim(0, 10, 5, 5);
    SIMPLE_ASSERT(!iter_zero_dim.has_next(), "Zero rows should have no blocks");
    
    BlockIterator2D iter_zero_cols_dim(10, 0, 5, 5);
    SIMPLE_ASSERT(!iter_zero_cols_dim.has_next(), "Zero cols should have no blocks");
...
bool test_alignment_utilities() {
    ...
    SIMPLE_ASSERT(is_aligned(buffer, 1), "Any pointer is 1-byte aligned");
    
    // Test unaligned
    char* unaligned = buffer + 1;
    SIMPLE_ASSERT(!is_aligned(unaligned, 64), "buffer+1 should not be 64-byte aligned");
    SIMPLE_ASSERT(!is_aligned(unaligned, 2), "buffer+1 should not be 2-byte aligned");
    
    // Test align_up
    void* aligned_up = align_up(unaligned, 64);
    SIMPLE_ASSERT(is_aligned(aligned_up, 64), "align_up should produce aligned pointer");
    SIMPLE_ASSERT(aligned_up >= unaligned, "align_up should not go backward");
    ...
    SIMPLE_ASSERT(aligned_down <= unaligned, "align_down should not go forward");
    ...
    SIMPLE_ASSERT(offset == 0, "Offset for aligned pointer should be 0");
...
bool test_alignment_utilities_const() {
    ...
    SIMPLE_ASSERT(is_aligned(aligned_up, 64), "const align_up should work");
    
    const void* aligned_down = align_down(unaligned, 64);
    SIMPLE_ASSERT(is_aligned(aligned_down, 64), "const align_down should work");
