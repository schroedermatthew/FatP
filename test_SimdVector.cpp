...
TEST_CASE(construction)
{
    ...
    for (size_t i = 0; i < SimdVectorF::width; ++i) {
        SIMPLE_ASSERT(loaded_result[i] == static_cast<float>(i), "Loaded vector should contain original data");
    }
...
TEST_CASE(fms)
{
    ...
    for (size_t i = 0; i < SimdVectorF::width; ++i) {
        SIMPLE_ASSERT(std::abs(result[i] - expected) < 1e-6f, 
    ...
    for (size_t i = 0; i < SimdVectorF::width; ++i) {
        SIMPLE_ASSERT(std::abs(result2[i] - expected2) < 1e-6f, 
...
TEST_CASE(double_precision)
{
    ...
    prod.store_aligned(result);
    for (size_t i = 0; i < SimdVectorD::width; ++i) {
        SIMPLE_ASSERT(std::abs(result[i] - 6.0) < 1e-12,
...
TEST_CASE(mask_operations)
{
    ...
    SIMPLE_ASSERT(mask_lt.all(), "2.0 < 3.0 should be true for all lanes");

    // a > b should be all false
    SIMPLE_ASSERT(mask_gt.none(), "2.0 > 3.0 should be false for all lanes");

    // a == a should be all true
    SIMPLE_ASSERT(mask_eq.all(), "a == a should be true for all lanes");

    // Test popcount
    SIMPLE_ASSERT(mask_lt.popcount() == SimdVectorF::width, "All-true mask popcount");
    SIMPLE_ASSERT(mask_gt.popcount() == 0, "All-false mask popcount");
...
TEST_CASE(select_blend)
{
    ...
    for (size_t i = 0; i < SimdVectorF::width; ++i) {
        SIMPLE_ASSERT(std::abs(result[i] - 2.0f) < 1e-6f,
...
TEST_CASE(partial_mask_select)
{
    ...
    SIMPLE_ASSERT(mask.popcount() == expected_true,
                  "partial_mask_select: mask popcount should match true lanes");

    // Sanity check: mask should be neither all() nor none()
    SIMPLE_ASSERT(!mask.all(), "partial_mask_select: mask should not be all()");
    SIMPLE_ASSERT(!mask.none(), "partial_mask_select: mask should not be none()");
...
TEST_CASE(nan_propagation)
{
    ...
    for (size_t i = 0; i < SimdVectorF::width; ++i) {
        if (i == nan_lane) {
            SIMPLE_ASSERT(std::isnan(sum_result[i]),
                          "nan_propagation: sum lane with NaN should be NaN");
            SIMPLE_ASSERT(std::isnan(prod_result[i]),
            ...
            SIMPLE_ASSERT(std::abs(sum_result[i] - expected_sum) < 1e-5f,
                          "nan_propagation: finite sum mismatch");
            SIMPLE_ASSERT(std::abs(prod_result[i] - expected_prod) < 1e-5f,
...
TEST_CASE(mask_not_operator)
{
    ...
    SIMPLE_ASSERT(mask.all(), "Original mask should be all true");
    SIMPLE_ASSERT(not_mask.none(), "NOT of all-true should be all-false");
    SIMPLE_ASSERT(not_mask.popcount() == 0, "NOT mask popcount should be 0");
    ...
    SIMPLE_ASSERT(original_count + flipped_count == SimdVectorF::width,
