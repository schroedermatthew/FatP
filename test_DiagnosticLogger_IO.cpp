...
bool test_file_sink_basic()
{
    ...
    
    {
        ...
        SIMPLE_ASSERT(sink != nullptr, "FileSink created successfully");
        SIMPLE_ASSERT(sink->is_valid(), "FileSink is valid");
        ...
        SIMPLE_ASSERT(!contents.empty(), "File has content");
        SIMPLE_ASSERT(contents.find("Test file sink message") != std::string::npos, "Message in file");
    }
...
bool test_file_sink_multiple_writes()
{
    ...
    
    {
        ...
        SIMPLE_ASSERT(lines == 10, "10 lines written");
    }
...
bool test_file_sink_append_mode()
{
    ...
    size_t lines = countLines(filename);
    SIMPLE_ASSERT(lines == 2, "Both messages appended");
...
bool test_file_sink_invalid_path()
{
    ...
    SIMPLE_ASSERT(sink == nullptr, "Invalid path returns nullptr");
...
bool test_ring_buffer_sink()
{
    ...
    
    {
        ...
        size_t lines = countLines(filename);
        SIMPLE_ASSERT(lines == 10, "All ring buffer messages dumped");
    }
...
bool test_ring_buffer_overflow()
{
    ...
    
    {
        ...
        size_t lines = countLines(filename);
        SIMPLE_ASSERT(lines == 1024, "Ring buffer capacity is 1024");
    }
...
bool test_rotating_file_sink_basic()
{
    ...
    
    {
        ...
        SIMPLE_ASSERT(sink->is_valid(), "RotatingFileSink is valid");
        ...
        SIMPLE_ASSERT(fs::exists(filename), "Base file exists");
    }
...
bool test_rotating_file_sink_rotation()
{
    ...
    
    {
        ...
        SIMPLE_ASSERT(rotated, "Files were rotated");
    }
...
bool test_resilient_sink_primary_works()
{
    ...
    
    {
        ...
        std::string contents = readFileContents(filename);
        SIMPLE_ASSERT(contents.find("Primary test") != std::string::npos, "Primary sink used");
    }
...
bool test_async_sink()
{
    ...
    
    {
        ...
        size_t lines = countLines(filename);
        SIMPLE_ASSERT(lines == 100, "All async messages written");
        SIMPLE_ASSERT(asyncSink->dropped() == 0, "No messages dropped");
    }
...
bool test_async_sink_high_load()
{
    ...
    
    {
        ...
        size_t lines = countLines(filename);
        SIMPLE_ASSERT(lines >= 400, "Most messages written under load");
    }
...
bool test_rate_limiting_sink()
{
    ...
    
    {
        ...
        size_t lines = countLines(filename);
        SIMPLE_ASSERT(lines < 100, "Some messages were rate limited");
        SIMPLE_ASSERT(rateLimitSink->dropped() > 0, "Some messages dropped");
    }
...
bool test_rate_limiting_burst()
{
    ...
    
    {
        ...
        size_t lines = countLines(filename);
        SIMPLE_ASSERT(lines >= 10, "Burst allows initial messages");
    }
...
bool test_filtering_sink()
{
    ...
    
    {
        ...
        size_t lines = countLines(filename);
        SIMPLE_ASSERT(lines == 2, "Only Warning and Error logged");
        
        std::string contents = readFileContents(filename);
        SIMPLE_ASSERT(contents.find("Warning") != std::string::npos, "Warning present");
        SIMPLE_ASSERT(contents.find("Error") != std::string::npos, "Error present");
        SIMPLE_ASSERT(contents.find("Trace") == std::string::npos, "Trace filtered");
        SIMPLE_ASSERT(contents.find("Debug") == std::string::npos, "Debug filtered");
        SIMPLE_ASSERT(contents.find("Info") == std::string::npos, "Info filtered");
    }
...
bool test_filtering_sink_custom_filter()
{
    ...
    
    {
        ...
        size_t lines = countLines(filename);
        SIMPLE_ASSERT(lines == 2, "Only important messages logged");
    }
...
bool test_initialize_rotating_logger()
{
    ...
    SIMPLE_ASSERT(fs::exists(filename), "Rotating log file created");
...
bool test_combined_sinks()
{
    ...
    
    {
        ...
        SIMPLE_ASSERT(fs::exists(file1), "First file created");
        SIMPLE_ASSERT(fs::exists(file2), "Second file created");
        
        std::string contents1 = readFileContents(file1);
        std::string contents2 = readFileContents(file2);
        
        SIMPLE_ASSERT(contents1.find("Combined sink test") != std::string::npos, "Message in file1");
        SIMPLE_ASSERT(contents2.find("Combined sink test") != std::string::npos, "Message in file2");
...
bool test_async_sink_flush_consistency()
{
...

    {
        ...
        size_t lines = countLines(filename);
        SIMPLE_ASSERT(lines == 50, "Flush ensured exactly 50 lines were written");
    }
...
bool test_async_with_filtering()
{
    ...
    
    {
        ...
        size_t lines = countLines(filename);
        SIMPLE_ASSERT(lines == 10, "Only errors logged through combined async+filter");
    }
...
bool test_rotating_file_sink_scope_guard_safety()
{
    ...
    
    {
        ...
        SIMPLE_ASSERT(sink != nullptr, "RotatingFileSink created");
        SIMPLE_ASSERT(sink->is_valid(), "RotatingFileSink is valid initially");
        ...
        SIMPLE_ASSERT(sink->is_valid(), "Sink still valid after rotation");
        ...
        std::string contents = readFileContents(filename);
        SIMPLE_ASSERT(contents.find("Post-rotation test") != std::string::npos, "Post-rotation write succeeded");
    }
...
bool test_log_level_guard_restoration()
{
    ...
    SIMPLE_ASSERT(logger.getLevel() == original, "Level restored after guard destruction");
    
    {
        LogLevelTestGuard guard(logger, LogLevel::Fatal);
        SIMPLE_ASSERT(logger.getLevel() == LogLevel::Fatal, "Level changed to Fatal");
    }
    
    SIMPLE_ASSERT(logger.getLevel() == original, "Level restored after second guard");
...
bool test_resilient_sink_scope_guard_state_management()
{
    ...
    
    {
        ...
        SIMPLE_ASSERT(primary->writeCount == 1, "Primary attempted once");
        
        LogRecord record2(LogLevel::Info, "Second message", loc);
        resilient->write(record2);
        
        SIMPLE_ASSERT(primary->writeCount == 1, "Primary not attempted after failure");
        
        resilient->flush();
        
        std::string contents = readFileContents(fallbackFile);
        SIMPLE_ASSERT(contents.find("First message") != std::string::npos, "Fallback has first message");
        SIMPLE_ASSERT(contents.find("Second message") != std::string::npos, "Fallback has second message");
    }
...
bool test_rotating_file_sink_multiple_rotations_with_guard()
{
    ...
    
    {
        ...
        SIMPLE_ASSERT(sink->is_valid(), "Sink valid after multiple rotations");
        ...
        std::string contents = readFileContents(filename);
        SIMPLE_ASSERT(contents.find("Final message") != std::string::npos, "Final message written successfully");
    }
