...
TEST_CASE(malformed_literals)
{
    ...
    SIMPLE_ASSERT(!r3.has_value(), "capitalized Null must fail");
    
    auto r4 = json_decode_from<int>("NULL");
    SIMPLE_ASSERT(!r4.has_value(), "uppercase NULL must fail");
    return true;
}
...
TEST_CASE(fuzz_ints)
{
    ...
    for (int i = 0; i < 2000; ++i)
    {
        ...
        SIMPLE_ASSERT(*r == v, "Fuzz int mismatch");
    }
...
TEST_CASE(fuzz_int64)
{
    ...
    for (int i = 0; i < 2000; ++i)
    {
        ...
        auto r = fuzz_roundtrip(v);
        SIMPLE_ASSERT(r.has_value(), r.error().message.c_str());
        SIMPLE_ASSERT(*r == v, "Fuzz int64 mismatch");
    }
...
TEST_CASE(fuzz_doubles)
{
    ...
    for (int i = 0; i < 2000; ++i)
    {
        ...
        auto r = fuzz_roundtrip(v);
        SIMPLE_ASSERT(r.has_value(), r.error().message.c_str());
        SIMPLE_ASSERT(std::fabs(*r - v) < 1e-9, "Fuzz double mismatch");
    }
...
TEST_CASE(fuzz_strings)
{
    ...
    for (int i = 0; i < 2000; ++i)
    {
        ...
        auto r = fuzz_roundtrip(v);
        SIMPLE_ASSERT(r.has_value(), r.error().message.c_str());
        SIMPLE_ASSERT(*r == v, "Fuzz string mismatch");
    }
...
TEST_CASE(fuzz_vector_int)
{
    ...
    for (int iter = 0; iter < 500; ++iter)
    {
        ...
        auto r = fuzz_roundtrip(v);
        SIMPLE_ASSERT(r.has_value(), r.error().message.c_str());
        SIMPLE_ASSERT(*r == v, "Fuzz vector<int> mismatch");
    }
...
TEST_CASE(fuzz_map_string_int)
{
    ...
    for (int iter = 0; iter < 300; ++iter)
    {
        ...
        SIMPLE_ASSERT(r.has_value(), r.error().message.c_str());
        SIMPLE_ASSERT(*r == m, "Fuzz map<string,int> mismatch");
    }
...
TEST_CASE(fuzz_nested_structures)
{
    ...
    for (int iter = 0; iter < 200; ++iter)
    {
        ...
        auto r = fuzz_roundtrip(v);
        SIMPLE_ASSERT(r.has_value(), r.error().message.c_str());
        SIMPLE_ASSERT(*r == v, "Fuzz nested vector<map<string,int>> mismatch");
    }
