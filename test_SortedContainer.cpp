...
TEST_CASE(fuzzy_unique_policy)
{
    ...
    SIMPLE_ASSERT(fat_p::is_fuzzy_unique_policy<fat_p::FuzzyUniquePolicy<>>::value,
               "Trait should detect FuzzyUniquePolicy");
    SIMPLE_ASSERT(!fat_p::is_fuzzy_unique_policy<fat_p::OnlyUniquePolicy>::value,
    ...
    SIMPLE_ASSERT(result3.has_value() && result3.value(), "Non-duplicate should succeed");
    ...
    SIMPLE_ASSERT(result4.has_value() && !result4.value(), 
...
TEST_CASE(fuzzy_chain_handling)
{
    ...
    SIMPLE_ASSERT(vec.size() >= 1, "Should keep at least one element");
    SIMPLE_ASSERT(vec.size() <= 3, "Should collapse some elements in chain");
    
    for (size_t i = 0; i + 1 < vec.size(); ++i)
    {
        double diff = std::abs(vec[i+1] - vec[i]);
        SIMPLE_ASSERT(diff >= chain_eps,
                   "Consecutive elements must be >= epsilon apart after dedup");
    }
    
    SIMPLE_ASSERT(std::abs(vec[0] - 1.0) < 1e-10, "First element should be 1.0");
...
TEST_CASE(fuzzy_default_epsilon)
{
    ...
    SIMPLE_ASSERT(result2.has_value() && !result2.value(), 
               "Very close value should be rejected with default epsilon");
    
    auto result3 = sv.insert(2.0);
    SIMPLE_ASSERT(result3.has_value() && result3.value(), "Different value should be accepted");
    
    SIMPLE_ASSERT(sv.size() == 2, "Should have 2 elements with default epsilon");
    
    FuzzySV sv2;
    std::vector<double> values = {1.0, 1.0 + 1e-15, 2.0, 2.0 + 1e-15};
    auto range_result = sv2.insertRange(values.begin(), values.end());
    SIMPLE_ASSERT(range_result.has_value(), "insertRange should succeed");
    SIMPLE_ASSERT(sv2.size() == 2, "Should have 2 elements after insertRange with default epsilon");
...
TEST_CASE(logging_unique_policy_forwarding)
{
    ...
    SIMPLE_ASSERT(TestObject::move_count.load() > initial_moves, 
...
TEST_CASE(unique_equivalence)
{
    ...
    auto vec = sv.toVector();
    SIMPLE_ASSERT(vec.size() == 5, "Should have 5 unique elements");
    
    std::vector<int> expected = {1, 2, 3, 4, 5};
    SIMPLE_ASSERT(vec == expected, "Should match expected unique sequence");
...
TEST_CASE(stable_sort_large_batch)
{
    ...
    SIMPLE_ASSERT(std::is_sorted(vec.begin(), vec.end()), "Should be sorted after large batch");
...
TEST_CASE(reserve_and_capacity)
{
    ...
    SIMPLE_ASSERT(cap_before >= 100, "Capacity should persist after inserts");
...
TEST_CASE(clear_method)
{
    ...
    SIMPLE_ASSERT(sv.size() == 3, "Should have 3 elements before clear");
...
TEST_CASE(basic_insert_and_find)
{
    ...
    auto vec = sv.toVector();
    SIMPLE_ASSERT(std::is_sorted(vec.begin(), vec.end()), "Container should maintain sorted order");
    
    std::vector<int> expected = {1, 3, 5, 7, 9};
    SIMPLE_ASSERT(vec == expected, "Elements should be in sorted order");
    
    auto it = sv.find(5);
    SIMPLE_ASSERT(it != sv.end(), "Should find existing element");
    SIMPLE_ASSERT(*it == 5, "Found element should be correct");
    
    auto it_not_found = sv.find(6);
    SIMPLE_ASSERT(it_not_found == sv.end(), "Should not find non-existing element");
...
TEST_CASE(only_unique_policy_basic)
{
    ...
    auto r2 = sv.insert(5);
    SIMPLE_ASSERT(r2.has_value() && !r2.value(), "Duplicate insert should be rejected");
    
    auto r3 = sv.insert(3);
    SIMPLE_ASSERT(r3.has_value() && r3.value(), "Different value should succeed");
    
    SIMPLE_ASSERT(sv.size() == 2, "Should have 2 unique elements");
...
TEST_CASE(range_constructor)
{
    ...
    SIMPLE_ASSERT(sv.size() == 5, "Should contain all source elements");
...
TEST_CASE(binary_search_methods)
{
    ...
    SIMPLE_ASSERT(lower != sv.end() && *lower == 3, "lower_bound should find first 3");
    
    auto upper = sv.upper_bound(3);
    SIMPLE_ASSERT(upper != sv.end() && *upper == 5, "upper_bound should find element after 3s");
    
    SIMPLE_ASSERT(sv.count(3) == 2, "Should have 2 occurrences of 3");
    SIMPLE_ASSERT(sv.count(4) == 0, "Should have 0 occurrences of 4");
...
TEST_CASE(erase_basic)
{
    ...
    SIMPLE_ASSERT(result.has_value() && result.value(), "Erase should succeed for existing element");
    SIMPLE_ASSERT(sv.size() == 2, "Size should decrease after erase");
    SIMPLE_ASSERT(sv.find(3) == sv.end(), "Element should no longer exist");
...
TEST_CASE(iterators)
{
    ...
    SIMPLE_ASSERT(forward_order == expected_forward, "Forward iteration should work");
    ...
    SIMPLE_ASSERT(reverse_order == expected_reverse, "Reverse iteration should work");
...
TEST_CASE(concurrent_inserts)
{
    ...
    SIMPLE_ASSERT(success_count.load() == expected_total, 
               "All inserts should succeed under concurrency");
    SIMPLE_ASSERT(sv.size() == static_cast<size_t>(expected_total), 
               "Size should match insert count");
    
    auto vec = sv.toVector();
    SIMPLE_ASSERT(std::is_sorted(vec.begin(), vec.end()), 
...
TEST_CASE(large_dataset)
{
    ...
    SIMPLE_ASSERT(sv.size() == TEST_RANGE_SIZE, "Should contain all elements");
...
TEST_CASE(deque_backend_policy)
{
    ...
    SIMPLE_ASSERT(sv.size() == 3, "Deque backend should work");
    
    auto cap = sv.capacity();
    SIMPLE_ASSERT(cap == sv.size(), "Deque capacity() returns size()");
