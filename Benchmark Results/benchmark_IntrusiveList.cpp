...
FATP_META:
  ...
  file_role: benchmark
  path: benchmarks/benchmark_IntrusiveList.cpp
  namespace: fat_p
...
struct FatPNode : public fat_p::IntrusiveListNode<FatPNode>
{
    ...
    explicit FatPNode(int64_t v = 0) : value(v) {}
};
...
struct StdNode
{
    ...
    explicit StdNode(int64_t v = 0) : value(v) {}
};
...
struct BoostNode : public boost::intrusive::list_base_hook<>
{
    ...
    explicit BoostNode(int64_t v = 0) : value(v) {}
};
#endif
...
struct EastlNode : public eastl::intrusive_list_node
{
    ...
    explicit EastlNode(int64_t v = 0) : value(v) {}
};
#endif
...
struct EtlNode : public etl::bidirectional_link<0>
{
    ...
    explicit EtlNode(int64_t v = 0) : value(v) {}
};

// ETL's remove() requires operator== (non-member for MSVC compatibility)
inline bool operator==(const EtlNode& lhs, const EtlNode& rhs) { return &lhs == &rhs; }
inline bool operator!=(const EtlNode& lhs, const EtlNode& rhs) { return !(lhs == rhs); }
#endif
...
struct LlvmNode : public llvm::ilist_node<LlvmNode>
{
    ...
    explicit LlvmNode(int64_t v = 0) : value(v) {}
};
#endif
...
struct IListAdapter
{
    ...
    void record_time(double ns_per_op) { times.push_back(ns_per_op); }
    Stats get_stats() const { return compute_stats(times); }
    void clear_times() { times.clear(); }
};
...
public:
    const char* name() const override { return "fat_p::IntrusiveList"; }
    ...
    size_t node_sizeof() const override { return sizeof(FatPNode); }
...
public:
    const char* name() const override { return "std::list<T*>"; }
    ...
    size_t size() const override { return list_.size(); }
    size_t node_sizeof() const override { return sizeof(StdNode) + 2 * sizeof(void*); }  // + list node overhead

    void setup_is_linked() override
    {
        // std::list has no is_linked() - nothing to setup
    }

    size_t count_linked() override
    {
        // std::list has no is_linked() - would require O(N) search per node
        return 0;
    }
};
...
public:
    const char* name() const override { return "boost::intrusive::list"; }
    ...
    size_t size() const override { return list_.size(); }
    size_t node_sizeof() const override { return sizeof(BoostNode); }
...
public:
    const char* name() const override { return "eastl::intrusive_list"; }
    ...
    size_t size() const override { return list_.size(); }
    size_t node_sizeof() const override { return sizeof(EastlNode); }
...
public:
    const char* name() const override { return "etl::intrusive_list [!]"; }  // [!] = has limitations
    ...
    size_t size() const override { return list_.size(); }
    size_t node_sizeof() const override { return sizeof(EtlNode); }
...
public:
    const char* name() const override { return "llvm::simple_ilist"; }
    ...
    size_t size() const override { return list_.size(); }
    size_t node_sizeof() const override { return sizeof(LlvmNode); }
    ...
    size_t count_linked() override
    {
        // LLVM: ilist_node_base has Prev/Next pointers
        size_t count = 0;
        for (const auto& node : nodes_)
        {
...
void benchmark_is_linked(std::vector<std::unique_ptr<IListAdapter>>& adapters, size_t N)
{
    ...
    print_cpu_context();
    print_contract("O(1) membership check - all intrusive lists can check via link pointers");

    std::cout << "Measuring time to check is_linked() for " << N << " nodes (half linked).\n";
    std::cout << "All intrusive lists: O(1) check via link/owner pointers\n";
    std::cout << "std::list: would require O(N) search - skipped\n\n";
    ...
    for (auto& adapter : adapters)
    {
        // Skip std::list - it would require O(N) search per node
        if (std::string(adapter->name()).find("std::list") != std::string::npos)
        {
            std::cout << std::setw(30) << std::left << adapter->name() 
                      << "  (skipped - would require O(N) search per node)\n";
            continue;
        }
        auto stats = adapter->get_stats();
        print_result_row(adapter->name(), stats);
    ...
    std::cout << "\nNote: All intrusive lists have O(1) is_linked check.\n";
    std::cout << "fat_p unique advantage: owner pointer identifies WHICH list owns the node.\n";
    std::cout << "Other intrusive lists only know IF linked, not to which specific list.\n";
...
int main(int argc, char* argv[])
{
    ...
    benchmark_is_linked(adapters, 100000);
