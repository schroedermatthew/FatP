...
namespace cpp_utilities::testing
{
    ...
    // FIX #29: Save benchmark results
    void save_benchmark_result(const std::string& name, double mean_ns, double stddev_ns) {
        static bool first_run = true;
        std::ofstream out("enforced_init_benchmarks.csv", std::ios::app);
        
        if (first_run) {
            out << "Timestamp,Benchmark,Mean(ns),StdDev(ns)\n";
            first_run = false;
        }
    ...
    void run_benchmarks() {
        ...
        // Benchmark 1: Basic initialization overhead - FIX #7
        {
            ...
            save_benchmark_result("init_get_overhead", enforced_time * 1000, 0);
        }
        ...
        // Benchmark 2: Lazy initialization - FIX #12: Isolated overhead
        {
            ...
            save_benchmark_result("lazy_init", lazy_time * 1000, 0);
        }
