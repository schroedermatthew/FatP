...
**What happens outside a function's domain?** The value and every partial become NaN and propagate, so a NaN value is never paired with a finite-looking gradient. At a boundary where the derivative is unbounded the value is infinite and so is the partial (`log` at 0). Either way, read `mValue` first.
