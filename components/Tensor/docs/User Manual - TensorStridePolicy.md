...
- Zero-overhead iteration for contiguous data
...
Stride1DPolicy compiles to the same assembly as a manual stride loop:
...
Most tensor iteration is 2D row-major. Stride2DPolicy handles this with near-zero overhead.
...
| Implementation | 1M elements (ns/elem) | Overhead |
|----------------|----------------------|----------|
| Manual nested loop | 0.9 | baseline |
| Stride2DPolicy | 1.0 | ~10% |
| TensorStridePolicy | 1.5-2.8 | 70-200% |

Stride2DPolicy is the sweet spot for 2D iteration.
...
flowchart TB
    ...
    P3 --> Q2{"Layout?"}
    Q2 -->|"Contiguous"| C1["~70% overhead"]
    Q2 -->|"Non-contiguous"| C2["~200% overhead"]
```

### Performance Hierarchy

| Policy | Overhead vs Manual | Use Case |
|--------|-------------------|----------|
| Stride1DPolicy | ~0% | 1D strided access |
| Stride2DPolicy | ~10% | 2D row-major, pitched |
| TensorStridePolicy (contiguous) | ~70% | General N-D, row-major |
| TensorStridePolicy (strided) | ~200% | Padded, transposed, complex |
...
| Small stride (2-4) | 80-90% |
| Large stride (>16) | 20-50% |
| Random | 5-10% |
