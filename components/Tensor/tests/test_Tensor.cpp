...
FATP_TEST_CASE(enhanced_bounds_checking)
{
    ...
    catch (const std::out_of_range& e)
    {
        ...
        FATP_ASSERT_TRUE(msg.find("dimension") != std::string::npos || msg.find("out of range") != std::string::npos,
    ...
    catch (const std::out_of_range& e)
    {
        ...
        std::string msg = e.what();
        FATP_ASSERT_TRUE(msg.find("mismatch") != std::string::npos, "Should mention dimension mismatch");
    }
...
bool test_v51_zero_overhead_release()
{
    ...
    [[maybe_unused]] auto tracker = tensor.create_tracker();
