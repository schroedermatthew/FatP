...
FATP_META:
  ...
  file_role: test
  path: components/SortedContainer/tests/test_SortedContainer.cpp
  layer: Testing
  namespace: fat_p
...
{
    using FuzzySV = fat_p::SortedContainer<double, fat_p::FuzzyUniquePolicy<fat_p::HybridComparisonPolicy, double>>;
    FuzzySV sv;
    ...
    FATP_ASSERT_TRUE(result4.has_value() && !result4.value(), "Fuzzy duplicate before should be rejected");
...
FATP_TEST_CASE(fuzzy_insert_range)
{
    ...
    {
        FATP_ASSERT_GT(std::abs(vec[i + 1] - vec[i]), FUZZY_EPSILON, "No two elements should be within epsilon");
    }
...
FATP_TEST_CASE(fuzzy_chain_handling)
{
    ...
    for (size_t i = 0; i + 1 < vec.size(); ++i)
    {
        ...
        FATP_ASSERT_GE(diff, chain_eps, "Consecutive elements must be >= epsilon apart after dedup");
    }
...
FATP_TEST_CASE(logging_unique_policy_forwarding)
{
    ...
    using LoggingSV = fat_p::SortedContainer<TestObject, fat_p::LoggingUniquePolicy<fat_p::OnlyUniquePolicy>>;
    LoggingSV sv;
    ...
    FATP_ASSERT_GT(TestObject::move_count.load(), initial_moves, "Move insert should trigger move operations");
...
FATP_TEST_CASE(erase_basic)
{
    ...
    FATP_ASSERT_TRUE(result2.has_value() && !result2.value(), "Erase should return false for non-existing element");
...
{
    fat_p::SortedContainer<int,
                           fat_p::AllowDuplicatesPolicy,
                           std::less<int>,
                           std::allocator<int>,
    ...
    FATP_ASSERT_EQ(success_count.load(), expected_total, "All inserts should succeed under concurrency");
    FATP_ASSERT_EQ(sv.size(), static_cast<size_t>(expected_total), "Size should match insert count");

    auto vec = sv.toVector();
    FATP_ASSERT_TRUE(std::is_sorted(vec.begin(), vec.end()), "Should remain sorted after concurrent inserts");
