...
- SIMD-accelerated vector operations (AVX2, SSE2, NEON)
...
Vector operations automatically use the best available SIMD instruction set:

| Platform | Instruction Set | int32 per op | int64 per op | double per op |
|----------|-----------------|--------------|--------------|---------------|
| x86-64 (modern) | AVX2 | 8 | 4 | 4 |
| x86-64 (fallback) | SSE2 | 4 | 2 | 2 |
| ARM64 (AArch64) | NEON | 4 | 2 | 2 |
| Other | Scalar | 1 | 1 | 1 |
...
// On AVX2: Uses _mm256_add_epi32 (8 ints per instruction)
// On SSE2: Uses _mm_add_epi32 (4 ints per instruction)
// On NEON: Uses vaddq_s32 (4 ints per instruction)
// On other: Scalar loop
```
...
// Check which SIMD is available
#ifdef FATP_INT_SIMD_AVX2
    // AVX2 path available
...
**Overflow detection techniques by architecture:**

| Architecture | Integer Overflow Detection | FP Overflow Detection |
|--------------|---------------------------|----------------------|
| AVX2/SSE2 | XOR sign-bit check: `(a^r) & (b^r)` | Unordered compare for NaN |
| NEON | Differential saturation (wrapping vs saturating) | vcgeq_f64 for Inf |

**SaturatingPolicy bonus on NEON:** When using `SaturatingPolicy` on ARM, the library leverages hardware saturation instructions (`vqaddq_s32`, `vqsubq_s32`) for zero-overhead saturating arithmetic.
...
- **SIMD-accelerated vector operations** (AVX2, SSE2, NEON): Up to 8 int32 or 4 double per instruction
...
CheckedArithmetic is organized into multiple headers for modularity:

| Header | Purpose |
|--------|---------|
| `CheckedArithmetic.h` | Main include (pulls in all functionality) |
| `CheckedArithmeticBase.h` | Core types, MathError enum, policy traits |
| `CheckedArithmeticPolicies.h` | Policy definitions |
| `CheckedArithmeticInt.h` | Integer scalar operations |
| `CheckedArithmeticFP.h` | Floating-point operations |
| `CheckedArithmetic_IntSimd.h` | SIMD dispatch layer |
| `CheckedArithmetic_IntSimd_AVX2.h` | AVX2 implementation |
| `CheckedArithmetic_IntSimd_SSE2.h` | SSE2 implementation |
| `CheckedArithmetic_IntSimd_NEON.h` | NEON implementation |
| `CheckedArithmetic_IntSimd_Common.h` | Shared SIMD utilities |

For most users, including `CheckedArithmetic.h` is sufficient.
...
| `enforce.h` | Contract enforcement in debug builds |
| `SimdVector.h` | Portable SIMD abstraction for float/double (CheckedArithmetic handles integer SIMD) |
| `AlignedVector.h` | Aligned storage for SIMD vector operations |
| `HpcVector.h` | NUMA-aware + aligned storage for HPC |

---

*CheckedArithmetic.h and related headers — Fat-P Library*
