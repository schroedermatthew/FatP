...
struct Stats { double median = 0, mean = 0, stddev = 0; };
...
void print_result_header() {
    std::cout << std::setw(28) << "Library" << std::setw(14) << "Median(ns)" << std::setw(14) << "Mean(ns)" << std::setw(12) << "Stddev\n";
    std::cout << std::string(68, '-') << "\n";
}

void print_result(const std::string& name, const Stats& s) {
    std::cout << std::fixed << std::setprecision(2) << std::setw(28) << name << std::setw(14) << s.median << std::setw(14) << s.mean << std::setw(12) << s.stddev << "\n";
}
...
std::vector<size_t> generate_indices(size_t n, size_t max_index, uint64_t seed) {
    ...
    for (size_t i = 0; i < n; ++i) indices[i] = dist(rng);
    return indices;
}
...
void benchmark_single_bit_operations(size_t iterations) {
    ...
    // Fat-P BitSet
    {
        ...
        for (size_t w = 0; w < WARMUP_RUNS(); ++w) {
            fat_p::BitSet<N> bits;
            for (size_t i = 0; i < iterations; ++i) bits.set_unchecked(indices[i]);
    ...
    // std::bitset
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            std::bitset<N> bits;
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) bits.set(indices[i]);
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += bits.count();
        }
...
#if HAS_LLVM
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            ...
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) bits.set(static_cast<unsigned>(indices[i]));
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += bits.count();
        }
...
#if HAS_CROARING
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            ...
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) bits.add(static_cast<uint32_t>(indices[i]));
            samples.push_back(t.elapsed_ns() / iterations);
...
void benchmark_population_count(size_t iterations) {
    ...
    for (size_t idx : sparse) { fatp_bits.set_unchecked(idx); std_bits.set(idx); }

#if HAS_BOOST
    boost::dynamic_bitset<> boost_bits(N);
    for (size_t idx : sparse) boost_bits.set(idx);
#endif
#if HAS_LLVM
    llvm::BitVector llvm_bits(static_cast<unsigned>(N));
    for (size_t idx : sparse) llvm_bits.set(static_cast<unsigned>(idx));
#endif
#if HAS_CROARING
    roaring::Roaring roaring_bits;
    for (size_t idx : sparse) roaring_bits.add(static_cast<uint32_t>(idx));
#endif
#if HAS_BITMAGIC
    bm::bvector<> bm_bits;
    for (size_t idx : sparse) bm_bits.set(static_cast<bm::bvector<>::size_type>(idx));
#endif
...

    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) sum += fatp_bits.count();
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += sum;
        }
        print_result("fat_p::BitSet", compute_stats(samples));
...

    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) sum += std_bits.count();
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += sum;
        }
        print_result("std::bitset", compute_stats(samples));
...
#if HAS_BOOST
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) sum += boost_bits.count();
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += sum;
        }
        print_result("boost::dynamic_bitset", compute_stats(samples));
...
#if HAS_LLVM
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) sum += llvm_bits.count();
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += sum;
        }
        print_result("llvm::BitVector", compute_stats(samples));
...
#if HAS_CROARING
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) sum += roaring_bits.cardinality();
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += sum;
        }
...
#if HAS_BITMAGIC
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) sum += bm_bits.count();
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += sum;
        }
        print_result("bm::bvector<>", compute_stats(samples));
...
void benchmark_find_operations(size_t num_set_bits) {
...

    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iters; ++i) sum += fatp_bits.find_first();
...

    {
        auto find_first_std = [](const std::bitset<N>& bits) -> size_t {
            for (size_t i = 0; i < N; ++i) if (bits.test(i)) return i;
            return N;
        };
        std::vector<double> samples;
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iters; ++i) sum += find_first_std(std_bits);
            samples.push_back(t.elapsed_ns() / iters);
            benchmark_sink += sum;
        }
...
#if HAS_BOOST
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iters; ++i) sum += boost_bits.find_first();
            samples.push_back(t.elapsed_ns() / iters);
            benchmark_sink += sum;
        }
        print_result("boost::dynamic_bitset", compute_stats(samples));
...
#if HAS_CROARING
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iters; ++i) sum += roaring_bits.minimum();
            samples.push_back(t.elapsed_ns() / iters);
            benchmark_sink += sum;
        }
        print_result("roaring::Roaring", compute_stats(samples));
...

    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            ...
            Timer t; t.start();
            for (size_t i = 0; i < iter_iters; ++i) {
                for (size_t j = 0; j < N; ++j) if (std_bits.test(j)) sum += j;
            }
            samples.push_back(t.elapsed_ns() / iter_iters);
            benchmark_sink += sum;
        }
...
#if HAS_BOOST
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            ...
            Timer t; t.start();
            for (size_t i = 0; i < iter_iters; ++i) {
                for (size_t pos = boost_bits.find_first(); pos != boost::dynamic_bitset<>::npos; pos = boost_bits.find_next(pos))
                    sum += pos;
            }
            samples.push_back(t.elapsed_ns() / iter_iters);
            benchmark_sink += sum;
...
#if HAS_CROARING
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            ...
            Timer t; t.start();
            for (size_t i = 0; i < iter_iters; ++i) {
                for (auto val : roaring_bits) sum += val;
            }
            samples.push_back(t.elapsed_ns() / iter_iters);
            benchmark_sink += sum;
        }
...
#if HAS_BITMAGIC
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            ...
            Timer t; t.start();
            for (size_t i = 0; i < iter_iters; ++i) {
                for (auto it = bm_bits.first(); it != bm_bits.end(); ++it) sum += *it;
            }
            samples.push_back(t.elapsed_ns() / iter_iters);
            benchmark_sink += sum;
        }
...
void benchmark_sparse_iteration(size_t num_set_bits) {
...

    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iters; ++i) for (size_t idx : fatp_bits) sum += idx;
            samples.push_back(t.elapsed_ns() / iters);
            benchmark_sink += sum;
        }
...

    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iters; ++i) for (size_t j = 0; j < N; ++j) if (std_bits.test(j)) sum += j;
            samples.push_back(t.elapsed_ns() / iters);
            benchmark_sink += sum;
        }
        print_result("std::bitset (O(N) scan)", compute_stats(samples));
...
#if HAS_CROARING
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iters; ++i) for (auto val : roaring_bits) sum += val;
            samples.push_back(t.elapsed_ns() / iters);
            benchmark_sink += sum;
        }
        print_result("roaring::Roaring", compute_stats(samples));
...
#if HAS_BITMAGIC
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iters; ++i) for (auto it = bm_bits.first(); it != bm_bits.end(); ++it) sum += *it;
            samples.push_back(t.elapsed_ns() / iters);
            benchmark_sink += sum;
        }
        print_result("bm::bvector<>", compute_stats(samples));
...
void benchmark_bitwise_operations(size_t iterations) {
    ...
    for (size_t idx : indices1) { fatp_a.set_unchecked(idx); std_a.set(idx); }
    for (size_t idx : indices2) { fatp_b.set_unchecked(idx); std_b.set(idx); }

#if HAS_CROARING
    roaring::Roaring roaring_a, roaring_b;
    for (size_t idx : indices1) roaring_a.add(static_cast<uint32_t>(idx));
    for (size_t idx : indices2) roaring_b.add(static_cast<uint32_t>(idx));
#endif
#if HAS_BITMAGIC
    bm::bvector<> bm_a, bm_b;
    for (size_t idx : indices1) bm_a.set(idx);
    for (size_t idx : indices2) bm_b.set(idx);
#endif
...

    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) { auto result = fatp_a & fatp_b; sum += result.count(); }
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += sum;
        }
        print_result("fat_p::BitSet", compute_stats(samples));
...

    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) { auto result = std_a & std_b; sum += result.count(); }
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += sum;
        }
        print_result("std::bitset", compute_stats(samples));
...
#if HAS_CROARING
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) { auto result = roaring_a & roaring_b; sum += result.cardinality(); }
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += sum;
        }
        print_result("roaring::Roaring", compute_stats(samples));
...
#if HAS_BITMAGIC
    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            size_t sum = 0;
            Timer t; t.start();
            for (size_t i = 0; i < iterations; ++i) { bm::bvector<> result = bm_a & bm_b; sum += result.count(); }
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += sum;
        }
        print_result("bm::bvector<>", compute_stats(samples));
...
void benchmark_range_operations(size_t range_size, size_t iterations) {
...

    {
        ...
        for (size_t r = 0; r < MEASURED_RUNS(); ++r) {
            ...
            for (size_t i = 0; i < iterations; ++i) {
                std::bitset<N> bits;
                for (size_t j = 0; j < range_size; ++j) bits.set(j);
                sum += bits.count();
            }
            samples.push_back(t.elapsed_ns() / iterations);
            benchmark_sink += sum;
...
#if HAS_BOOST
    std::cout << std::setw(30) << "boost::dynamic_bitset (base)" << " | " << std::setw(12) << sizeof(boost::dynamic_bitset<>) << "\n";
#endif
#if HAS_LLVM
