...
FATP_META:
  ...
  file_role: test
  path: components/DiagnosticLogger/tests/test_DiagnosticLogger_ScopeGuard.cpp
  layer: Testing
...
FATP_TEST_CASE(rotating_file_guaranteed_reopen)
{
    ...
    FATP_ASSERT_TRUE(contents.find("Post-rotation") != std::string::npos, "Post-rotation write succeeded");
...
FATP_TEST_CASE(rotating_file_multiple_rotations_stability)
{
    ...
    std::string contents = readFileContents(filename);
    FATP_ASSERT_TRUE(contents.find("FINAL_MESSAGE") != std::string::npos, "Final message written successfully");
...
FATP_TEST_CASE(resilient_sink_early_return_path)
{
    ...
    FATP_ASSERT_TRUE(primaryContents.find("Primary success") != std::string::npos, "Primary received message");
