...
For performance-critical code, Fat-P provides lower-state specializations: **Stride2DPolicy** for fixed 2D traversal and **Stride1DPolicy** for strided 1D traversal. Measure them on the target compiler and workload.
...
flowchart TB
    ...
    subgraph Tier3["Tier 3: 1D Optimized"]
        T3["Stride1DPolicy<br/>1D strided<br/>Minimal policy state"]
    end
...
| 3 | Stride1DPolicy | Low policy complexity | Column extraction, strided 1D |

---

## Performance

TensorStridePolicy's overhead comes from rank-generic odometer logic (carry propagation across dimensions). Stride2DPolicy eliminates this with fixed two-index arithmetic. Stride1DPolicy reduces to a single multiply-add per step. Manual loops avoid all policy overhead but lose the abstraction. See `components/PolicyIterator/results/` for current platform-specific benchmark data.
