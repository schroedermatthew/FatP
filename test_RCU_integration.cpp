...
bool test_rcu_tensor_basic() {
    ...
    // Verify reads
    {
        ...
        SIMPLE_ASSERT(tensor[0] == 0.0, "First element should be 0");
        SIMPLE_ASSERT(tensor[50] == 50.0, "Middle element should be 50");
        SIMPLE_ASSERT(tensor[99] == 99.0, "Last element should be 99");
    }
...
bool test_rcu_concurrent_readers_writers() {
    ...
    SIMPLE_ASSERT(read_count.load() > 1000, "Should have many reads");
    SIMPLE_ASSERT(write_count.load() == 100, "Should have 100 writes (50*2)");
...
bool test_rcu_performance() {
    ...
    SIMPLE_ASSERT(ns_per_read < 500.0, "RCU reads should be fast");
...
bool test_rcu_complex_updates() {
    ...
    // Verify structure
    {
        auto guard = rcu_tensor.read();
        const auto& tensor = *guard;
        SIMPLE_ASSERT(tensor.shape().size() == 2, "Should be 2D");
        SIMPLE_ASSERT(tensor.shape()[0] == 100, "First dim should be 100");
        SIMPLE_ASSERT(tensor.shape()[1] == 100, "Second dim should be 100");
        SIMPLE_ASSERT(tensor.size() == 10000, "Total size should be 10000");
    }
    ...
    // Verify arithmetic
    {
        auto guard = rcu_tensor.read();
        const auto& tensor = *guard;
        SIMPLE_ASSERT(tensor[0] == 0.0, "First should be 0");
        SIMPLE_ASSERT(tensor[50] == 100.0, "Element 50 should be 100");
    }
