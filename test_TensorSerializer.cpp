...
#include "test_TensorSerializer.h"
#endif
...
TEST_CASE(int8_roundtrip)
{
    ...
    SIMPLE_ASSERT(serialized.has_value(), serialized.error().message.c_str());

    auto deserialized = deserialize_tensor<std::int8_t>(*serialized);
    SIMPLE_ASSERT(deserialized.has_value(), deserialized.error().message.c_str());
...
TEST_CASE(truncated_header)
{
    ...
    SIMPLE_ASSERT(!result.has_value(), "Truncated header should fail");
...
TEST_CASE(wrong_magic)
{
    Tensor<int> t({2, 2});
    auto serialized = serialize_tensor(t);
    SIMPLE_ASSERT(serialized.has_value(), "Serialize should succeed");

    (*serialized)[0] = 0x00;

    auto result = deserialize_tensor<int>(*serialized);
    SIMPLE_ASSERT(!result.has_value(), "Wrong magic should fail");
...
TEST_CASE(wrong_version)
{
    ...
    auto result = deserialize_tensor<int>(*serialized);
    SIMPLE_ASSERT(!result.has_value(), "Wrong version should fail");
...
TEST_CASE(type_mismatch)
{
    ...
    SIMPLE_ASSERT(!result.has_value(), "Type mismatch should fail");
...
TEST_CASE(invalid_ndim_zero)
{
    ...
    auto result = deserialize_tensor<int>(*serialized);
    SIMPLE_ASSERT(!result.has_value(), "Zero ndim should fail");
...
TEST_CASE(invalid_ndim_too_large)
{
    ...
    auto result = deserialize_tensor<int>(*serialized);
    SIMPLE_ASSERT(!result.has_value(), "ndim > 32 should fail");
...
TEST_CASE(truncated_data)
{
    ...
    auto result = deserialize_tensor<double>(*serialized);
    SIMPLE_ASSERT(!result.has_value(), "Truncated data should fail");
...
TEST_CASE(empty_buffer)
{
    ...
    SIMPLE_ASSERT(!result.has_value(), "Empty buffer should fail");
...
TEST_CASE(fuzz_float_2d)
{
    ...
    for (int iter = 0; iter < 100; ++iter)
    {
        ...
        auto serialized = serialize_tensor(t);
        SIMPLE_ASSERT(serialized.has_value(), "Fuzz serialize failed");

        auto deserialized = deserialize_tensor<float>(*serialized);
        SIMPLE_ASSERT(deserialized.has_value(), "Fuzz deserialize failed");
