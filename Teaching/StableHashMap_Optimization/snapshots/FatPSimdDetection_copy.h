 ...
 * @return String like "AVX-512BW", "AVX-512F", "AVX2", "SSE2", "NEON-AArch64",
 * "NEON", "Scalar"
 */
 ...
 *   "CPU supports AVX-512. Recompile with /arch:AVX512 (MSVC) or -mavx512bw
 * (GCC/Clang)" nullptr (if already optimal)
 */
inline const char* optimization_hint() noexcept
{
    ...
    {
        return "CPU supports AVX-512BW. Recompile with /arch:AVX512 (MSVC) or "
               "-mavx512bw (GCC/Clang)";
    }
    if (cpu_has_avx512f() && !compiled_with_avx512f())
    {
        return "CPU supports AVX-512F. Recompile with /arch:AVX512 (MSVC) or "
               "-mavx512f (GCC/Clang)";
    }
    if (cpu_has_avx2() && !compiled_with_avx2())
    {
        return "CPU supports AVX2. Recompile with /arch:AVX2 (MSVC) or -mavx2 "
               "(GCC/Clang)";
    }
#endif
