/**
 * @file benchmark_EqualityComparisonsAny.cpp
 * @brief Unified benchmarks for fat_p::EqualityComparisons and
 * fat_p::EqualityAny.
 *
 ...
 *   FATP_BENCH_TARGET_WORK  - Target element comparisons per benchmark
 * (default: 5000000) FATP_BENCH_BATCHES      - Number of measurement batches
 * (default: 20)
 *
 * Build:
 *   g++ -std=c++17 -O3 -DNDEBUG -march=native
 * benchmark_EqualityComparisonsAny.cpp -o bench_eq
 *
 * Windows (MSVC):
...
{
public:
    explicit BenchmarkScope() = default;
};
#endif
...
struct ExtendedStats
{
    ...
    static ExtendedStats from(const BenchmarkStats& bs, size_t batch_count)
    {
        ...
        {
            es.ci95_low_ms = es.ci95_high_ms = bs.mean_ms;
        }
        return es;
    }
};
...
{
    std::cout << "\n"
              << fat_p::testing::colors::cyan() << fat_p::testing::colors::bold() << title
...
{
    std::cout << fat_p::testing::colors::cyan() << title << fat_p::testing::colors::reset() << "\n"
              << std::string(std::strlen(title), '-') << "\n";
}

void printTableHeader()
{
    std::cout << std::left << std::setw(24) << "Case" << std::right << std::setw(14) << "Fat-P" << std::setw(14)
              << "Baseline" << std::setw(10) << "Speedup" << "\n"
              << std::string(62, '-') << "\n";
}

void printTableHeaderStats()
{
    std::cout << std::left << std::setw(20) << "Case" << std::right << std::setw(12) << "Median" << std::setw(12)
              << "Mean" << std::setw(12) << "Stddev"
              << "  CI95\n"
              << std::string(76, '-') << "\n";
}
...
void printRow(const std::string& label, double fatpTime, double baselineTime)
{
    ...
    std::cout << std::left << std::setw(24) << label << std::right << std::setw(14) << format_time(fatpTime)
              << std::setw(14) << format_time(baselineTime) << std::setw(10) << std::fixed << std::setprecision(2)
              << speedup << "x\n";
}
...
void printRowStats(const std::string& label, const ExtendedStats& s)
{
    ...
    std::cout << std::left << std::setw(20) << label << std::right << std::setw(12) << format_time(s.median_ms)
              << std::setw(12) << format_time(s.mean_ms) << std::setw(12) << format_time(s.stddev_ms) << "  "
              << ci.str() << "\n";
}
...
void printRowPair(const std::string& label, const ExtendedStats& fatp, const ExtendedStats& base)
{
    ...
    std::cout << std::left << std::setw(24) << label << std::right << std::setw(14) << format_time(fatp.median_ms)
              << std::setw(14) << format_time(base.median_ms) << std::setw(10) << std::fixed << std::setprecision(2)
              << speedup << "x\n";
}
...
generateOrderedMap(std::size_t keys, std::size_t vals, std::uint32_t seed = 42)
{
...
generateUnorderedMap(std::size_t keys, std::size_t vals, std::uint32_t seed = 42)
{
...
std::unordered_map<std::string, double> generateUnorderedMapScalar(std::size_t keys, std::uint32_t seed = 42)
{
    ...
    for (std::size_t i = 0; i < keys; ++i)
    {
        m["key_" + std::to_string(i)] = dist(rng);
    }
    return m;
}
                            ...
                            double eps)
{
    ...
    for (; ita != a.end(); ++ita, ++itb)
    {
        ...
        if (!manualVectorEpsilon(ita->second, itb->second, eps))
        {
            return false;
        }
    }
...
bool manualUnorderedMapNestedEpsilon(const std::unordered_map<std::string, std::vector<double>>& a,
                                     const std::unordered_map<std::string, std::vector<double>>& b,
                                     double eps)
{
    if (a.size() != b.size())
    {
    ...
    {
        auto it = b.find(key);
        if (it == b.end() || !manualVectorEpsilon(vec, it->second, eps))
        {
            return false;
        }
    }
                                     ...
                                     double eps)
{
    ...
    {
        auto it = b.find(key);
        if (it == b.end() || std::fabs(val - it->second) > eps)
        {
            return false;
        }
    }
...
bool manualTupleEpsilon(const std::tuple<double, double, double>& a,
                        ...
                        double eps)
{
    return std::fabs(std::get<0>(a) - std::get<0>(b)) <= eps && std::fabs(std::get<1>(a) - std::get<1>(b)) <= eps &&
...
bool manualVectorFatPSemanticsEpsilon(const std::vector<double>& a, const std::vector<double>& b, double eps)
{
    ...
    for (std::size_t i = 0; i < a.size(); ++i)
    {
        if (!manualFatPSemanticsDoubleEpsilon(a[i], b[i], eps))
        {
            return false;
        }
    }
...
void registerExtraAnyTypes()
{
    registerAnyType<std::vector<std::any>>();
}
...
void benchTuple(const BenchConfig& cfg)
{
    ...
    {
        tuplesA[i] = generateTuple3(static_cast<uint32_t>(i * 97 + 1));
        tuplesB[i] = tuplesA[i];
    }
...
void benchMismatchDetection(const BenchConfig& cfg)
{
    ...
    {
        std::cout << "Skipped: kStopOnFirstError=false (no early-exit behavior to "
                     "measure).\n\n";
    }
}
...
int main()
{
    BenchmarkScope scope; // Windows: elevate priority, pin to non-zero core
    ...
    std::cout << fat_p::testing::colors::green() << "Benchmark complete." << fat_p::testing::colors::reset() << "\n";
    return 0;
}
