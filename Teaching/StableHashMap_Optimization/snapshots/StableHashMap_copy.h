...
namespace testing { namespace stablehashmap { template<typename> class StableHashMapTester; } }
...
inline uint8_t H2(size_t hash) { return static_cast<uint8_t>(hash & 0x7F); }
...
struct Group {
    ...
    BitMask match_empty_or_deleted() const {
        ...
        // Exclude sentinel bytes - use scalar array since vgetq_lane_u8 requires constant index
...
public:
    ...
    template <typename MapType> friend class fat_p::testing::stablehashmap::StableHashMapTester;
...
private:
    ...
    std::pair<size_t, bool> find_slot(const K& key, size_t h) const {
        if (FATP_UNLIKELY(capacity_ == 0)) return {SIZE_MAX, false};
