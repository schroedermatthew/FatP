 ...
 * @brief Unified benchmarks for fat_p::EqualityComparisons and fat_p::EqualityAny.
 *
 ...
 *   FATP_BENCH_BATCHES      - Number of measurement batches (default: 20)
 *
 * Build:
...
public:
    ...
    ~BenchmarkScope()
    {
        ...
        if (old_affinity_ != 0) SetThreadAffinityMask(GetCurrentThread(), old_affinity_);
    }
    BenchmarkScope(const BenchmarkScope&) = delete;
    BenchmarkScope& operator=(const BenchmarkScope&) = delete;
};
#else
class BenchmarkScope { public: explicit BenchmarkScope() = default; };
#endif
...
struct BenchConfig { std::size_t targetWork; std::size_t batches; bool verboseStats; };
