...
public:
    ...
    {
        if (map.ctrl_ == nullptr || map.size_ == 0)
        {
            return 0.0;
        }
        ...
        for (size_t i = 0; i < map.capacity_; ++i)
        {
            if (map.ctrl_[i] < 0x80 && map.nodes_[i] != nullptr)
            {
                ...
                size_t dist = (i >= ideal) ? (i - ideal) : (i + map.capacity_ - ideal);
                total_dist += dist;
                ++count;
            }
        }
...
TEST_CASE(raii_erase_correctness)
{
    ...
    ASSERT_EQ(TrackedRAII::ctor_count, TrackedRAII::dtor_count, "Ctor/dtor count should match");
...
TEST_CASE(reference_stability_across_insert)
{
    ...
    ASSERT_TRUE(static_cast<const void*>(ptr1_after) == addr1, "Pointer address should be unchanged after rehash");
...
TEST_CASE(reference_stability_mixed_operations)
{
    ...
    for (size_t j = 0; j < stable_ptrs.size(); ++j)
    {
        ...
        ASSERT_EQ(*stable_ptrs[j], stable_keys[j] * 100, "Value should be correct through stable pointer");
    }
...
TEST_CASE(reference_stability_with_strings)
{
    ...
    ASSERT_TRUE(static_cast<const void*>(map.find(long_key)) == addr, "Pointer should be stable");
...
// Uses string_view consistently to ensure identical hashes for identical
// content
...
TEST_CASE(heterogeneous_try_emplace_rvalue)
{
    ...
    auto make_long_key = []() {
        return std::string("this_is_a_very_long_key_that_exceeds_sso_buffer_size_"
                           "on_most_implementations");
    };
    ...
    ASSERT_TRUE(map.contains("this_is_a_very_long_key_that_exceeds_sso_buffer_"
                             "size_on_most_implementations"),
...
TEST_CASE(churn_probe_distance_stability)
{
    ...
    ASSERT_TRUE(avg_delta < 3.0, "Average probe distance degraded significantly");

    // Max probe can vary more due to key distribution luck
    ASSERT_TRUE(aged_max < baseline_max + 25, "Maximum probe distance grew excessively");
...
TEST_CASE(churn_no_ghost_slots)
{
    ...
    ASSERT_EQ(full_slots, map.size(), "Full slots should equal size()");
    ...
    ASSERT_EQ(manual_count, map.size(), "Iteration count should match size()");
...
TEST_CASE(churn_latency_stability)
{
    ...
    ASSERT_TRUE(ratio < 1.50, "Lookup performance degraded excessively after churn");
...
double bench_ns_per_op(ResetFn&& reset_fn, Fn&& fn)
{
    ...
    if (med <= 0.0)
    {
        std::cerr << "[bench] WARNING: invalid measurement (" << med << " ns/op). Skipping sample.\n";
...
void benchmark_find(size_t N)
{
    ...
    std::cout << std::setw(8) << N << " | FIND   | StableHashMap " << fmap_ns << " ns | unordered_map " << umap_ns
...
void benchmark_insert(size_t N)
{
    ...
    std::cout << std::setw(8) << N << " | INSERT | StableHashMap " << fmap_ns << " ns | unordered_map " << umap_ns
              << " ns | Speedup " << (umap_ns / fmap_ns) << "x\n";
}
...
void benchmark_erase(size_t N)
{
    ...
    std::cout << std::setw(8) << N << " | ERASE  | StableHashMap " << fmap_ns << " ns | unordered_map " << umap_ns
              << " ns | Speedup " << (umap_ns / fmap_ns) << "x\n";
}
...
void benchmark_build_from_empty(size_t N)
{
    ...
    std::cout << std::setw(8) << N << " | BUILD  | StableHashMap " << fmap_ns_per_elem << " ns/elem | unordered_map "
              << umap_ns_per_elem << " ns/elem | Speedup " << (umap_ns_per_elem / fmap_ns_per_elem) << "x\n";
}
...
void benchmark_rehash_inclusive(size_t N)
{
    ...
    std::cout << std::setw(8) << N << " | REHASH | StableHashMap " << fmap_ns << " ns/op | unordered_map " << umap_ns
...
void benchmark_mixed(size_t N)
{
    ...
    std::cout << std::setw(8) << N << " | MIXED  | StableHashMap " << fmap_ns << " ns/op | unordered_map " << umap_ns
              << " ns/op | Speedup " << (umap_ns / fmap_ns) << "x\n";
}
...
void benchmark_hash_quality(size_t N)
{
    ...
    std::cout << "Hit: SHM(std)=" << std::setw(7) << std_hit_ns << " SHM(SM64)=" << std::setw(7) << sm_hit_ns << " ("
              << (std_hit_ns < sm_hit_ns ? "std wins" : "SM64 wins") << ") | ";
    std::cout << "Miss: SHM(std)=" << std::setw(7) << std_miss_ns << " SHM(SM64)=" << std::setw(7) << sm_miss_ns << " ("
...
void benchmark_stablehashmap()
{
    ...
    std::cout << "\n=== BUILD FROM EMPTY ===\n";
    std::cout << "Rebuild from empty is prohibitively slow for a large number of "
                 "values.\n";
    std::cout << "Large maps sizes are not appropriate for these benchmark "
                 "sanity checks.\n";
    ...
    std::cout << "\n=== HASH QUALITY IMPACT (StableHashMap: std::hash vs "
                 "SplitMix64) ===\n";
    std::cout << "Compares StableHashMap<K,V> vs StableHashMap<K,V,SplitMix64Hash>.\n";
    std::cout << "SplitMix64: high-quality 64-bit mixer (not borrowed from any "
                 "library).\n";
    std::cout << "SHM(std) = StableHashMap with std::hash, SHM(SM64) = "
                 "StableHashMap with SplitMix64.\n\n";
    ...
    for (size_t N : {1'000, 10'000, 100'000})
    {
        ...
        using HeteroMap = StableHashMap<std::string, size_t, TransparentStringHash, TransparentStringEqual>;
        HeteroMap map;
        map.reserve(N);
        for (size_t i = 0; i < N; ++i)
        {
    ...
    {
        const size_t base_elements = static_cast<size_t>(LF_BUCKETS * target_load);
        ...
        std::iota(insert_batch.begin(), insert_batch.end(), static_cast<int>(base_elements));

        std::vector<int> find_batch(base_keys.begin(), base_keys.begin() + BATCH_SIZE);
        std::shuffle(find_batch.begin(), find_batch.end(), std::mt19937(999));

        std::vector<int> erase_batch(base_keys.begin(), base_keys.begin() + BATCH_SIZE);
        ...
        double insert_ns = bench_ns_per_op<BATCH_SIZE>(insert_reset, [&]() {
        ...
        double erase_ns = bench_ns_per_op<BATCH_SIZE>(erase_reset, [&]() {
        ...
        std::cout << "    " << std::setw(5) << (target_load * 100) << "% |" << std::setw(11) << find_ns << "  |"
                  << std::setw(12) << insert_ns << "  |" << std::setw(11) << erase_ns << "\n";
    }
