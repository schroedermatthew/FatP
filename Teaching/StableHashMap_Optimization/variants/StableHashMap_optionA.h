...
bool diagnostic_find(const Key& key, ProbeCounters& out) const
{
    ...
    while (true) {
        ...
        const uint32_t full = static_cast<uint32_t>(stablehash_detail::Group::kWidth) -
                              stablehash_detail::popcount32(available_mask);
        out.mFullSlotsVisited += full;

        const stablehash_detail::BitMask matches = g.match(h2);
...
private:
    ...
    std::pair<size_t, bool> find_slot(const K& key, size_t h) const {
        ...
        while (true) {
            ...
            if (g.match_empty()) return {SIZE_MAX, false};
            seq.next();
        }
    }
