...
//   - P0 Critical: FixedHashMap non-movable/non-swappable (prevents dangling
//   pointers)
...
//     NOTE: FixedHashMap is NON-MOVABLE and NON-SWAPPABLE (pointers would
//     dangle)
//
// Usage:
...
//   fat_p::FixedHashMap<K, V, 8192> fixed;  // Tombstone + 8KB fixed buffer
//   (non-movable!)
//
// Hash Finalizer (SplitMix64):
//   By default, a SplitMix64 finalizer is applied to all hashes to protect
//   against poor hash functions (e.g., std::hash<int> is identity on many
//   platforms).
//
//   To disable the finalizer for hash functions that already have good
//   avalanche properties (e.g., absl::Hash, wyhash), define `is_avalanching` in
//   your hash:
//
//   struct MyGoodHash {
//       using is_avalanching = void;  // Opt-out of built-in mixer
//       size_t operator()(int64_t x) const { return wyhash(&x, sizeof(x), 0,
//       _wyp); }
//   };
...
// =============================================================================
// MSVC requires C++20 for [[no_unique_address]], use
// [[msvc::no_unique_address]] in C++17
#if defined(_MSC_VER)
...
/// Best for: theoretical advantage on miss-after-delete (not observed in
/// practice)
...
/// Uses platform-appropriate aligned allocation (posix_memalign,
/// _aligned_malloc)
...
/// need move semantics.
///
/// Template parameter N is approximate byte capacity (actual may be slightly
/// more due to internal alignment requirements).
///
...
///                      fat_p::TombstoneDeletion, fat_p::FixedAllocator<4096>>
///                      map;
///
...
// --- AVX-512BW: 64-byte groups (commented out - frequency throttling concerns)
// --- Uncomment to enable. Requires: -mavx512bw (GCC/Clang) or /arch:AVX512
// (MSVC) WARNING: AVX-512 can cause 10-20% frequency reduction on some Intel
// CPUs.
/*
...
public:
    ...
    BitMask match(uint8_t h2) const {
        ...
        return BitMask(static_cast<uint32_t>(_mm512_cmpeq_epi8_mask(ctrl_,
match_vec)));
    }
...
zero)));
    ...
    BitMask match_empty_or_deleted() const {
        // Full slots have h2 with high bit SET (0x80-0xFF range after H2
transform)
        // Empty (0x00), Deleted (0x7E), Sentinel (0x7F) have high bit CLEAR
        // We want empty and deleted but NOT sentinel
        ...
        return BitMask(static_cast<uint32_t>(has_high_bit_clear &
~is_sentinel));
    }
...
public:
    ...
    BitMask match_empty_or_deleted() const
    {
        // Full slots have h2 with high bit SET (0x80-0xFF range after H2 transform)
        // Empty (0x00), Deleted (0x7E), Sentinel (0x7F) have high bit CLEAR
        // We want empty and deleted but NOT sentinel
...
public:
    ...
    void next()
    {
        offset_ = (offset_ + Group::kWidth) & mask_;
    }
...
private:
    ...
    std::conditional_t<uses_tombstones, size_t, EmptyMember> tombstones_{};
    ...
    // noexcept specifications for move/swap - conditional on
    ...
    // These ensure find/erase/contains don't hard-error when Hash/KeyEqual can't
    // handle K
    template <typename K>
    ...
    // Applied by default to protect against bad user hashes (e.g., std::hash<int>
    // is identity) Skipped if Hash::is_avalanching is defined (e.g., absl::Hash,
    // wyhash) Uses SplitMix64 on 64-bit, MurmurHash3 finalizer on 32-bit (avoids
    // UB from >> 33)
    template <typename K>
    size_t hash_key(const K& k) const
    {
        ...
        {
            // User's hash already has good distribution, skip mixer
            return h ? h : 1;
        }
        else
        {
        ...
        if (old_ctrl)
        {
            for (size_t i = 0; i < old_cap; ++i)
            {
                if (fasthash_detail::is_full(old_ctrl[i]))
                {
                    ...
                    new (&slots_[idx]) Slot();
                    slots_[idx].emplace(std::move(old_slots[i].key), std::move(old_slots[i].value), home);
                    old_slots[i].destroy();
                    ++size_;
                }
            }
...
public:
    ...
    FastHashMap(const FastHashMap& other)
        ...
        , allocator_()
    { // Default construct allocator (can't copy stateful
      // allocators)
        if (other.capacity_ > 0)
        {
            ...
            for (size_t i = 0; i < other.capacity_; ++i)
            {
                if (fasthash_detail::is_full(other.ctrl_[i]))
                {
                    ...
                    new (&slots_[i]) Slot();
                    slots_[i].emplace(other.slots_[i].key, other.slots_[i].value, other.slots_[i].home());
                }
    ...
    // Lookup - Supports heterogeneous lookup (e.g., find with string_view for
    // string keys)
    template <typename K, std::enable_if_t<is_hetero_lookup_enabled<K>, int> = 0>
    ...
    public:
        ...
        const Key& key() const
        {
            return slots_[idx_].key;
        }
        Value& value() const
        {
            return slots_[idx_].value;
        }
        ...
        bool operator==(const iterator& o) const
        {
            return ctrl_ == o.ctrl_ && idx_ == o.idx_;
        }
        bool operator!=(const iterator& o) const
        {
            return !(*this == o);
    ...
    private:
        void skip_empty()
        {
            while (idx_ < cap_ && !fasthash_detail::is_full(ctrl_[idx_]))
            {
                ++idx_;
            }
        }
    ...
    iterator begin()
    {
        return iterator(ctrl_, slots_, 0, capacity_);
    }
    iterator end()
    {
        return iterator(ctrl_, slots_, capacity_, capacity_);
    }
