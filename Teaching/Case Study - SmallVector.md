...
status: "reviewed"
---
# Case Study - Multi-Index Design, Stack/Heap/Cache, and Why `SmallVector` Wins

## Scope
This case study focuses on why using `std::vector` for small multi-index arguments (e.g., `{i, j, k}`) inside tight tensor-style loops can be a performance disaster, and how **inline storage** (SmallVector / small-buffer optimization) removes heap traffic from the hot path.

## Not covered
- Full tensor library design (broadcasting, views, slicing, strides, ownership models).
- SIMD/vectorization of the math kernel (this is about **index container costs**).
- Multithreading/task schedulers.
...
## Case Study Card
**Problem:** Clean tensor indexing syntax accidentally triggers heap allocation per access.  
**Constraint:** Heap alloc/free + cache misses dominate when the index container is tiny but dynamically allocated.  
**Symptom:** Runtime dominated by allocator overhead (millions of malloc/free calls).  
**Root cause:** Passing/constructing a heap-allocated dynamic container (`std::vector`) for fixed-size index tuples.  
**Fix pattern:** Use inline storage (`SmallVector`, `std::array<N>`, or a fixed-size span-like index view) for small multi-indices.  
**FAT‑P components used:** `SmallVector` (or equivalent inlined-vector facility).  
**Build-mode gotchas:** None specific; performance differences may change under sanitizers.  
**Guarantees:** Removes per-access heap allocations for small index sizes (within inline capacity).  
**Non‑guarantees:** Does not guarantee speedups when indices are large/dynamic, or when the kernel is dominated by other costs.

## Table of Contents
- [⚠️ Before You Read Further: The Disaster You May Have Written](#before-you-read-further-the-disaster-you-may-have-written)
- [A Quick Conversation Before We Dive In](#a-quick-conversation-before-we-dive-in)
- [1. Motivation: Tensor Multi-Index the "Easy" Way](#1-motivation-tensor-multi-index-the-easy-way)
- [2. The Memory Hierarchy: Why Your Computer Has So Many Kinds of Memory](#2-the-memory-hierarchy-why-your-computer-has-so-many-kinds-of-memory)
- [3. Stack vs Heap: Two Very Different Ways to Get Memory](#3-stack-vs-heap-two-very-different-ways-to-get-memory)
- [4. std::vector Multi-Index: What Actually Happens](#4-stdvector-multi-index-what-actually-happens)
- [5. Why This Design Is Slow (Hardware Perspective)](#5-why-this-design-is-slow-hardware-perspective)
- [6. Enter `SmallVector`: Inline Storage as the Fix](#6-enter-smallvector-inline-storage-as-the-fix)
- [7. Tensor API with SmallVector](#7-tensor-api-with-smallvector)
- [8. Cache Behavior with SmallVector](#8-cache-behavior-with-smallvector)
- [9. Passing SmallVector: Value vs Reference](#9-passing-smallvector-value-vs-reference)
- [10. Putting It All Together](#10-putting-it-all-together)
- [11. Recommended Tensor Multi-Index Pattern (Final)](#11-recommended-tensor-multi-index-pattern-final)
- [12. Empirical Validation: Benchmark Results](#12-empirical-validation-benchmark-results)
- [13. Conclusion](#13-conclusion)
- [What To Do Now](#what-to-do-now)
- [One Last Thing](#one-last-thing)
- [Appendix A: Raw Benchmark Data](#appendix-a-raw-benchmark-data)
- [Appendix B: Benchmark Source Code](#appendix-b-benchmark-source-code)
...
graph TB
    ...
    style L1 fill:#FFFF99,color:#000
    style L2 fill:#FFD700,color:#000
    style L3 fill:#FFA500,color:#000
    style DRAM fill:#FF6B6B,color:#000
    style OFFCHIP fill:#ff9999,stroke:#ff0000,stroke-width:2px,color:#000
```
...
graph LR
    ...
    style CL0 fill:#90EE90,stroke:#333,stroke-width:2px,color:#000
```
...
graph TB
    ...
    style RSP fill:#ff9999,stroke:#ff0000,stroke-width:3px,color:#000
    style UNUSED fill:#f0f0f0,stroke-dasharray: 5 5,color:#000
```
...
graph TB
    ...
    style COUNT fill:#90EE90,color:#000
    style IDX fill:#90EE90,color:#000
```
...
graph LR
    ...
    style HEADER fill:#ffcccc,color:#000
    style DATA fill:#90EE90,color:#000
```
...
graph LR
    ...
    style F3 fill:#90EE90,stroke:#00aa00,stroke-width:3px,color:#000
```
...
graph TB
    ...
    style FREE64 fill:#f0f0f0,color:#000
    style YOUR fill:#90EE90,color:#000
    style REMAIN fill:#ffffcc,color:#000
```
...
graph TB
    ...
    style B3 fill:#ff9999,stroke:#ff0000,stroke-width:2px,color:#000
