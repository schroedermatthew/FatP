...
TEST_CASE(throw_on_null)
{
    ...
    SIMPLE_ASSERT(caught, "Should throw on null load");

    AtomicSharedPtr<TestData, false> non_throwing_ref;
    auto val = non_throwing_ref.load();
    SIMPLE_ASSERT(!val, "Should return null without throwing");
...
TEST_CASE(concurrent_stores)
{
    ...
    SIMPLE_ASSERT(ref.load(), "Final value should exist");
...
TEST_CASE(lock_free_query)
{
    ...
    if (always_lf)
    {
        SIMPLE_ASSERT(lf, "is_always_lock_free implies is_lock_free");
    }
#else
    SIMPLE_ASSERT(!lf, "C++17 path always returns false for is_lock_free");
    SIMPLE_ASSERT(!always_lf, "C++17 path always returns false for is_always_lock_free");
#endif
...
TEST_CASE(bool_conversion)
{
    ...
    SIMPLE_ASSERT(!empty_ref, "Empty ref should be false");

    AtomicSharedPtr<TestData> valid_ref(std::make_shared<TestData>(1));
    SIMPLE_ASSERT(static_cast<bool>(valid_ref), "Valid ref should be true");
